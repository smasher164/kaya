...
// little-endian. Constants come from kaya.h through the CKaya module.

internal import CKaya
import Foundation

public enum KayaValue: Hashable {
    case bool(Bool)
    case i64(Int64)
    case f64(Double)
    case str(String)
...
struct KayaTx {
    ...
    mutating func setColumnHeaders(_ widgetId: UInt64, _ sorted: UInt32, _ direction: UInt32, _ count: UInt32, _ pathLen: UInt32, _ titles: [KayaValue]) {
    ...
    mutating func createBreakpoint(_ window: UInt64, _ sizeClass: KayaValue, _ count: UInt32, _ setters: [KayaValue]) {
        let kayaAt = self.begin(UInt16(KAYA_TX_CREATE_BREAKPOINT))
        self.u64(window)
        self.value(sizeClass)
        self.u32(count)
        self.u32(0)
        self.values(setters)
        self.end(kayaAt)
    ...
    mutating func setRichText(_ widgetId: UInt64, _ count: UInt32, _ runs: [KayaValue], _ text: KayaValue) {
    ...
    mutating func applyEdit(_ widgetId: UInt64, _ start: UInt64, _ stop: UInt64, _ count: UInt32, _ runs: [KayaValue], _ text: KayaValue) {
    ...
    mutating func setAxis(_ widgetId: UInt64, _ axis: Int64) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_AXIS))
        self.u32(UInt32(KAYA_SOURCE_CONST))
        self.value(.i64(axis))
        self.end(kayaAt)
    }
    ...
    mutating func bindAxis(_ widgetId: UInt64, _ signalId: UInt64) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_AXIS))
        self.u32(UInt32(KAYA_SOURCE_SIGNAL))
    ...
    /// set_property bound to one field of the element of the
    /// enclosing For, `level` Fors up (0 = nearest).
    mutating func bindAxisElement(_ widgetId: UInt64, level: UInt32 = 0, field: UInt32 = 0) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_AXIS))
        self.u32(UInt32(KAYA_SOURCE_ELEMENT))
    ...
    mutating func setHelp(_ widgetId: UInt64, _ help: String) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_HELP))
        self.u32(UInt32(KAYA_SOURCE_CONST))
        self.value(.str(help))
        self.end(kayaAt)
    }
    ...
    mutating func bindHelp(_ widgetId: UInt64, _ signalId: UInt64) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_HELP))
        self.u32(UInt32(KAYA_SOURCE_SIGNAL))
    ...
    /// set_property bound to one field of the element of the
    /// enclosing For, `level` Fors up (0 = nearest).
    mutating func bindHelpElement(_ widgetId: UInt64, level: UInt32 = 0, field: UInt32 = 0) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_HELP))
        self.u32(UInt32(KAYA_SOURCE_ELEMENT))
    ...
    mutating func setFill(_ widgetId: UInt64, _ fill: Bool) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_FILL))
        self.u32(UInt32(KAYA_SOURCE_CONST))
        self.value(.bool(fill))
        self.end(kayaAt)
    }

    /// set_property with a signal-bound fill value.
    mutating func bindFill(_ widgetId: UInt64, _ signalId: UInt64) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_FILL))
        self.u32(UInt32(KAYA_SOURCE_SIGNAL))
    ...
    /// set_property bound to one field of the element of the
    /// enclosing For, `level` Fors up (0 = nearest).
    mutating func bindFillElement(_ widgetId: UInt64, level: UInt32 = 0, field: UInt32 = 0) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_FILL))
        self.u32(UInt32(KAYA_SOURCE_ELEMENT))
    ...
    mutating func setMinColumnWidth(_ widgetId: UInt64, _ minColumnWidth: Double) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_MIN_COLUMN_WIDTH))
        self.u32(UInt32(KAYA_SOURCE_CONST))
        self.value(.f64(minColumnWidth))
        self.end(kayaAt)
    }

    /// set_property with a signal-bound min_column_width value.
    mutating func bindMinColumnWidth(_ widgetId: UInt64, _ signalId: UInt64) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_MIN_COLUMN_WIDTH))
        self.u32(UInt32(KAYA_SOURCE_SIGNAL))
    ...
    /// set_property bound to one field of the element of the
    /// enclosing For, `level` Fors up (0 = nearest).
    mutating func bindMinColumnWidthElement(_ widgetId: UInt64, level: UInt32 = 0, field: UInt32 = 0) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_MIN_COLUMN_WIDTH))
        self.u32(UInt32(KAYA_SOURCE_ELEMENT))
    ...
    mutating func setWrap(_ widgetId: UInt64, _ wrap: Bool) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_WRAP))
        self.u32(UInt32(KAYA_SOURCE_CONST))
        self.value(.bool(wrap))
        self.end(kayaAt)
    }

    /// set_property with a signal-bound wrap value.
    mutating func bindWrap(_ widgetId: UInt64, _ signalId: UInt64) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_WRAP))
        self.u32(UInt32(KAYA_SOURCE_SIGNAL))
    ...
    /// set_property bound to one field of the element of the
    /// enclosing For, `level` Fors up (0 = nearest).
    mutating func bindWrapElement(_ widgetId: UInt64, level: UInt32 = 0, field: UInt32 = 0) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_WRAP))
        self.u32(UInt32(KAYA_SOURCE_ELEMENT))
    ...
    mutating func setDocument(_ widgetId: UInt64, _ handle: UInt64) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_DOCUMENT))
        self.u32(UInt32(KAYA_SOURCE_CONST))
        self.value(.blob(handle))
        self.end(kayaAt)
    }
    ...
    mutating func bindDocument(_ widgetId: UInt64, _ signalId: UInt64) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_DOCUMENT))
        self.u32(UInt32(KAYA_SOURCE_SIGNAL))
    ...
    /// set_property bound to one field of the element of the
    /// enclosing For, `level` Fors up (0 = nearest).
    mutating func bindDocumentElement(_ widgetId: UInt64, level: UInt32 = 0, field: UInt32 = 0) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_DOCUMENT))
        self.u32(UInt32(KAYA_SOURCE_ELEMENT))
    ...
    mutating func setFilled(_ widgetId: UInt64, _ filled: Int64) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_FILLED))
        self.u32(UInt32(KAYA_SOURCE_CONST))
        self.value(.i64(filled))
        self.end(kayaAt)
    }

    /// set_property with a signal-bound filled value.
    mutating func bindFilled(_ widgetId: UInt64, _ signalId: UInt64) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_FILLED))
        self.u32(UInt32(KAYA_SOURCE_SIGNAL))
    ...
    /// set_property bound to one field of the element of the
    /// enclosing For, `level` Fors up (0 = nearest).
    mutating func bindFilledElement(_ widgetId: UInt64, level: UInt32 = 0, field: UInt32 = 0) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_FILLED))
        self.u32(UInt32(KAYA_SOURCE_ELEMENT))
    ...
    mutating func setFollowsEnd(_ widgetId: UInt64, _ followsEnd: Bool) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_FOLLOWS_END))
        self.u32(UInt32(KAYA_SOURCE_CONST))
        self.value(.bool(followsEnd))
        self.end(kayaAt)
    }

    /// set_property with a signal-bound follows_end value.
    mutating func bindFollowsEnd(_ widgetId: UInt64, _ signalId: UInt64) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_FOLLOWS_END))
        self.u32(UInt32(KAYA_SOURCE_SIGNAL))
    ...
    /// set_property bound to one field of the element of the
    /// enclosing For, `level` Fors up (0 = nearest).
    mutating func bindFollowsEndElement(_ widgetId: UInt64, level: UInt32 = 0, field: UInt32 = 0) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_FOLLOWS_END))
        self.u32(UInt32(KAYA_SOURCE_ELEMENT))
    ...
    mutating func setMaxLines(_ widgetId: UInt64, _ maxLines: Double) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_MAX_LINES))
        self.u32(UInt32(KAYA_SOURCE_CONST))
        self.value(.f64(maxLines))
        self.end(kayaAt)
    }

    /// set_property with a signal-bound max_lines value.
    mutating func bindMaxLines(_ widgetId: UInt64, _ signalId: UInt64) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_MAX_LINES))
        self.u32(UInt32(KAYA_SOURCE_SIGNAL))
    ...
    /// set_property bound to one field of the element of the
    /// enclosing For, `level` Fors up (0 = nearest).
    mutating func bindMaxLinesElement(_ widgetId: UInt64, level: UInt32 = 0, field: UInt32 = 0) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_MAX_LINES))
        self.u32(UInt32(KAYA_SOURCE_ELEMENT))
    ...
    mutating func setSymbol(_ widgetId: UInt64, _ symbol: Int64) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
    ...
    mutating func bindSymbol(_ widgetId: UInt64, _ signalId: UInt64) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_SYMBOL))
        self.u32(UInt32(KAYA_SOURCE_SIGNAL))
    ...
    /// set_property bound to one field of the element of the
    /// enclosing For, `level` Fors up (0 = nearest).
    mutating func bindSymbolElement(_ widgetId: UInt64, level: UInt32 = 0, field: UInt32 = 0) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_PROPERTY))
        self.u64(widgetId)
        self.u32(UInt32(KAYA_PROP_SYMBOL))
        self.u32(UInt32(KAYA_SOURCE_ELEMENT))
    ...
    mutating func setWindowRememberFrame(_ window: UInt64, _ rememberFrame: Bool) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_WINDOW_PROP))
        self.u64(window)
        self.u32(UInt32(KAYA_WPROP_REMEMBER_FRAME))
        self.u32(UInt32(KAYA_SOURCE_CONST))
        self.value(.bool(rememberFrame))
        self.end(kayaAt)
    }

    /// set_window_prop with a signal-bound remember_frame value (window 0, the primary surface).
    mutating func bindWindowRememberFrame(_ window: UInt64, _ signalId: UInt64) {
        let kayaAt = self.begin(UInt16(KAYA_TX_SET_WINDOW_PROP))
        self.u64(window)
        self.u32(UInt32(KAYA_WPROP_REMEMBER_FRAME))
        self.u32(UInt32(KAYA_SOURCE_SIGNAL))
...
/// (docs/dnd-plan.md D1).
func kayaParseRepresentation(_ raw: UnsafeRawBufferPointer, _ at: Int)
    -> (parts: [KayaClipPart], next: Int)
{
    ...
    -> (clip: KayaClipValues, next: Int)
{
    ...
    let (parts, next) = kayaParseRepresentation(raw, at + 8)
    return (KayaClipValues(kind: kind, parts: parts), next)
}
...
/// landed before it.
struct KayaDropValues {
    let operation: UInt32
    let before: Bool
    let x: Double
    let y: Double
    let anchor: [KayaValue]
    let clip: KayaClipValues
}
...
typealias KayaOccurrence = (kind: UInt16, id: UInt64, keys: [KayaValue], payload: KayaValue?, files: [KayaPickedFile], clip: KayaClipValues?, drop: KayaDropValues?, tail: [KayaValue])

func kayaParseOccurrence(_ rec: [UInt8]) -> KayaOccurrence? {
    rec.withUnsafeBytes { raw -> KayaOccurrence? in
        ...
        var drop: KayaDropValues? = nil
        if kind == UInt16(KAYA_OCCURRENCE_DROPPED)
        {
            let operation = raw.loadUnaligned(fromByteOffset: at, as: UInt32.self)
            let before = raw.loadUnaligned(fromByteOffset: at + 4, as: UInt32.self) != 0
            let anchorLen = Int(raw.loadUnaligned(fromByteOffset: at + 8, as: UInt32.self))
            let clipKind = raw.loadUnaligned(fromByteOffset: at + 12, as: UInt32.self)
            var dropAt = at + 16
            var point: [Double] = []
            for _ in 0..<2 {
                point.append(Double(bitPattern:
                    raw.loadUnaligned(fromByteOffset: dropAt + 8, as: UInt64.self)))
                dropAt += 16
            }
            var anchor: [KayaValue] = []
            for _ in 0..<anchorLen {
                let ktype = raw.loadUnaligned(fromByteOffset: dropAt, as: UInt32.self)
                let klen = Int(raw.loadUnaligned(fromByteOffset: dropAt + 4, as: UInt32.self))
                switch ktype {
                case UInt32(KAYA_VALUE_BOOL):
                    anchor.append(.bool(raw[dropAt + 8] != 0))
                case UInt32(KAYA_VALUE_I64):
                    anchor.append(.i64(raw.loadUnaligned(fromByteOffset: dropAt + 8, as: Int64.self)))
                case UInt32(KAYA_VALUE_F64):
                    anchor.append(.f64(Double(bitPattern:
                        raw.loadUnaligned(fromByteOffset: dropAt + 8, as: UInt64.self))))
                default:
                    anchor.append(.str(String(
                        decoding: raw[(dropAt + 8)..<(dropAt + 8 + klen)], as: UTF8.self)))
                }
                dropAt += 8 + ((klen + 7) & ~7)
            }
            let parts = kayaParseRepresentation(raw, dropAt).parts
            drop = KayaDropValues(
                operation: operation, before: before, x: point[0], y: point[1],
                anchor: anchor, clip: KayaClipValues(kind: clipKind, parts: parts))
        }
        if kind == UInt16(KAYA_OCCURRENCE_DRAG_ENDED)
        {
        ...
        return (kind, id, keys, payload, [], clip, drop, tail)
    }
}
