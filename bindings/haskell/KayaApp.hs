...
-- records with the opener and template_end.
bracketTpl :: (BuildState -> (Word64, BuildState)) -> (Word64 -> Builder) -> Tpl a
           -> BuildState -> ((Word64, a), BuildState)
bracketTpl alloc opener (Tpl body) s0 =
  let (self, s1) = alloc s0
      s2 = s1 {bRecords = bRecords s1 <> opener self}
      (a, s3) = body s2
      s4 = s3 {bRecords = bRecords s3 <> W.txTemplateEnd}
   in ((self, a), s4)
...
instance Declare Build where
  ...
  collection = Build $ \s ->
    let c = bCounters s
        n = cCollection c + 1
        s' = s {bCounters = c {cCollection = n}}
     in (Collection n, s' {bRecords = bRecords s' <> W.txCreateCollection n})
  forEach (Collection cid) body =
    Build $ \s ->
      let ((self, a), s') =
            bracketTpl (unBuild allocW) (`W.txCreateFor` cid) body s
       in ((Widget self, a), s')
  when_ (Signal sid) body =
    Build $ \s ->
      let ((self, a), s') =
            bracketTpl (unBuild allocW) (`W.txCreateWhen` sid) body s
       in ((Widget self, a), s')
...
instance Declare Tpl where
  ...
  forEach (Collection cid) body =
    Tpl $ \s ->
      let ((self, a), s') =
            bracketTpl (unTpl allocN) (`W.txCreateFor` cid) body s
       in ((Node self, a), s')
  when_ (Signal sid) body =
    Tpl $ \s ->
      let ((self, a), s') =
            bracketTpl (unTpl allocN) (`W.txCreateWhen` sid) body s
       in ((Node self, a), s')
...
insert (Collection n) path key value = emitB (W.txCollectionInsert n path key value)

update :: Collection -> [W.Value] -> W.Value -> W.Value -> Build ()
update (Collection n) path key value = emitB (W.txCollectionUpdate n path key value)

remove :: Collection -> [W.Value] -> W.Value -> Build ()
remove (Collection n) path key = emitB (W.txCollectionRemove n path key)
...
-- the block's result (the handles the app keeps).
buildTx :: App -> Build a -> IO a
buildTx app (Build f) = do
  counters <- readIORef (appCounters app)
  let (a, s) = f (BuildState counters mempty)
...
kayaMain :: (App -> IO ()) -> IO ()
kayaMain setup = do
  app <- App <$> newIORef (Counters 0 0 0 0) <*> newIORef Map.empty <*> newIORef Map.empty
  setup app
  done <- newEmptyMVar
