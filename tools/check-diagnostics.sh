...
# The Rust arm's subject: the core's asset diagnostic, whose sentence is
# the one every binding raises. Named once, here, so the two negatives
# below and the python that doctors them cannot drift onto two paths.
RUST_TARGET=crates/kaya/src/assets.rs

TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT

# $1: subcommand. audit ROOT... [REAL=DOCTORED ...]
#     splice PREFIX_FILE OUT | sole OUT            (the swift negatives)
#     rust-sole TARGET OUT | rust-prose TARGET OUT (the rust negatives)
run_py() {
    python3 - "$@" <<'PY'
import os
import re
...
# Rust's lexer. `//` and `///` line comments, NESTED `/* */`, `"…"` with
# backslash escapes and raw newlines allowed (a `\` at end of line is
# Rust's line continuation and the escape rule eats it), the raw forms
# `r"…"` / `r#"…"#` / `r##"…"##` with their byte and c prefixes, and
# CHAR LITERALS, which are lexed only so they cannot derail the reader:
# `'"'` would otherwise open a string that never closes and `'{'` would
# be counted as a bracket. A `'` that is not a well-formed char literal
# is a lifetime (`&'static str`) and is left alone. Rust has no `\(…)`
# interpolation — placeholders live INSIDE the literal and are read
# later, by `placeholder()`.
...
def block_value(code, open_at, close_at):
    ...
    `()`, has no answer of its own, and its `return`s are counted where
    every other `return` is.
    """
...
def expr_end(code, start, stop, terminator):
    ...
    bracket that closes the construct around it. No newline heuristic is
    needed the way Swift's rule needs one — Rust punctuates."""
    i, depth = start, 0
    while i < stop:
        c = code[i]
        if c in "([{":
...
def site_literals(code, spans, start, end):
    """One value position's literals, with whether each MAY interpolate.

    Depth 0 always; depth 1 when the bracket that opened it belongs to a
    prose builder. A literal handed to anything else — `.expect(…)`,
    `push_str(…)`, a comparison — is not this function's answer.
    """
    strings = [s for s in spans if s[2] == "string" and s[0] >= start and s[1] <= end]
    ...
    while i < end:
        ...
        if c in "([{":
            ...
            stack.append("fmt" if FMT_CALLEE.search(before) else
                         ("prose" if PROSE_CALLEE.search(before) else None))
        elif c in ")]}":
            if stack:
                stack.pop()
        i += 1
...
def placeholder(lit):
    ...
    `{name}` and `{shown:?}` are all this function went and got a value;
    `{{` is a printed brace and measures nothing."""
    i = 0
    while i < len(lit):
        if lit[i] == "{":
            if lit[i + 1:i + 2] == "{":
...
def rust_answers(code, spans, text, body_start, body_end):
    """Each value position that carries literals, with what it prints."""
    out = []
...
def report(path, text, line, name, cl, prose, cannot_see):
    ...
    for idx, (_start, found) in enumerate(cl):
        ...
        bad.append(
            f"{path}:{text[:found[0][0]].count(chr(10)) + 1}: {name} answer "
            f"{idx + 1} of {len(cl)} "
...
def audit_rust(path, text):
    ...
    for m in RUST_FN.finditer(code):
        ...
        # `-> String` inside a closure parameter cannot claim to be one.
        # This is the transport rule from the header: no String, no str,
        # no Cow, no sentence to believe.
        b, c = report(
            path, text, line(m.start()), name,
            rust_answers(code, spans, text, open_at + 1, end),
            bool(re.search(r"\bString\b|\bstr\b|\bCow\b", ret)),
            f"{path}:{line(m.start())}: {name} returns prose this gate cannot "
            "see: no `return`, no match arm and no tail expression in it "
            "carries a string literal, so its answers are built somewhere "
            "else and the sole-branch rule cannot be checked. Write each "
            "answer as `return format!(…)`, or as the value of a match arm "
            "(a format!/panic!/concat!/String::from, or a literal with "
            "`.to_owned()` on it), or teach tools/check-diagnostics.sh to "
            "follow this shape.")
        bad += b
        census.append(c)
    return bad, census
...
if cmd == "audit":
    roots, override = [], {}
    for a in rest:
        if "=" in a and a.split("=", 1)[0].endswith(READ_EXT):
            k, v = a.split("=", 1)
    ...
    for path in walk(roots):
        ...
        for m in OTHER_FUNC.finditer(text):
            bad.append(
                ...
                "and Rust only, so nothing is checking it. Add this language to "
...
if cmd in ("rust-sole", "rust-prose"):
    ...
    with open(target, encoding="utf-8") as fh:
        text = fh.read()
    spans, err = rust_spans_of(text, target)
    if err:
        sys.exit("check-diagnostics: self-test: " + err)
    code = masked(text, spans)
    here = find_rust_fn(code, name)
    if not here:
        sys.exit(f"check-diagnostics: self-test: no {name} in {target} — the "
                 "function the Rust arm was built for is gone; re-point the "
                 "self-test at whatever replaced it, do not delete it")
    if cmd == "rust-sole":
        # SOLE-ANSWER: one sentence for every cause.
        body = ("fn " + name + "(name: &str) -> String {\n"
                "    let _ = name;\n"
        ...
        if len(cl) < 2:
            sys.exit(f"check-diagnostics: self-test: {name} has {len(cl)} "
                     "answers, so there is no SECOND one to perturb — either "
                     "the function collapsed (which the tree audit below will "
                     "say in its own words) or this gate stopped reading it")
        pieces = cl[1][1]
        fixed = '"that asset name cannot be used"'
        new = text[:pieces[0][0]] + fixed + text[pieces[-1][1]:]
        note = (f"answer 2 of {len(cl)} rewritten to interpolate nothing: "
                f"{pieces[-1][1] - pieces[0][0]} bytes out, {len(fixed)} bytes in")
    if new == text:
        sys.exit("check-diagnostics: self-test: the perturbation changed "
                 "NOTHING — 0 substitutions is a failed self-test")
    with open(out, "w", encoding="utf-8") as fh:
...
if run_py audit "$RUST_TARGET" "$RUST_TARGET=$TMP/sole.rs" \
        >"$TMP/st3.out" 2>"$TMP/st3.err"; then
    echo "check-diagnostics: SELF-TEST FAILED — an asset_why_not reduced to one" \
        "single answer PASSED this gate. SOLE-ANSWER is not biting in Rust, so" \
        "the core's diagnostic — the one sentence nine bindings raise — is held" \
        "to nothing." >&2
    exit 1
fi
if ! grep -q "asset_why_not has exactly ONE answer" "$TMP/st3.err"; then
    echo "check-diagnostics: SELF-TEST FAILED — the one-answer rust copy was" \
        "rejected, but not for SOLE-ANSWER. A negative test that fails for the" \
        "wrong reason proves nothing. What it said:" >&2
    cat "$TMP/st3.err" >&2
    exit 1
fi

if ! run_py rust-prose "$RUST_TARGET" "$TMP/prose.rs"; then
    echo "check-diagnostics: self-test: could not build the fixed-prose" \
        "rust copy" >&2
    exit 1
fi
if run_py audit "$RUST_TARGET" "$RUST_TARGET=$TMP/prose.rs" \
        >"$TMP/st4.out" 2>"$TMP/st4.err"; then
    echo "check-diagnostics: SELF-TEST FAILED — an asset_why_not whose SECOND" \
        "answer interpolates nothing PASSED this gate. That is the shape the" \
        "whole file exists for: a sentence printed for every state that reaches" \
        "it, believed by the next reader." >&2
    exit 1
fi
if ! grep -q "asset_why_not answer 2 of " "$TMP/st4.err"; then
    echo "check-diagnostics: SELF-TEST FAILED — the fixed-prose rust copy was" \
        "rejected, but NOT for being fixed prose in its second answer. What it" \
        "said:" >&2
    cat "$TMP/st4.err" >&2
    exit 1
fi
...
echo "check-diagnostics: self-test: the pre-fix body ($PREFIX_REV) fails" \
    "FIXED-PROSE and a one-answer body fails SOLE-ANSWER, in Swift; a" \
    "one-answer $RUST_TARGET fails SOLE-ANSWER and a fixed second answer" \
    "fails FIXED-PROSE, in Rust"
