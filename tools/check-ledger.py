...
def lexical(text):
    ...
    lines = text.split("\n")
    bullets = sum(1 for ln in lines if re.match(r"^- ", ln))
    heads = [ln for ln in lines if HEADING.match(ln)]
    return bullets, sum(1 for ln in heads if not organizing(ln))
...
def census(es, secs, text):
    ...
    missing = [name for name in ORGANIZING
               if not any(organizing(ln) and HEADING.match(ln).group(2)
                          .startswith(name) for ln in text.split("\n")
                          if HEADING.match(ln))]
    if missing:
...
headless = "\n".join(ln for ln in text.split("\n")
                     if not (HEADING.match(ln) and not organizing(ln)))
...
sec_heads = [n for n, ln in enumerate(text.split("\n"), 1)
             if HEADING.match(ln) and not organizing(ln)]
if not sec_heads:
