...
floor's documentation. This sweep began (2026-08-10) as one rule — a
sugar guest does not name a widget kind — because that was the spelling
whose sugar had just been completed. The floor census that followed
(docs/tpl-props-plan.md §2) swept check-sugar-surface's whole
per-language floor vocabulary, 44 patterns, over every guest, and found
the rest of the story: 28 patterns with zero hits anywhere (absolute,
sweepable with no exemptions), a handful needing a SHARPER pattern
before they could gate without failing legitimate code, and one family
(the SetText verb/floor collision) that no pattern can split because the
receiver's TYPE decides — retired by renaming instead (F3).
...
- kaya's For combinator collides with every language's own iteration
  ...
  Zero collisions in the corpus today, but a gate that would fail
  `list.forEach(System.out::println)` the day someone writes it is the
  failure mode a gate must never have. kaya's For takes the COLLECTION
  first and the body second; every stdlib form takes the body alone. So
  the discriminator is >= 2 top-level arguments, which needs a
  paren-balanced scan — a comma regex breaks on nested calls.
- Swift's stdlib forEach is trailing-closure (`xs.forEach { .. }` or
  `.forEach({ .. })`); kaya's takes the collection in the parens
  (`tx.forEach(groups) { .. }`). One character decides: `\\.forEach\\([^{]`.
- Generated files are exempt BY MARKER, not by filename glob: all four
  generators emit `Code generated by ...; DO NOT EDIT.` within the first
  lines, and a glob would be a second list of the same fact
  (check-gates has been burned by two lists of one thing before).
- OCaml's element-bind family must not match the sugar that replaced
  ...
  (`label ~bind_field:element ()`), and the `:` after the label is what
  separates them — the pattern requires whitespace or `(` after the
  name. Haskell gets the twin pre-emptively (its family exists in the
  binding; zero guest hits today).

COMMENTS ARE STRIPPED FIRST: the converted guests all explain their old
floor spelling in a comment above the new sugar call, so a sweep that
reads comments reports every file it just fixed (measured — a first
pass counted 14 widget-kind hits where there were 13).

EVERY RULE IS WATCHED, EVERY RUN. Each carries a `fire` line (must
match) and a `quiet` line (must not); the self-test runs them through
the same engine as the real sweep before any verdict is printed. A rule
whose fire line stops firing, or whose quiet line starts, fails the
sweep as SELFTEST-BROKEN — a guard nobody has seen fail is worse than
none.
...
RULES = {
    ...
    ".hs": [
        ...
        # the verb kept setText, the floor became setTextProp, and the
        # floor name is sweepable precisely because the rename made it
        # unambiguous.
        ("the setTextProp prop write", "line",
         r"(?<![A-Za-z])setTextProp[\s(]",
         "setTextProp n \"hi\"",
         "setText editor doc"),
        # A For whose result is dropped is what `each` is — promoted
        # from the milestone2-scoped scene rule the day the template
        # zone gained `each` (it had nowhere to point before).
    ...
    ".ml": [
        ...
        # The whole renamed template floor at once (docs/tpl-props-plan.md
        # F3): OCaml moved its template prop writes into Tpl.Floor, so
        # ONE module-path pattern covers every floor function, including
        # ones added later, by construction.
        ("the template floor module", "line",
         r"(?<![A-Za-z_.])Floor\.",
         "Tpl.(Floor.set_text row \"hi\")",
         "let row = label ~bind_field:element () in"),
        # A For whose result is dropped is what `each` is — promoted
        # repo-wide the day Tpl gained `each`.
        ("a For whose result it drops", "line",
         r"^[\s]*let [a-z_]+, \(\) =",
         "       let todo_list, () =",
         "       let todo_list, items ="),
...
# exemption is a claim on the record, not an absence; the anti-vacuity
# check below fires only when this is non-empty, and an entry whose path
# stops matching fails rather than sitting unread. Currently empty: the
# scalar-element slice (a6c23be) removed the last three.
EXEMPT = {}
...
def selftest():
    ...
    the same engine as the real sweep, so a rotted pattern cannot pass."""
    broken = []
...
def main():
    ...
    # ANTI-VACUITY, both halves (see the widget-kind-only version's
    # history for why): a sweep that reads no files agrees with
