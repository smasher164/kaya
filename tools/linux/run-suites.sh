...
DEPTH_SCENES="windowed canvas sizepolicy dnd tooltips tasks notify richtext ownundo richlabel notes richrows format flexshrink listrow tints badge emoji"
BUILD_EXAMPLES=()
...
for proto in x11 wayland; do
    ...
    # The platform tints, light and dark (docs/tints-plan.md).
    run "$proto" tints-rust env KAYA_SELFTEST=tints \
        "$CARGO_TARGET_DIR/debug/examples/tints"
    run "$proto" tintsdark-rust env KAYA_APPEARANCE=dark KAYA_SELFTEST=tints \
        "$CARGO_TARGET_DIR/debug/examples/tints"
    ...
    run "$proto" search-java env KAYA_SELFTEST=search KAYA_LIB="$LIB" \
        tools/linux/a11y-leg.sh java -cp /tmp/java-guests dev.kaya.guests.Main
    # THE SUBMIT GESTURE (docs/submit-plan.md §5): Return on the three text kinds.
    run "$proto" submit-rust env KAYA_SELFTEST=submit "$CARGO_TARGET_DIR/debug/examples/submit"
    run "$proto" submit-python env KAYA_SELFTEST=submit KAYA_LIB="$LIB" \
        python3 guests/python/submit.py
    run "$proto" submit-js env KAYA_SELFTEST=submit KAYA_LIB="$LIB" \
        node guests/js/submit.ts
    run "$proto" submit-go env KAYA_SELFTEST=submit /tmp/go-guests/kaya-go
    run "$proto" submit-csharp env KAYA_SELFTEST=submit KAYA_LIB="$LIB" \
        dotnet exec "$CS_GUEST"
    ...
    # THE RICH LABEL SCENE (docs/rich-text-plan.md R8, §15): the inline
    # vocabulary read-only on a label. Rust alone for richtext's reason,
    # and pooled beside it — the scene drives clicks and reads the core's
