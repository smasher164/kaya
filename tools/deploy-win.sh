#!/usr/bin/env bash
# The menus scene runs every language here. Its legs are the one
...
#
# The <scene>_<lang> family is the parser's real surface, held level
# with the `all` case's launcher calls by the clause below. Not
# enumerated here on purpose: a second hand-written list of the same
# thing is the drift this file already had once.
#
...
# leg, mid-debugging, being told "unknown argument" by the script that
# just ran that very leg.
if ! python3 - "$0" <<'PY'
import pathlib
import re
import sys
...
# THE SELF-TEST FIRST: a checker that cannot see the drift it exists
# for reports OK on every tree.
#
...
# leg it would go looking for a launcher for.
call = "run_suite"
...
if ! ssh -n -o BatchMode=yes -o ConnectTimeout=5 "$HOST" 'exit 0' 2>/dev/null; then
    ...
    if "$(utmctl_bin)" status "$VM_NAME" 2>/dev/null | grep -qi started; then
        ...
        # guest that BUGCHECKED and is rebooting, not only of one that hung.
        # Killing it there costs twice — the crash dump is written to the
        # pagefile and only converts to C:\Windows\Minidump on the NEXT boot
        # (7 of 12 crashes left no dump that way), and a power cut mid-boot
        # is what corrupted this VM's boot volume on 2026-08-04. A reboot
    ...
    # docs/traps.md for two weeks while the guest was really BUGCHECKING —
    # twelve times, all viogpudo.sys+0xB52C. A dump written to the pagefile
    # only becomes a file on the boot AFTER the crash, so this is the first
...
# THE scene list: every mechanical per-scene surface below derives from
# it (cross-build examples, exe/python/go shipping, taskkill). Adding a
# scene here is the ONE registration; the suite blocks stay explicit
...
# SCENES (whose per-language surfaces glob for a11y.py, a11y.go, ...
# and fail loudly, correctly) or go unexercised on this lane entirely,
# which is how the WinUI accessibility read ended up committed
# unproven.
#
# EMPTY TODAY, and that is the list working. The variable stays because
...
# list above: SCENES' per-language surfaces glob for editor.py /
# editor.cs / editor.java and would fail loudly and correctly, and
# DEPTH_SCENES builds and ships a RUST example.
#
# What it must still join is the TASKKILL SWEEP, the one mechanical
# surface that does not care what language a guest is in: a leg that
# aborts leaves editor_go.exe holding kaya.dll, and the next deploy's
...
#
# Windows publishes no document-modified affordance at any layer, so on
# this platform the `dirty` prop lowers to TEXT — a leading `*`
# composed into the rendered caption. The caption is already a
# composition of the window's own title and a covering nav entry's, and
# a text marker becomes a third input that EVERY caption write has to
# apply or the mark blinks out on a push, a pop, or a split-mode
# change. There were five such writes.
#
...
# the type system cannot help — `Window::SetTitle` is a WinRT
# projection method and is always callable — and it lives HERE, in the
# deploy every Windows change goes through, rather than in a gate list
...
# <KAYA_SCENES_DIR>/<name>.steps, so the .steps files must reach the
# VM. EVERY run, not the --provision block: legs depend on these, and a
# provisioning-only ship means a scene edit never arrives.
...
# THE ASSET ROOT, AS A UNIT. `asset(name)` resolves every asset through
# ONE root, which the core finds by KAYA_ASSET_DIR (docs/assets-plan.md
# A2, A5.2), so `scp -r` of the root is one paragraph for every asset
# there will ever be and a file that fails to arrive fails the hash
# below rather than one scene on one lane.
#
...
#
# INTO THE REPO-MIRROR PATH — C:\kaya is already the repo root as far
...
# compared BYTE-FOR-BYTE across every platform (CLAUDE.md, invariant
# 6). Set every deploy rather than once at --provision: it is a
# per-user registry value any Explorer settings change can put back.
# Verified after setting, because a write that did not take is the kind
# of thing that reads as success.
...
# A toast is a foreground window owned by the shell, and while one is
# up SetForegroundWindow FAILS for everything else — so every leg that
...
# 2026-07-31: a "Turn off notifications from OneDrive?" SUGGESTION
# toast took out all ten of them, twice in a row, while the other 121
# legs passed.
#
# Three values, because the shell has three places to say it. Set every
# deploy, like HideFileExt above: any settings visit can put them back.
...
# THE FOREGROUND LOCK MUST BE OFF, the third setting in a row that
# exists because the desktop can quietly refuse a window the foreground.
#
...
# default) before letting anyone else in. So with the default value the
# ONLY reason the shortcut-injection legs pass is that some earlier leg
# happened to deliver input to the session — which a long-running VM
# accumulates incidentally, and which is why this was invisible for
# months. A REBOOT REMOVES IT: measured 2026-07-31, after restarting
# the VM every menus and commands leg failed, reproducibly. Injecting
# from a helper does NOT fix it, because the foreground right goes to
# the process that received the input.
#
...
# below, from INSIDE that session — a call from here cannot work,
# because an ssh session is session 0 with its own window station and
# SPI applies per window station (measured 2026-08-04).
...
# WIPED AND RECREATED, NOT COPIED OVER. scp writes the files it is
# given and DELETES NOTHING, so a source file that leaves the repo
# lives on in C:\kaya forever — and Go compiles a directory, not a file
# list, so one leftover is a compile error in a package that is correct
# in the tree. MEASURED 2026-08-07: after the scene-library split moved
# todo_kaya.go out of guests/go/todos, the VM's stale copy made the
# build fail with `guests\go\todos\todo_kaya.go: undefined: Todo`.
#
# ONE RECURSIVE COPY OF THE WHOLE GO TREE, not a loop over SCENES: the
# guests are ONE PROGRAM, so a leg needs every scene present, not the
# one it names.
...
# every real deploy, and a per-file diff would reintroduce the
# stale-mix class those recreations kill. The kaya.dll remote-hash
...
PYEOF
    ...
    if [ "$ship_count" -gt 0 ]; then
        ...
        # an earlier run is still alive and holding it. set -e already
        # stops here, which is the important part — the alternative is a
        # lane that runs every leg against the PREVIOUS dll.
    ...
    # javac takes explicit files) and compiled IN PLACE — the C#
    # precedent, where the suite builds what it verifies. Recreated from
    # scratch every deploy. Quote-free on purpose: Windows sshd re-wraps
    # the command in its own cmd /c "...", and interior double quotes
    # re-pair across the line (docs/traps.md).
...
# suites would then run against the previous deploy. One Get-FileHash
# round trip for the whole set (each individual one cost ~1.4s). Order
# is the contract: the remote list and the local list are compared
# line-by-line.
verify_deployed() {
    local locals=(
        "$TARGET/kaya.dll"
...
# `#[cfg(all(test, unix))]`.
#
# WHAT IT REACHES THAT NO OTHER LANE CAN: on Windows the redeemed
# handle is a Win32 HANDLE and not a descriptor, so protocol.rs's
# `raw_handle` and `file_from_raw` take their OTHER arm — code no unix
# run compiles, let alone executes. Proven by construction on
# 2026-08-09: breaking the windows arm of `raw_handle` alone turned
# this phase red while `cargo test` on mac stayed green.
#
# THE COUNT COMES OUT OF THE SOURCE. A filter that matches nothing
# exits 0 with "0 passed", so the number of `#[test]`s in the module is
# read from capi.rs and the run must report exactly that many passed.
#
# SCOPED TO capi::picked_tests, measured rather than timid: the whole
# lib suite on this guest is 309 passed / 3 failed today, and all three
# failures are POSIX assumptions in HARNESS tests rather than anything
# about Windows in the core. Widening this filter is one edit the day
# those three are fixed, and it is the widening to make.
guest_unit_module() {
...
PY
    ...
    # the --no-run above and then filtered out of the only invocation that
    # runs anything, so four of them had never executed. They need this
    # machine because they measure this machine: DirectWrite's system font
    # collection, and a font file written under the app root and read back
    # through its own name table.
...
#
# WHY IT IS A LANE PHASE: a promoted window's caption title is aimed at
# the WINDOW's centre, clamped when a header would collide, and NO
# SCENE CAN SEE THAT — every harness read of a title goes through the
# string, which is the same whether the TextBlock sits on the window's
# centre line or 63 DIP left of it. Only an outside observer with UIA
# and the window's visible frame can say by how many physical pixels,
# at widths a scene never drives.
#
...
# none:
#   - the probe declares its plan (AIMPLAN n) before it runs any of it,
#     and exactly n AIMV rows must come back;
#   - at least CAPTION_CENTRE_MIN_CLEAR of those rows must be UNCLAMPED,
#     so a run in which every row went clamped cannot satisfy the drift
#     rule vacuously;
#   - every unclamped row's drift must be 0;
#   - and no row may report the title OVERLAPPING a header.
...
# and is window-scoped. Anchoring never crosses machines: frame names
# and harness epochs share the VM clock.
rec_suite_start() {
...
# before any leg, because ten legs per lane bet on the answer.
#
# Every chord-injecting leg raises the guest window and confirms it
# before pressing anything. When the desktop refuses, each dies 3s
# later with "could not foreground the guest window for shortcut
# injection", which reads as a WinUI bug and is not one; the rest of
# the lane passes. That happened on 2026-08-04 after a VM restart,
# cost a lane, and was cleared by a reboot — a remedy that lived in a
# human's head.
#
...
case "$SUITE" in
    all)
        ...
        # The confirm scene: the modal-alert grammar (ContentDialog), all
        # three answer paths through the REAL press.
        # The stall diagnostic (crates/kaya/src/stall.rs): the one scene that
        # deliberately blocks the app thread, in EVERY LANGUAGE THIS LANE
        # CARRIES, because the misuse it guards is available in all of them.
        ...
        run_suite nav_java
        # (The depth scenes' legs are not here: they run drained, in their
        # own block below.)
        ...
        # ProgressBar. It is the first control whose template REQUIRES the
        # XamlControlsResources merge, and ms-appx resolves against the
        # PROCESS exe's directory — so every leg arranges kaya's minimal
        # resources.pri beside its host exe (docs/traps.md, the pri-adjacency
        # rule).
        ...
        run_suite a11y_java
        # The select scene: ComboBox — the same XamlControlsResources
        # dependency as ProgressBar, so the legs reuse the pri-adjacency
        # arrangement above.
        ...
        # The radio scene: RadioButtons — the same XamlControlsResources
        # dependency class as ProgressBar, same pri-adjacency arrangement.
        ...
        # launchers (tools/guest/run_<scene>_rust.cmd, shipped by the deploy
        # glob). Run BEFORE the menu block's foreground-sensitive legs, and
        # drained around, since an accessibility read foregrounds nothing but
        # a scene may.
        #
        # NAMED ONE BY ONE rather than looped over $DEPTH_SCENES: check-steps
        # requires a LITERAL `run_suite <scene>_` here (a loop over a variable
        # is invisible to it) and then requires that leg's launcher to be
        # checked in — so a loop over the same list ran each depth scene a
        # SECOND time, for as long as it existed.
        #
        # The dirty scene (docs/dirty-plan.md), rust-only until the sugar
        # sweep. On this platform the chrome is TEXT — Windows has no
        # modified affordance at any layer — so `expect_dirty` reads the real
        # OS caption for a leading `*`, which is also why the scene asserts
        # its title only while clean. Drained around, not for the foreground
        # reason: the leg drives a real WM_CLOSE on its own window and the
        # veto keeps it, but a window disappearing out from under a pooled
        # leg reads as somebody else's bug.
        drain_suites
        ...
        # THE STYLING SCENE (docs/styling-plan.md slice 1). On this backend
        # the brand is the six SystemAccentColor* stops in the Light/Dark
        # ThemeDictionaries (never SystemAccentColor itself, the documented
        # no-op), prominent is AccentButtonStyle, destructive the critical
        # brush on the caption, and `heading` is
        # AutomationProperties.HeadingLevel — the UIA read expect_ax freezes.
        # Pooled: no typed input, no window close, nothing
        ...
        # sizes, weights and metrics stay the platform's. It exists because a
        # typeface fails SILENTLY — DirectWrite renders a family it does not
        # have in something else — so only `expect_typeface`, which reads the
        # family the text system actually resolved off the real TextBlocks,
        # tells a typo, a stale lowering and a working swap apart. Pooled
        # with the styling family. The vendored font reaches these guests
        # through the ship near the top of this script and the KAYA_FONT_FILE
        # line in each launcher.
        run_suite typeface_rust
        run_suite typeface_python
        run_suite typeface_go
        run_suite typeface_csharp
        run_suite typeface_java
        # THE TOOLBAR SCENE (docs/chrome-plan.md C2). On this backend that is
        # a CommandBar in the shell Grid's second Auto row — one AppBarButton
        # per `primary` catalog action, in catalog preorder — and the scene
        # reads the REAL bar: the promoted names it publishes to UIA, the
        # icon in each button's slot, and IsEnabled on the button object. The
        # unpromoted catalog stays in the MenuBar one row above, which is why
        # nothing fills SecondaryCommands.
        #
        # Pooled with the styling/typeface family: no typed input, no window
        # close, nothing foreground-sensitive.
        ...
        # The assets conformance scene (docs/assets-plan.md). THIS LANE IS
        # THE ONE WHERE THE STAGED ROOT IS THE ONLY ROUTE: the VM has no repo
        # of its own, so the deploy copies guests/assets into the repo mirror
        # every run and names it machine-wide in KAYA_ASSET_DIR, and the
        # scene's frozen census proves it copied the WHOLE root. Deliberately
        # outside the deploy stamp, so an asset edit cannot be swallowed by a
        # stamp skip. Pooled: no typed input, no window close, nothing
        # foreground-sensitive.
        ...
        drain_suites
        # The ranges scene (docs/ranges-plan.md D1-D4). Rust-only until the
        # sugar sweep gives the other eight guests a spelling.
        #
        # DRAINED AROUND, for the `type`/`compose` reason rather than the
        # window one: `type` injects OS-GLOBAL keystrokes, and `compose`
        # starts a TSF composition in whatever document holds the KEYBOARD
        # FOCUS — so a pooled leg stealing the foreground mid-scene would put
        # a composition in someone else's control.
        run_suite ranges_rust
        drain_suites
        # The background scene: work off the app thread, posted back. Its
        # worker parks until a click releases it, so a binding that ran the
        ...
        # gate (docs/background-work-plan.md §5). Read a timeout here as
        # that, not as VM load.
        # The filedialog scene: the Shell's own dialog, driven for real over
        # UI Automation. SERIAL, BETWEEN DRAINS, like the menus legs and for
        # a sibling reason — a file dialog is OS-GLOBAL chrome: modal, it
        # must hold the FOREGROUND to be driven, and the harness finds it by
        # searching the desktop, so two at once means one is in the
        # background with its presses swallowed and BOTH legs fail. Measured
        # 2026-07-31: the rust leg had been green for weeks and started
        # failing the moment a python leg joined it in the four-wide pool.
        drain_suites
        ...
        drain_suites
        # The save scene (docs/save-plan.md D5). Rust-only until the sugar
        # sweep gives the other guests a `save_file` spelling.
        #
        # SERIAL, BETWEEN DRAINS, and it is the filedialog rule rather than a
        # second one: the Shell's save dialog is the same OS-GLOBAL modal
        # chrome, found the same way — `live_dialog` walks the DESKTOP for a
        # visible `#32770` and takes the first — so with a picker up beside
        # it both legs fail, which reads as a backend bug.
        run_suite save_rust
        drain_suites
        # THE TEXT EDITOR (docs/editor-plan.md), the only script on this lane
        # that drives an APP rather than a feature.
        #
        # GO ALONE, and no rust leg is coming: the plan chose Go so that a
        # BINDING's awkward corners would show. `editor` is therefore in
        # neither SCENES nor DEPTH_SCENES — both derive a cross-built rust
        # example this app does not have — and the leg runs from its own
        # checked-in launcher.
        #
        # SERIAL, BETWEEN DRAINS, for BOTH reasons this file gives: the
        # Shell's open and save dialogs are OS-GLOBAL modal chrome found by
        # walking the DESKTOP for a visible `#32770`, and `type` injects
        # OS-global keystrokes that land wherever the foreground is.
        run_suite editor_go
        drain_suites
        ...
        drain_suites
        # The split scene through TwoPaneView, with resize_window driving the
        # real size-class transition. Every language this lane runs.
        #
        # TwoPaneView is a pri-adjacency control: its template needs the
        # XamlControlsResources merge and ms-appx resolves against the
        # PROCESS exe's directory, so the go and csharp launchers take the
        # progress shape. Measured 2026-07-27: with the plain launchers both
        # crashed at the first expect_split with 0xc000027b, a stowed XAML
        # exception, while rust/python/java passed.
        ...
        drain_suites
        # The undo scene: two tiers behind one Edit>Undo, and the ledger
        # that orders them (docs/undo-plan.md). A DEPTH SCENE, and it
        # belongs in THIS serial block rather than the depth block above for
        # the menus reason exactly: its `type` verb puts REAL KEYSTROKES on
        # the system input queue and foregrounds the guest to do it, so a
        # concurrent leg's SetForegroundWindow would take the typing.
        #
        # NOT PINNED BY check-steps' menu_serial CLAUSE, which matches
        # `run_suite (menus|filedialog)_` by name: that pattern must grow an
        # `undo` arm, or a future refactor can re-pool this leg with nothing
        # to say so.
        run_suite undo_rust
        drain_suites
        # The menus scene — every language, each leg ALONE between drains,
        # never in the pool above. WinUI shortcut injection is OS-global
        # (docs/traps.md): the harness foregrounds the guest and puts the
        # real chord on the system input queue, so a concurrent leg's
        # SetForegroundWindow would steal it. check-steps pins the
        # drain/run/drain barrier on each one.
        ...
        drain_suites
        # The clipboard scene. THE LEGS ARE MUTUALLY EXCLUSIVE, ONE DRAIN
        # EACH (docs/clipboard-plan.md §0d): there is one system clipboard
        # per session, and legs writing it concurrently are processes
        # assigning one variable — measured on mac, six of eight failed
        # concurrently and the same eight passed serially. NOT the menus
        # reason: menu_activate here drives the real invoke pipeline and no
        # chord is injected. check-steps pins the drain/run/drain shape.
