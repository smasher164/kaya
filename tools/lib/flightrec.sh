...
# -------------------------------------------------------------- Windows --
#
# The guest half is tools/guest/flightrec.ps1 and it must run in the
# INTERACTIVE session (schtasks /it) — an ssh session is session 0 and can
# see no desktop. These functions need `run_ssh` and the `scp` wrapper from
# tools/deploy-win.sh; they are no-ops anywhere those are not defined.
#
# TASK NAMES: the COLLECT is per leg, because two legs can fail at once and
# the existing run_guest_oneshot hardcodes one name. The SAMPLER is not —
# see flightrec_win_lane_sampler_start.
#
# THE PASS PATH TOUCHES THE VM NOT AT ALL. It used to cost three ssh round
# trips a leg and that is what took this lane 110s over its duration
# ceiling on the recorder's first matrix; the numbers are in
# docs/deferred.md's flight-recorder entry.

flightrec_win_ready() {
    [ "${FLIGHTREC_OK:-0}" = 1 ] || return 1
    declare -F run_ssh >/dev/null 2>&1 || return 1
    return 0
}

# Clear the previous lane's backstop, start the ONE sampler, and learn the
# guest's clock. Three round trips per LANE, where there were three per LEG.
flightrec_win_lane_start() {
    flightrec_win_ready || return 0
    run_ssh "del C:\\kaya\\flightrec\\ALL.stop C:\\kaya\\flightrec\\lane-foreground.txt 2>nul & exit /b 0" >/dev/null 2>&1 || true
    flightrec_win_clock_sync
    flightrec_win_lane_sampler_start
    return 0
}

# ONE SAMPLER FOR THE WHOLE LANE.
#
# The foreground is a MACHINE-WIDE property: one poller answers for every
# leg at once, so there is nothing per-leg to start or stop. It writes a
# capped ring with guest-epoch timestamps and the failure path reads the
# leg's window out of it, which is what flightrec_win_clock_sync is for.
flightrec_win_lane_sampler_start() {
    flightrec_win_ready || return 0
    run_ssh "schtasks /create /tn kayafr_lane /tr \"wscript C:\\kaya\\run-hidden-args.vbs flightrec.cmd sample lane\" /sc once /st 00:00 /it /rl highest /f >nul && schtasks /run /tn kayafr_lane >nul" >/dev/null 2>&1 || true
    return 0
}

# The host-to-guest clock offset, read ONCE per lane. Without it the ring
# is a wall of guest timestamps no leg can be attributed to.
flightrec_win_clock_sync() {
    flightrec_win_ready || return 0
    local guest
    guest="$(run_ssh 'powershell -NoProfile -Command "[int64](Get-Date -UFormat %s)"' 2>/dev/null | tr -d '\r\n ')"
    case "$guest" in
        '' | *[!0-9]*) FLIGHTREC_WIN_SKEW=0 ;;
        *) FLIGHTREC_WIN_SKEW=$((guest - ${EPOCHSECONDS:-0})) ;;
    esac
    export FLIGHTREC_WIN_SKEW
    echo "flightrec: the guest's clock is ${FLIGHTREC_WIN_SKEW}s from this host's"
    return 0
}

# The lane's backstop, called from the runner's EXIT trap. Each leg's own
# stop and task-delete already ran if the leg ended normally; this is for
# the legs that did not, so no sampler outlives the lane and quietly loads
# the machine the next lane is timed on.
flightrec_win_cleanup() {
    flightrec_win_ready || return 0
    run_ssh "if not exist C:\\kaya\\flightrec mkdir C:\\kaya\\flightrec >nul 2>nul & echo stop > C:\\kaya\\flightrec\\ALL.stop & exit /b 0" >/dev/null 2>&1 || true
    return 0
}

# flightrec_win_collect <leg> — run the guest-side collection NOW. Called
# twice at most: once from the timeout path while the guest is STILL ALIVE
# (the only moment a window, a dialog or a stack exists to photograph), and
# once at leg end if that never happened.
flightrec_win_collect() {
    flightrec_win_ready || return 0
    local leg="$1" tries=0
    run_ssh "schtasks /create /tn kayafrc_$leg /tr \"wscript C:\\kaya\\run-hidden-args.vbs flightrec.cmd collect $leg\" /sc once /st 00:00 /it /rl highest /f >nul && schtasks /run /tn kayafrc_$leg >nul" >/dev/null 2>&1 || true
    while [ "$tries" -lt 30 ]; do
        if run_ssh "type C:\\kaya\\flightrec\\$leg-collect.txt" 2>/dev/null | grep -q COLLECTDONE; then
            break
        fi
        tries=$((tries + 1))
        sleep 2
    done
    run_ssh "schtasks /delete /tn kayafrc_$leg /f >nul 2>nul & exit /b 0" >/dev/null 2>&1 || true
    return 0
}

flightrec_win_pull() { # <bundle> <leg> <remote-suffix> <section> <dest-name>
    local bundle="$1" leg="$2" suffix="$3" section="$4"
    local dest="$bundle/$5"
    scp "$HOST:C:/kaya/flightrec/$leg-$suffix" "$dest" >/dev/null 2>&1 || true
    if [ ! -s "$dest" ]; then
        rm -f "$dest"
        printf 'flightrec: the guest wrote no %s for this leg (C:\\kaya\\flightrec\\%s-%s)\n' \
            "$section" "$leg" "$suffix" >"$bundle/$section.skip"
        flightrec_mark "$bundle" "$section" skip 0
        return 0
    fi
    local bytes
    bytes="$(wc -c <"$dest" 2>/dev/null | tr -d ' ')"
    [ -n "$bytes" ] || bytes=0
    flightrec_mark "$bundle" "$section" ok "$bytes"
    return 0
}

# flightrec_win_leg <leg> <verdict> <secs> <log> <collected-already> <start-epoch>
#
# A PASS RETURNS AFTER ONE `printf`. Everything below the verdict test is
# the failure path: no bundle is scaffolded, no ssh is spoken and no
# process is spawned when a leg passes.
flightrec_win_leg() {
    [ "${FLIGHTREC_OK:-0}" = 1 ] || return 0
    local leg="$1" verdict="$2" secs="$3" log="$4" already="${5:-0}" t0="${6:-0}"
    local bundle="" fail=""
    if [ "$verdict" != PASS ]; then
        bundle="$(flightrec_bundle windows "$leg")"
        fail="$(flightrec_fail_sentence "$log")"
        if [ -n "$bundle" ]; then
            # The guest has already exited on the ordinary failure path, so
            # this second collect answers only the retrospective questions
            # (event log, process list) and honestly skips the rest. The
            # live one, if it happened, already wrote the file this
            # overwrites — so it is skipped when it did.
            [ "$already" = 1 ] || flightrec_win_collect "$leg"
            flightrec_adopt "$bundle" "leg-log" "$log"
            flightrec_win_foreground "$bundle" "$t0"
            flightrec_win_pull "$bundle" "$leg" "collect.txt" "desktop" "desktop.txt"
            flightrec_win_pull "$bundle" "$leg" "shot.png" "shot" "shot.png"
            flightrec_bundle_report "$bundle"
        fi
    fi
    flightrec_leg windows "$leg" "$verdict" "$secs" "$fail" "$bundle"
    return 0
}

# The lane-wide foreground ring, with THIS leg's window named at the top of
# it. The ring is machine-wide and shared by every concurrent leg, so a
# reader needs to be told which lines are theirs; the guest-epoch bounds
# come from the host's own clock plus the offset read once at lane start.
flightrec_win_foreground() { # <bundle> <leg-start-host-epoch>
    local bundle="$1" t0="$2"
    local dest="$bundle/foreground.txt"
    scp "$HOST:C:/kaya/flightrec/lane-foreground.txt" "$dest.ring" >/dev/null 2>&1 || true
    if [ ! -s "$dest.ring" ]; then
        rm -f "$dest.ring"
        printf 'flightrec: the lane sampler wrote no ring (C:\\kaya\\flightrec\\lane-foreground.txt) — it did not start, or the desktop never changed hands\n' \
            >"$bundle/foreground.skip"
        flightrec_mark "$bundle" "foreground" skip 0
        return 0
    fi
    local skew="${FLIGHTREC_WIN_SKEW:-0}"
    {
        printf 'flightrec: this leg ran %s..%s in GUEST epoch seconds (host %s..%s, guest clock %ss off).\n' \
            "$((t0 + skew))" "$((${EPOCHSECONDS:-0} + skew))" \
            "$t0" "${EPOCHSECONDS:-0}" "$skew"
        printf 'flightrec: the ring below is LANE-WIDE — every leg that ran concurrently is in it.\n'
        cat "$dest.ring"
    } >"$dest"
    rm -f "$dest.ring"
    local bytes
    bytes="$(wc -c <"$dest" 2>/dev/null | tr -d ' ')"
    [ -n "$bytes" ] || bytes=0
    flightrec_mark "$bundle" "foreground" ok "$bytes"
    return 0
}
