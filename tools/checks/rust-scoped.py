...
with scratch_dir("rust-scoped-") as tmp:
    for name, body, refusal in cases:
        path = tmp / f"{name}.rs"
        path.write_text(g.doctor(name, source, "BODY", body, want=1), encoding="utf-8")
...
print("rust-scoped: 11 compiler cases; eight refusals, three accepted controls")
