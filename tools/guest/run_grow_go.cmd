...
rmdir /s /q C:\kaya\legs\grow_go 2>nul
mkdir C:\kaya\legs\grow_go\state 2>nul
set XDG_STATE_HOME=C:\kaya\legs\grow_go\state
set KAYA_SELFTEST=grow
set KAYA_VERB_TRACE=C:\kaya\flightrec\grow_go-vtrace.txt
...
go build -o C:\kaya\grow_go.exe dev.kaya/guests/go/cmd > C:\kaya\out_grow_go.txt 2>&1
if errorlevel 1 goto done
grow_go.exe >> C:\kaya\out_grow_go.txt 2>&1
:done
echo EXIT=%ERRORLEVEL% >> C:\kaya\out_grow_go.txt
