...
rmdir /s /q C:\kaya\legs\format_java 2>nul
mkdir C:\kaya\legs\format_java\state 2>nul
set XDG_STATE_HOME=C:\kaya\legs\format_java\state
set KAYA_SELFTEST=format
set KAYA_VERB_TRACE=C:\kaya\flightrec\format_java-vtrace.txt
java -cp C:\kaya\java\classes dev.kaya.guests.Main > C:\kaya\out_format_java.txt 2>&1
echo EXIT=%ERRORLEVEL% >> C:\kaya\out_format_java.txt
