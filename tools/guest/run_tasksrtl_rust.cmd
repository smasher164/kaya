@echo off
cd /d C:\kaya
rem The locale knob (docs/compliance-plan.md §2.2).
...
rmdir /s /q C:\kaya\legs\tasksrtl_rust 2>nul
mkdir C:\kaya\legs\tasksrtl_rust\state 2>nul
set XDG_STATE_HOME=C:\kaya\legs\tasksrtl_rust\state
set KAYA_SELFTEST=tasksrtl
set KAYA_VERB_TRACE=C:\kaya\flightrec\tasksrtl_rust-vtrace.txt
tasks.exe > C:\kaya\out_tasksrtl_rust.txt 2>&1
echo EXIT=%ERRORLEVEL% >> C:\kaya\out_tasksrtl_rust.txt
