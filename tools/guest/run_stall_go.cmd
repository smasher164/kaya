...
rmdir /s /q C:\kaya\legs\stall_go 2>nul
mkdir C:\kaya\legs\stall_go\state 2>nul
set XDG_STATE_HOME=C:\kaya\legs\stall_go\state
set KAYA_SELFTEST=stall
set KAYA_VERB_TRACE=C:\kaya\flightrec\stall_go-vtrace.txt
...
go build -o C:\kaya\stall_go.exe dev.kaya/guests/go/cmd > C:\kaya\out_stall_go.txt 2>&1
if errorlevel 1 goto done
stall_go.exe >> C:\kaya\out_stall_go.txt 2>&1
:done
echo EXIT=%ERRORLEVEL% >> C:\kaya\out_stall_go.txt
