...
rmdir /s /q C:\kaya\legs\formatar_go 2>nul
mkdir C:\kaya\legs\formatar_go\state 2>nul
set XDG_STATE_HOME=C:\kaya\legs\formatar_go\state
set KAYA_LOCALE=ar-EG
set KAYA_SELFTEST=formatar
set KAYA_VERB_TRACE=C:\kaya\flightrec\formatar_go-vtrace.txt
...
go build -o C:\kaya\legs\formatar_go\format_go.exe dev.kaya/guests/go/cmd > C:\kaya\out_formatar_go.txt 2>&1
if errorlevel 1 goto done
C:\kaya\legs\formatar_go\format_go.exe >> C:\kaya\out_formatar_go.txt 2>&1
:done
echo EXIT=%ERRORLEVEL% >> C:\kaya\out_formatar_go.txt
