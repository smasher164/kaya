...
rmdir /s /q C:\kaya\legs\table_csharp 2>nul
mkdir C:\kaya\legs\table_csharp\state 2>nul
set XDG_STATE_HOME=C:\kaya\legs\table_csharp\state
set KAYA_SELFTEST=table
set KAYA_VERB_TRACE=C:\kaya\flightrec\table_csharp-vtrace.txt
...
rem per-leg builds raced the shared obj\ and cs-out, CS2012). The
rem table's header cells are Buttons, so this leg needs the merge.
C:\kaya\cs-out\kaya-guests.exe > C:\kaya\out_table_csharp.txt 2>&1
echo EXIT=%ERRORLEVEL% >> C:\kaya\out_table_csharp.txt
