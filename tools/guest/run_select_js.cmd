...
rmdir /s /q C:\kaya\legs\select_js 2>nul
mkdir C:\kaya\legs\select_js\state 2>nul
set XDG_STATE_HOME=C:\kaya\legs\select_js\state
set KAYA_SELFTEST=select
set KAYA_VERB_TRACE=C:\kaya\flightrec\select_js-vtrace.txt
node C:\kaya\select.ts > C:\kaya\out_select_js.txt 2>&1
echo EXIT=%ERRORLEVEL% >> C:\kaya\out_select_js.txt
