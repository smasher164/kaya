...
rmdir /s /q C:\kaya\legs\formatde_java 2>nul
mkdir C:\kaya\legs\formatde_java\state 2>nul
set XDG_STATE_HOME=C:\kaya\legs\formatde_java\state
set KAYA_LOCALE=de-DE
set KAYA_SELFTEST=formatde
set KAYA_VERB_TRACE=C:\kaya\flightrec\formatde_java-vtrace.txt
java -cp C:\kaya\java\classes dev.kaya.guests.Main > C:\kaya\out_formatde_java.txt 2>&1
echo EXIT=%ERRORLEVEL% >> C:\kaya\out_formatde_java.txt
