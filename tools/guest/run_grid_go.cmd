...
rmdir /s /q C:\kaya\legs\grid_go 2>nul
mkdir C:\kaya\legs\grid_go\state 2>nul
set XDG_STATE_HOME=C:\kaya\legs\grid_go\state
set KAYA_SELFTEST=grid
set KAYA_VERB_TRACE=C:\kaya\flightrec\grid_go-vtrace.txt
...
go build -o C:\kaya\grid_go.exe dev.kaya/guests/go/cmd > C:\kaya\out_grid_go.txt 2>&1
if errorlevel 1 goto done
grid_go.exe >> C:\kaya\out_grid_go.txt 2>&1
:done
echo EXIT=%ERRORLEVEL% >> C:\kaya\out_grid_go.txt
