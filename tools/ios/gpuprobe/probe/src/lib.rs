...
const FONT: &[u8] = include_bytes!("../../../../../guests/assets/fonts/sora-wght.ttf");

/// Where `say!` also writes: the report file the device app hands back,
/// nothing on the mac (stdout is the report there).
...
macro_rules! say {
    ...
    ($($arg:tt)*) => {{
        let line = format!($($arg)*);
        println!("{line}");
        if let Some(f) = REPORT.lock().unwrap_or_else(|e| e.into_inner()).as_mut() {
            use std::io::Write;
...
fn gpu() -> Gpu {
    ...
    // THE ADAPTER'S OWN LIMITS, never wgpu's defaults: the iPhone 17 Pro's
    // Metal adapter allows 15 inter-stage shader variables where the default
    // table asks 16, and the request failed before anything was measured
    // (2026-09-10, the first device run). The refusal goes into the report.
    let (device, queue) = match pollster::block_on(adapter.request_device(&wgpu::DeviceDescriptor { label: Some("probe"), required_features: wgpu::Features::empty(), required_limits: adapter.limits(), ..Default::default() })) {
        Ok(pair) => pair,
        Err(e) => { say!("gpuprobe: the device request failed on {:?}: {e}", info.name); panic!("device"); }
    };
...
pub fn breakdown() {
    let g = gpu();
    say!("BREAKDOWN of one vello_hybrid frame, median of 15 after 3 warm-ups; then a burst of 20 frames with ONE wait at the end");
    say!("{:<26} {:<16} {:>9} {:>9} {:>9} {:>9} {:>11} | {:>14} {:>12}", "drawing", "track", "strips", "encode", "submit", "wait", "frame", "burst/frame", "cpu MT ref");
    ...
    for d in &drawings {
        for (tname, track) in &tracks {
            ...
            say!("{:<26} {:<16} {:>7.2}ms {:>7.2}ms {:>7.2}ms {:>7.2}ms {:>9.2}ms | {:>12.2}ms {:>10.2}ms", d.name, tname, median(ts), median(te), median(tsub), median(tw), median(tf), burst, median(tm));
        }
    }
}
...
pub fn fills() {
    let g = gpu();
    let threads = (std::thread::available_parallelism().map(|n| n.get()).unwrap_or(1).saturating_sub(1)).min(8) as u16;
    say!("FILL-HEAVY FRAMES at phone size 1200x2400 (2.88 Mpx) and desk 1600x1000; median of 15 after 3 warm-ups; burst = 20 frames in flight, one wait");
    say!("{:<16} {:<16} {:>9} {:>9} {:>11} {:>13} {:>12}", "drawing", "track", "cpu 1T", "cpu MT", "gpu frame", "gpu burst/fr", "strips (cpu)");
    for n in [10usize, 50, 200, 800] {
        ...
        for (tname, track) in [("phone 1200x2400", (1200.0, 2400.0)), ("desk 1600x1000", (1600.0, 1000.0))] {
            ...
            g.device.poll(wgpu::PollType::wait_indefinitely()).unwrap();
            let burst = tb.elapsed().as_secs_f64() * 1000.0 / 20.0;
            say!("{:<16} {:<16} {:>7.2}ms {:>7.2}ms {:>9.2}ms {:>11.2}ms {:>10.2}ms", d.name, tname, median(t1), median(tm), median(tf), burst, median(ts));
        }
    }
}
...
fn gpu_for_classic() -> Gpu {
    ...
    let (device, queue) = match pollster::block_on(adapter.request_device(&wgpu::DeviceDescriptor { label: Some("classic"), required_features: adapter.features() & wanted, required_limits: adapter.limits(), ..Default::default() })) {
        Ok(pair) => pair,
        Err(e) => { say!("gpuprobe: the classic device request failed on {:?}: {e}", info.name); panic!("device"); }
    };
    Gpu { device, queue, adapter_name: info.name, backend: format!("{:?}", info.backend) }
}
...
pub fn classic() {
    ...
    let threads = (std::thread::available_parallelism().map(|n| n.get()).unwrap_or(1).saturating_sub(1)).min(8) as u16;
    say!("CLASSIC VELLO 0.10.0 (all-GPU compute) on {:?} via {}: median of 15 after 3 warm-ups; burst = 20 frames submitted, one wait; against vello_cpu on {threads} threads", g.adapter_name, g.backend);
    say!("{:<18} {:<16} {:>9} {:>11} {:>13} {:>13} {:>10}   {}", "drawing", "track", "cpu MT", "encode(cpu)", "classic frame", "classic burst", "vs cpu px", "");
    ...
    for d in &drawings {
        ...
        for (tname, track) in tracks {
            ...
            say!("{:<18} {:<16} {:>7.2}ms {:>9.2}ms {:>11.2}ms {:>11.2}ms   {differing} differ ({pct:.1}%), max delta {max}", d.name, tname, median(tm), median(te), median(tf), burst);
        }
    }
}
...
pub fn timing() {
    let threads = (std::thread::available_parallelism().map(|n| n.get()).unwrap_or(1).saturating_sub(1)).min(8) as u16;
    let g = gpu();
    say!("host: {} threads available; vello_cpu MT uses {threads}; Level::new() = {:?}; GPU adapter {:?} via {}", std::thread::available_parallelism().map(|n| n.get()).unwrap_or(0), Level::new(), g.adapter_name, g.backend);
    let drawings = [portfolio_chart(), text_page(), octagons()];
    let tracks: [(&str, (f64, f64)); 3] = [("native", (0.0, 0.0)), ("desk 1600x1000", (1600.0, 1000.0)), ("phone 1200x2400", (1200.0, 2400.0))];
    say!();
    say!("{:<26} {:<16} {:>10} {:>9} {:>9} {:>11} {:>12} {:>9}   {}", "drawing", "track", "pixels", "cpu 1T", "cpu MT", "gpu render", "gpu+readback", "build", "hybrid vs cpu bytes");
    const N: usize = 15;
    for d in &drawings {
        for (tname, track) in &tracks {
            let track = if *tname == "native" { d.viewbox } else { *track };
            ...
            say!("{:<26} {:<16} {:>10} {:>8.2}ms {:>8.2}ms {:>10.2}ms {:>11.2}ms {:>8.2}ms   {differing} px differ ({pct:.2}%), max delta {max}{}", d.name, tname, pixels, median(t1), median(tm), median(tg), median(tgr), median(tb), if mt_exact { "" } else { "  [MT bytes != 1T bytes]" });
        }
        say!();
    }
}

/// Every mode, in order, headed and dated — the whole record on one host.
pub fn run_all() {
    say!("kaya gpuprobe: {} {} ({} threads available)", std::env::consts::OS, std::env::consts::ARCH, std::thread::available_parallelism().map(|n| n.get()).unwrap_or(0));
    say!("== timing =="); timing();
    say!("== breakdown =="); breakdown();
    say!("== fills =="); fills();
    say!("== classic =="); classic();
    say!("== done ==");
}

/// The device app's entry: run everything and write the report to `path`
/// (a C string), returning 0. The Swift host calls this off the main
/// thread and shows the file as it grows.
#[no_mangle]
pub extern "C" fn kaya_gpuprobe_run(path: *const std::os::raw::c_char) -> i32 {
    let path = unsafe { std::ffi::CStr::from_ptr(path) }.to_string_lossy().into_owned();
    match std::fs::File::create(&path) {
        Ok(f) => *REPORT.lock().unwrap_or_else(|e| e.into_inner()) = Some(f),
        Err(e) => { eprintln!("gpuprobe: cannot create {path}: {e}"); return 1; }
    }
    run_all();
    0
}
