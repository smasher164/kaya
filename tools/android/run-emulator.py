...
# Usage: tools/android/run-emulator.sh [compose|jvm|go|python|all]
#
...
if run([str(ROOT / "tools/check-targets.sh"), "android"]).returncode != 0:
    sys.exit(1)
...
# THE STAGED INSTALLS HAVE A WALL. tools/validate-all.sh starts the
...
def kaya_write_compose_marker():
    ...
    compose_id = out_of([str(ROOT / "tools/build-id.sh"),
                         "compose"]).strip()
...
# tools/check-gates.sh holds all five runners to this.
if status == 0:
