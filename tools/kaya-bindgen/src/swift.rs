...
pub fn emit(spec: &ProtocolSpec) -> String {
    ...
    c.line("}");
    c.line("");
    c.line("/// Decode one occurrence record (header included). nil for non-click");
    c.line("/// records. keys is [] for a click on a guest-created widget (id is a");
    c.line("/// widget id); otherwise id is a template node id and keys is the");
    c.line("/// copy's key path, outermost first.");
    c.line("func kayaParseClick(_ rec: [UInt8]) -> (id: UInt64, keys: [KayaValue])? {");
    c.line("    rec.withUnsafeBytes { raw in");
    c.line("        let kind = raw.loadUnaligned(fromByteOffset: 4, as: UInt16.self)");
    c.line("        guard kind == UInt16(KAYA_OCCURRENCE_BUTTON_CLICKED) else { return nil }");
