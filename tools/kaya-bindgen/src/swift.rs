...
//! from kaya.h through the CKaya module (Package.swift).
...
pub fn emit(spec: &ProtocolSpec) -> String {
    ...
    c.line("// little-endian. Constants come from kaya.h through the CKaya module.");
    c.line("");
    // The C API is the binding's private business — no guest names a C
    // symbol — so an INTERNAL import keeps a C type out of any public
    // signature by construction (tools/check-pins.py holds the mode).
    c.line("internal import CKaya");
    c.line("import Foundation");
    c.line("");
    // The one wire name a guest spells; everything else in this file is
    // internal to the Kaya module.
    c.line("public enum KayaValue: Hashable {");
    ...
    c.line("    /// A blob a decoder REDEEMED: an undo delta names a restored");
    c.line("    /// record's blob field by occurrence handle and the reader takes");
    c.line("    /// the bytes (crates/kaya/src/wire.rs, `undo_body`). Read-only —");
    c.line("    /// a write registers bytes and sends `blob`.");
    ...
    c.line("            u64(x)");
    c.line("        case .bytes:");
    c.line("            preconditionFailure(");
    c.line("                \"kaya: a redeemed blob's bytes are a READ value — a write registers them and sends a handle\")");
    c.line("        }");
    c.line("        pad()");
    c.line("    }");
    c.line("");
    ...
    c.line("}");
    c.line("");
    c.line("/// Redeem-and-release, for the two decoders that meet a blob: a");
    c.line("/// clip's values and an undo delta's restored record");
    c.line("/// (crates/kaya/src/wire.rs, `undo_body`). COPY THEN RELEASE, in");
    c.line("/// that order: the pointer borrows core memory that the release frees.");
    c.line("func kayaOccurrenceBlobBytes(_ handle: UInt64) -> [UInt8] {");
    c.line("    var length = 0");
    c.line("    var out: [UInt8] = []");
    c.line("    if let data = kaya_occurrence_blob(handle, &length) {");
    c.line("        out = [UInt8](UnsafeBufferPointer(start: data, count: length))");
    c.line("    }");
    ...
    c.line("            parts.append(.bytes(kayaOccurrenceBlobBytes(");
