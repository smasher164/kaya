...
fn go_receiver(ty: &str) -> String {
    ty.chars().next().unwrap().to_lowercase().to_string()
}
...
pub fn emit(spec: &ProtocolSpec) -> String {
    ...
    c.line("");
    for e in spec.enums {
        let ty = enum_go_type(e.name);
        for (name, value) in e.variants {
            match ty {
                Some(t) => c.line(&format!("\t{}{} {} = {}", camel(e.name), camel(name), t, value)),
                None => c.line(&format!("\t{}{} = {}", camel(e.name), camel(name), value)),
            }
        }
    }
    ...
    c.line("");
    for e in spec.enums {
        if let Some(ty) = enum_go_type(e.name) {
            let recv = go_receiver(ty);
            c.line(&format!("func ({recv} {ty}) String() string {{"));
            c.line(&format!("\tswitch {recv} {{"));
            for (name, _) in e.variants {
                c.line(&format!("\tcase {}{}:", camel(e.name), camel(name)));
                c.line(&format!("\t\treturn \"{name}\""));
            }
            c.line("\t}");
            c.line(&format!(
                "\treturn \"{ty}(\" + strconv.FormatInt(int64({recv}), 10) + \")\""
            ));
            c.line("}");
            c.line("");
        }
