...
2. Fast gates. `tools/gates.py` runs ALL of them and is the only thing
   ...
   *.sh population where they were policed by nothing (audit
   ...
   matrix runs neither. Five watched negatives, counts printed: a key
   missing from one locale, an argument a translation dropped, a guest
   asking for a key no catalog has, an unbalanced brace, the default
   locale's file gone),
