...
`widgets` and `template_nodes` as separate maps — so `tools/check-c-ids.py`
...
tier being the deliberately explicit floor. `tools/check-c-bounds.py` holds
...
- A canonical method vocabulary for derived signals: `eq`, `ne`, `lt`,
  ...
  tools/check-steps.py gates it. That put their iteration mechanisms in reach: Go's
...
- A guest asks KAYA where the platform keeps things — `kaya.Env` and
  ...
  tools/check-go-env.py holds the rule as static text for the one
...
- **Each platform decides where one pane becomes two, and kaya does not
  ...
  `tools/check-steps.py` enforces that band. The separate 600 that
...
(crates/kaya/src/scene.rs's MENU_ROLES; tools/check-roles.py holds
...
**A tap after the app has exited reaches the app** (docs/tasks-s9-plan.md).
The platform starts the process through its own door — the centre's
delegate on macOS and iOS, the content intent on Android, the portal's
action on Linux, the COM activator on Windows — and that launch is handed
to the core as the same `notification_result` a running app would have
received, queued when it arrives before the app thread exists. A
relaunched process never called show, so the one-shot registration cannot
exist and the app registers a process-level activation handler beside it;
the id is the app's own, which is what lets it map the tap back to the
thing the reminder was about. macOS and iOS have no programmatic tap, so
the lanes exercise the app's handling through the same funnel the delegate
uses and the OS's own relaunch is measured by hand and recorded — the
carve-out already stated for `notification_activate`, one step further.
The harness continues a scene in the relaunched process: a `relaunch`
...
sentence is a gate rather than a hope: `tools/check-app-identity.py`
...
- The invariant, uniform across platforms: exactly one UI thread runs all
  ...
  `Kaya.attach(this, filesDir.absolutePath)` (answered by the android_main! expansion —
  the second argument is the app's state root, which the core cannot derive on
  Android and needs before the app thread exists for the harness's second act,
  docs/tasks-s9-plan.md R6b; the
  return value says whether kaya presented or a guest-side backend like
  Compose should be mounted), or `KayaRing.attach(this, filesDir.absolutePath)` when the JVM
...
- Sum-typed elements complete milestone 3: a collection's schema is one
  ...
  is restated) and emitting a checked-in file that tools/gen-guests.py
...
- `tools/validate-mac.py` runs every scene natively on macOS across all
  ...
  `tools/deploy-win.py` runs five (Rust, Python, Go, C#, Java) on the
...
- Windows testing runs in a UTM Windows 11 ARM VM on the Apple hypervisor
  ...
  `tools/deploy-win.py user@host [--provision] [rust|python|go|csharp|java|all|<scene>_<lang>]`
