...
- The invariant, uniform across platforms: exactly one UI thread runs all
  ...
  owning it (validated at milestone 0: the entry is called from
  Activity.onCreate, builds the scene, and returns the thread to the
  Looper; the doorbell posts into it). The hosting relationship flips
  there; the threading invariant does not. This is also the eventual
  public split of the entry point, named by loop ownership: `run` — kaya
  owns the loop, one call from main, the seamless default on every
  platform where the process is yours — and an `attach` shape for hosts
  that already have a UI loop (embedding in a larger app), which is
  exactly what the Android backend does internally and will be promoted
  from once a second embedder needs it.
...
- Milestone 0 is a skeleton on every platform (window, event loop, Button,
  ...
  binding-bootstrap subject, deferred with it. A minimal C surface
