...
| C4 | answering from the notification itself (DESIGN: docs/notification-reply-plan.md, R1 ruled 2026-09-25. DEFERRED 2026-09-26 by the maintainer: built in one pass once kaya can run a reply on Android with no Activity, after C5-C7) | notification reply actions: a text field in the notification, the reply as an occurrence |
| C5 | an emoji button beside the compose field (BUILT 2026-09-26: an emoji button beside Send where the platform has a picker, none on iOS; docs/emoji-picker-plan.md) | the platform emoji picker (GTK EmojiChooser, the mac character palette, WinUI's emoji panel, the phones' keyboards) |
