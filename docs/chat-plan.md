...
| C2 | an unread count on the dock and taskbar (DESIGN: docs/app-badge-plan.md, R1 ruled 2026-09-25: the call never fails) | the app badge (tasks has section badges only) |
...
| C5 | an emoji button beside the compose field (DESIGN: docs/emoji-picker-plan.md, R1-R2 wanted) | the platform emoji picker (GTK EmojiChooser, the mac character palette, WinUI's emoji panel, the phones' keyboards) |
| C6 | an image attachment shown inline (DESIGN: docs/photo-attach-plan.md, R1-R3 wanted; the image needs a bounded fit, so display is not "nothing new") | nothing new for display; the photo picker on the phones |
| C7 | swipe a conversation to archive it on the phones (DESIGN: docs/swipe-actions-plan.md, R1-R3 wanted) | swipe actions |
