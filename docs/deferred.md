...
sheet, so the Windows sheet is a modal Popup.

DEPTH LANDED 2026-09-21: the spec records (tx 58-60, apply 46-48,
occurrences 31-32, `SHEET_PROPS`, the `detent` enum) and the hash, the
core's surface table with its refusals under nine unit tests, the Rust
sugar (`present_sheet`, `present_sheet_over`, `dismiss_sheet`, `SheetRef`,
`on_sheet_dismissed`, `on_dismiss_requested`), the four harness verbs, the
SwiftUI arm (a `.sheet` on every surface root, the chain through the sheet
content's own host, the title on the sheet NSWindow, Esc through the
platform's own path when unarmed and through a local key monitor when
...
- **The system-integration floor** (from the survey; the editor
  ...
  **clipboard** — note it is SYNCHRONOUS on
  ...
  seven typed refusals watched firing on a spliced breach).
  THE TEMPLATE ZONE IS PROVEN ON THE ANDROID LANE 2026-09-03
  (docs/dnd-plan.md §5's Compose paragraph): the depth-2 scene — a
  stamped source dragging its own per-copy payload, a stamped
  destination, and a reorderable row's own KEYED `drag_ended` — is green
  as `dnd-compose` with NO new Kotlin, because a stamped copy's
  declaration arrives as the same apply op a live one's does and every
  stamped copy now carries its (template node, keys) tag. That lane cuts
  `drag_file` alone, since no foreign source reaches a phone app (D9):
...
(The image line read `"64x64"` until 2026-09-08, when the widget moved off
the declared mark onto the scenery picture so that no shared scene depends
on the icon's size — docs/packaging-plan.md P2; the mark's bytes are still
read for the label.)
...
lane's rust-swiftui dnd leg runs the WHOLE scene — the keyed stamped source
and the stamped destination, the row's own `drag_ended`, and `drag_file`
through a `KayaDropSessionDouble` with a nil `localDragSession`, which is the
first leg anywhere to exercise the files branch. It found two defects in that
branch (no `public.file-url` off a real foreign drop; a temp copy named after
the TYPE) — both in docs/traps.md, both fixed.

THE MAC CROSS-APP WITNESS IS THE OPEN ITEM AND ITS OBSTACLE IS MEASURED,
not guessed: `beginDraggingSession` never composes a session for a
synthesized gesture, with the pointer moving and the button down (the probe
holds the whole table of variables tried). tools/mac/dragwitness is the
process; `--board` proves the cross-process byte exchange and `--selftest`
watches the shipped-crash negative it found. What is owed: either one hand
run with a person at the machine, or the check-pane-ladder-shaped gate the
probe names.
...
emit entries, and the regenerated bindings. THE TEMPLATE ZONE LANDED IN
ALL NINE 2026-09-03 (docs/dnd-plan.md §4 and its table): a declaration in
the For's body gives every stamped copy a CONSTANT payload and the
...
2026-09-03, ruled the same day ("churn is free"): inside a For's body a
representation IS the row's own field, spelled the way that binding's
template `label` takes one, and the binding computes the slot's canonical
index and packs `level << 32 | field` under record 49's `bound` mask
(docs/dnd-plan.md §4's table carries the nine spellings and their
verdicts). A file never binds; the LIVE chain and the KEYED form stay
constant-only — refused BY TYPE in the seven bindings whose template zone
now has its own chain (`TplDragRef`, `KayaTplDragRef`, OCaml's
`~text_field`, Haskell's `TplClip`) and BY NAME in Python and JS. The
scene's rename button is the proof the copy re-declares when the row's
record moves, and the keyed form is now the per-copy OVERRIDE rather than
the only route a row's data reaches its payload.
...
each a stub the runner reads (tools/check-stubs.py):
  ...
  - ~~**DEPTH STUB: dnd on gtk**~~ — LANDED 2026-09-03: a GtkDragSource
    ...
    DEPTH 2 ON THE SAME LANE 2026-09-03 — the files drop, the template
    zone, the reorder's insertion indicator and the cross-app witnesses,
    all four green on both pools (docs/dnd-plan.md §5 step 7). The
    `drag_file` verb's arm is `deliver_drop`, the destination's own arms
    factored out of `connect_drop` so the platform's signal and the verb
    enter one path: `GdkDrop` is abstract and has no constructor, so a
    drag nobody started cannot be handed to the signal, and the verb
    presents a `ClipOffer::Foreign` the verdict answers `local: false`
    for. `materialize` is what registers the file in the picked table, so
    a dropped file is redeemed exactly as a pasted one is. The template
    zone needed nothing: the per-widget arms already work per stamped
    copy, and the scene's `label@item[y]` / `label@item[x]` steps are the
    proof. THE INSERTION INDICATOR is a 3px accent line at the landing
    edge of the row under the pointer, `background-image:
    linear-gradient(...)` on the row (a line, not a shadow, and no layout
    — a border would displace the rows the drop is being aimed at), put
    up on enter and motion and taken down on leave, on drop and at the
    drag's end; no scene can see it, so tools/check-verbs.py holds its ten
    links with ten watched negatives, and it was filmed once by hand.
    THE CROSS-APP WITNESSES are tools/linux/dragwitness.py (a stock
    PyGObject GTK4 client with nothing of kaya in it) driven by
    tools/linux/dragwitness-leg.py: `dndwitness-out` drags kaya's source
    into the witness and `dndwitness-in` drags the witness's file into
    kaya's files target through the REAL GtkDropTargetAsync, both legs on
    both pools, both ends read on every leg. Two more findings in
    docs/traps.md: AT-SPI cannot say where a widget is (SCREEN extents 0,0
    on both protocols, WINDOW extents 26px off), so the leg takes its
    geometry from the harness's own `KAYA_DIAG dragdrive` line; and
  ...
  - ~~**DEPTH STUB: dnd on winui**~~ — LANDED 2026-09-03: BOTH routes, as
    ...
    DEPTH 2 ON THIS LANE, 2026-09-03 — the files drop, the template zone,
    the insertion indicator and BOTH cross-app witnesses. The template
    zone needed no new WinUI arm and that is the finding: a stamped
    copy's `set_drag_source`/`set_drop_target`/`accepts` arrive as the
    SAME apply ops with the copy's own tag, every one of those arms is
    keyed by widget id, and `wire_dnd` wires each copy as it is created —
    so the whole new scene went green on its first run (27s, 21
    observations). `drag_file` is the CLASSIC OLE ROUTE'S OWN IN-PROCESS
    EXERCISE rather than a stand-in: the verb hands kaya's registered
    `IDropTarget` a CF_HDROP-only `IDataObject` at the destination's
    screen point, hit test, verdict, `picked_register` and the `dropped`
    emission all real, measured at 51ms. That data object is a HAND-ROLLED
    vtable for `ole_get_data`'s own reason — STGMEDIUM's generated
    declaration is gated on `Win32_Graphics_Gdi`, so neither
    `#[implement(IDataObject)]` nor the generated `SetData` exists in this
    feature set (crates/kaya/Cargo.toml says why that feature stays off).
    THE INSERTION INDICATOR is kaya's own, since D8 declines
    `CanReorderItems`: a 2-DIP `{ThemeResource AccentFillColorDefaultBrush}`
    line at the hovered row's landing edge, carried by a `Popup` because
    WinUI has no adorner layer and a child of the For's own panel would be
    a child every `child_order` index counts. No scene can see it, so
    tools/check-verbs.py byte-freezes its four arms with a watched
    negative each. THE WITNESSES ARE TWO LANE LEGS, alone between drains
    (tools/guest/dnd-witness.ps1, the stock OLE app of
    tools/win/dragprobe staged and built by the deploy): kaya's own drag
    reached a foreign Win32 reader with `dev.kaya/note` intact as a
    stream, `CF_UNICODETEXT` "hello" and the pid-named local marker
    visible in the foreign format list, and a foreign `DoDragDrop` reached
    kaya through `RegisterDragDrop` with text and a FILE — the classic
    route's FIRST REAL DROP, armed since the morning with nothing ever
    sent to it — and Explorer itself in a third phase, `files target got
    explorer.txt explorer bytes (copy)`, which is probe 2's own question
    answered by the lane. Both assert through kaya's own labels, read back
    over UI Automation from outside the process. The Explorer phase cost
    three runs to a diagnostic that could not discriminate; the census
    that replaced it named the cause in one (docs/traps.md, PowerShell's
...
## ~~GAP — a mac input-driving leg run while a human holds the host's foreground is a swallowed-press red (first recorded 2026-09-18; the bundle names the frontmost app)~~ CLOSED 2026-09-18 (the job's tmp/idiom/notes-small.md, Part A): the mac lane's EXCLUSIVE set was EMPTY, so its hold branch was dead code — it is derived from the queue now, the twenty legs of the three scenes that press the file panel through the accessibility client (filedialog, save, editor; `type` is in-process and a drag a private pasteboard, so neither joins) — and INSIDE that hold the funnel reads HIDIdleTime and waits, bounded, for the host to have been idle 20s: 60s per leg, then the leg runs anyway and says so; 240s per lane, sized against the mac ceiling rather than bound x set; every wait skipped on a quiet host, which costs nothing. Every red mac verdict line carries the frontmost app and title at the moment of the red, read through the recorder's own window list. Proven three ways: a quiet full lane with zero waits printed; the same lane with a real human on the host — two legs cleared (12s, 6s), two expired at 60s and ran anyway, ALL PASS 477 legs; the wait's every branch watched through a self-test door (`KAYA_HID_IDLE_NS_OVERRIDE`, its only reader refused elsewhere by check-exclusive, every doctored sentence marked) including the budget clamp landing at exactly 240s; and a forced red reading `frontmost Terminal "…" (pid …)` on its FAIL line. check-exclusive's clause 7 holds the wait inside the hold, the literal bounds and the sentences (six watched negatives). The twenty holds cost the matrix-wide token ~137s per mac run.
...
## ~~GAP — act two's verdict file could be read EMPTY by the runner that polls it, and the act killed as a failed answer (first sighting 2026-09-18, matrix #40, links-rust-x11)~~ FIXED 2026-09-18: the harness publishes the verdict by rename (`<path>.tmp` then rename, held by harness::tests' the_act_two_verdict_is_published_by_rename, watched failing on the pre-fix text), and tools/linux's three pollers treat a file without its final newline as not yet answered; the other five pollers ride the writer's rename (docs/traps.md, the act-two verdict race). The bundle had read as a silent death — the act's log ended at its last step with no verdict and no ring dump — because the runner's own read was the killer, and the runner's sentence (`act two answered after 2.2s: ` with nothing after the colon) is what named it.
KEY: act2.verdict, published by rename, poll_verdict, answered after, empty verdict

## ~~GAP — a wayland clipboard seed expires when the guest window is not active: the compositor never tells an unfocused client the selection moved (first instrumented sighting 2026-09-18)~~ CLOSED 2026-09-18 (the job's tmp/idiom/notes-wlseed.md): the class reproduced deterministically with a second client taking the seat on the leg's own session (0/5 seeds before, 5/5 after), and fixed at its cause — `clipboard_seed` asks the compositor for this window's focus (`swaymsg '[pid=…] focus'`, whose own `success` answer the seed's sentence carries, since a request that read only the exit status would report a grant that never happened) before it spawns the foreign writer; the two in-process readings the charge named were measured unable to decide it — `gtk_window_is_active` is false on EVERY step of a green wayland leg (the headless seat advertises no keyboard, so gdk never sees `wl_keyboard.enter`) and `present()` loses on sway, which denies a self-activation token; the linux recorder carries a FOCUS RING now (tools/linux/focus-ring.py, once a second, sway's focused node on wayland and each xdotool field asked separately on x11 — the chained form printed `focus=none` for a focus it had), declared in FLIGHTREC_SECTIONS_LINUX and held by check-flightrec, proven on forced reds of both protocols; check-verbs holds the request before the spawn and the answer in the sentence (six links); the whole linux lane ALL PASS 775 legs. STILL OPEN INSIDE THE CLOSE: what took the seat in matrix #38 is unrecoverable (that bundle predates the ring); CPU load alone did not reproduce it (3 runs × 9 legs under 16 in-container spinners, 0 expiries), so the matrix ingredient is a hypothesis the ring answers next time; and THE COPY VERB HAS THE SAME SEAT DEPENDENCY WITH NO GUARD — under a thief holding the seat across the copy steps the guest's own set_selection is dropped (taking the selection needs an input serial), every expect_clipboard reads "" for 15s and gdk believes it owns the board, and a later seed expires even with the focus granted (measured twice in five runs); the remedy was the same request in front of every copy — TAKEN 2026-09-18 (the job's tmp/idiom/notes-small.md, Part B): one shared `clipboard_focus(what)` serves the seed and the copy, the copy asks ONCE PER COPY (the seat was taken between two copies of one leg, measured), and the grant is READ BACK out of sway's own `[pid=N con_id=__focused__] nop` criterion, because a 20Hz sampler caught `success: true` reported for a focus that never moved (a second client's window still coming up); the copy's class is 0/5 -> 5/5 under the thief, the six clipboard legs green by hand, the linux lane ALL PASS 775 legs, check-verbs at ten links and thirteen watched negatives. WHAT STAYS OPEN, MEASURED: a thief that takes the seat ~650ms before the leg's FIRST copy is 0/5 before and after, because a copy needs an input SERIAL as well as the seat, wtype's tap reaches a client only through a `wl_keyboard.enter`, and a window refocused milliseconds ago has not been sent one — the same copy is 5/5 with four seconds of settle between the steal and the copy; nothing inside the process reads the enter, so closing that would wait for it rather than for the focus (docs/traps.md). Under a thief taking the seat four times a second every copy lands and the leg dies at `menu_activate "Edit>Paste"`, GTK's own asynchronous paste losing the seat mid-read — a third surface. And `clipboard-ocaml-wayland` is not in run-suites.sh's KAYA_EXCLUSIVE_LEGS (python, js and rust are), which is a question, not a decision. KEY: clipboard_seed_focus, swaymsg focus, focus ring, focus-ring.py, prime_if_clipboard_scene, copy verb seat, wl_keyboard.enter
...
## ~~BUILD — task manager S3: local notifications~~ SHIPPED (rulings TAKEN 2026-09-08; SHIPPED 891ad047 the same day, matrix ALL PASS 1,745 legs on five lanes)

KEY: tasks S3, show_notification, cancel_notification, notification_result, notification_outcome, KAYA_CAP_NOTIFICATIONS, expect_notification, notification_activate, notify scene, BUNDLED_SCENES, identity.toml id, provisional authorization, portal notification
- The maintainer's rulings (docs/tasks-s3-plan.md N1-N7, "i'm cool with the rulings"; the Linux floor of 2026-09-07: post only where the desktop remembers and relaunches, no resident process, no inert click). Spec: TX 52 `show_notification` {notification, at, title, body}, TX 53 `cancel_notification`, occurrence 27 `notification_result` {notification, outcome}, enum `notification_outcome` {activated, refused}, `KAYA_CAP_NOTIFICATIONS` as the first RUNTIME capability bit (`kaya_grant_capabilities`, the core deciding the mac bundle bit before the app thread exists); hash 0x21005150bc085070.
- DEPTH GREEN on the mac 2026-09-08: the `notify` scene (guests/rust/notify.rs, tools/scenes/notify.steps) bundled by the mac lane (`BUNDLED_SCENES`, `stage_rust` in tools/lib/lanes/mac.py, one copy for validate-mac and run-leg; Info.plist from identity.toml's new `id`), the SwiftUI arm on UNUserNotificationCenter with provisional authorization under the harness, the three verbs in harness.rs and the interpreter, Rust's `show_notification`/`cancel_notification`/`on_notification`.
- OPEN, the breadth slice (docs/tasks-s3-plan.md §6 step 3):
  - THE SUGAR IS IN ALL NINE (2026-09-08): `show_notification` / `showNotification` / `ShowNotification` with the handler bound AT THE SHOW under each binding's own alert keyword (Python `on_result=`, Go/Java chained `.OnResult`, C#/Swift `onResult:`, OCaml `~on_result`, Haskell an attribute list plus the handler, JS an options object with the promise twin when `onResult` is absent), `cancel_notification` beside it, ONE-SHOT retirement at the result read out of each dispatch arm by tools/check-sugar-surface.py (a binding whose table kept the closure would fire a stale handler for a reused id on five lanes, silently), the `notifications` capability boolean in nine and `cap_numbers()` table-driven over EVERY `CAP_*` the core declares. FOUND BEFORE A LINE WAS WRITTEN: `notification_result` is byte-for-byte `alert_result`'s shape and all eight generators hard-coded the alert's decode arm BY NAME, so the new record was read by the click tail — the outcome taken for a key-path length, dropped when `activated`, one byte past the record when `refused` (docs/traps.md, "A record decoded by name"); the arm is DERIVED now (`code_answer_occurrence_names`) and `every_code_answer_is_decoded` refuses a generated file missing one, on every regeneration and on `--check`. `file_dialog_result` is the ONE by-name arm left in all eight (`{dialog u64, count u32, reserved u32, files Values}` — a second record of that shape would be read by the generic tail exactly as this one was); KEY: file_dialog_result by-name decode arm, OCC_FILE_DIALOG_RESULT. N4 IS BUILT: `app_identity()` takes no arguments in nine, the overloads RETIRED, the core reads `name`, `icon` and `id` out of guests/assets/identity.toml through the asset root and refuses a manifest missing any of the three or a record that carries values; check-app-identity's C4 had matched SEVEN of the nine identity guests since it was written (a glob that missed Go's nested file, C#'s and Java's names and matched the manifest instead) and is a walk with a floor of nine now, C8 holds the id to reverse-DNS, twenty watched negatives.
  - iOS WIRED 2026-09-08, AND THE PHONES' SHADE TAP IS NOT AVAILABLE THERE: the iOS lane is ALL PASS at 132 legs with `notify-swiftui` (3s), provisional authorization grants on the simulator with no prompt (alertSetting disabled, notificationCenterSetting enabled — the post is delivered quietly and the delivered list reads it back on the first read), and `notification_activate` takes the SAME single arm on both of the SwiftUI backend's platforms — the backend's own activation path one step past the tap — because six tap routes on the reachable shade (coordinate, element and element-relative, on the cell and the platter's button, provisional and full, foreground and backgrounded, every element found and hittable) never reach the delegate while the same driver's `sb_tap clear-button` and `press Allow` work in that shade; the one presentation that activates, a live banner over another app, is closed to the scene twice over (docs/traps.md, "The iOS simulator's shade will not activate a notification"; docs/tasks-s3-plan.md N5's amendment; tools/ios/notifyprobe and the driver's `shade` and `sb_drag` verbs reproduce it). A physical device is untested and might answer. CORRECTED BY S9 (2026-09-08): a BANNER over SpringBoard for an app that has exited does activate on a tap and relaunches it with the identifier (`hittable=false` is not a refusal on that route; docs/tasks-s9-plan.md §2.1) — the shade still activates nothing, and the carve-out stands because a banner is a five-second window no leg can depend on. KEY: shade tap, notifyprobe, ListCell, compact pill, banner.
  - A TARGET IS A LIVE WIDGET, four backends, one rule (2026-09-08; docs/traps.md, "A keyed target answers from a popped screen"): the interpreters (SwiftUI, Compose) resolve among nodes REACHABLE from a presented root of their own model — windows' roots, sections' roots, live entries' roots — memoized on a generation bumped by every apply batch AND by the user's own back, which is no batch; the two toolkit backends record what is DEAD instead — the mounted root of every surface kaya took away, at ApplyOp::PopEntry, at the user's back (a NATIVE pop is a different code path from the programmatic one, and `user_back` never reaches the apply op) and at DestroyWindow — and walk the SCENE's own parent record up from a candidate that already carries the authored id (WinUI's `tree_parent`, written at the top of AddChild before its early returns, because `child_order` is a LAYOUT structure with deliberate holes at a scroll's content child and a grid's cells; GTK's `widget.parent()`, which on GTK IS kaya's child order and keeps a detached subtree's internal links). The POLARITY is the finding that cost two windows lane runs: asking "is this widget still somewhere alive?" makes every honest unknown DEAD — a non-selected section's pane, a covered entry's page, a root detached while a split rebuilds, a control not yet through layout — so the question is "is it inside something kaya knows is gone", and an empty dead set answers live at once. Guard: `expect_no_target` after each `back` in tasks.steps on every lane; tools/check-steps.py holds both interpreters' predicates with watched negatives. KEY: expect_no_target, dead_roots, dead_subtrees, tree_parent, kayaLiveIds, kayaLivePresentedIds.
  - THE MAC WRAPPER CARRIES THE MANIFEST'S NAME AND MARK (2026-09-08): Notification Center had shown the scene stem ("the notify app") with a blank icon, since the lane's wrapper named the bundle after the stem and shipped no icon; tools/lib/lanes/mac.py reads `name`, `icon` and `id` through one `app_identity(root)` and writes CFBundleName, CFBundleDisplayName and an AppIcon.icns built from the 64px mark through an iconset (sips refuses an icns straight from a small png). MEASURED: the centre CACHES an app's name and icon per bundle id from its first post, so a rebundled app under the same id keeps the old ones (a fresh id shows the mark), and the post-delivery prompt ("Keep receiving notifications from the tasks app?") still names the bundle FOLDER (`tasks.app`) rather than CFBundleDisplayName — OPEN, a wrapper-folder naming question for the lane, not a shipping app's. KEY: CFBundleDisplayName, AppIcon.icns, iconset, Notification Center cache.
  - N7 built on the mac 2026-09-08 (the tasks guest's reminders, tools/scenes/tasks.steps' notification block, green under a fresh bundle id); the breadth holds below are what keeps it off the other four lanes.
  - ~~**DEPTH STUB: notify on gtk**~~ — CLOSED 2026-09-08. The GTK arm takes the ruled floor's two routes and nothing else: at startup it asks the session bus whether a portal answers `org.freedesktop.portal.Notification` and whether `org.freedesktop.host.portal.Registry.Register` takes the manifest's `id` ON THE POSTING CONNECTION, else whether `org.gtk.Notifications` has an owner, else grants no capability at all — one function, so the bit `kaya::run` grants and the route a post takes cannot disagree. A post is `AddNotification` with the guest's title and body and a default action whose target is the identifier string; a click arrives as the portal's `ActionInvoked` signal or as GNOME's `ActivateAction` on the app's own `notify-activated` action, answers the guest ONCE and withdraws the platform's copy; `at` goes to the session's scheduler (`systemd-run --user --on-calendar=… gapplication action <id> notify '<identifier>'`, then `at`, then a main-loop timer) and a cancel stops the unit and withdraws. The hand-spelled `dev.kaya.Milestone2` is gone: the GApplication id is the manifest's, read through the asset root. The lane runs six legs — `notify-rust`, `notifygnome-rust` and `notifyrefusal-rust` on x11 and wayland — and the tasks leg runs through tools/linux/notify-leg.sh as well as a11y-leg.sh; the image carries xdg-desktop-portal 1.20.3 and its GTK backend with their D-Bus service files OUT of the default search path, since a portal any leg can activate takes over GTK's file chooser (24 legs, measured; docs/traps.md). Linux lane ALL PASS, 761 legs in 266s. MEASURED WITH IT: the portal's Registry is per CONNECTION and needs a desktop entry whose `Exec` program is ABSOLUTE (docs/traps.md); the kaya id does not survive the freedesktop hop, so the harness's platform read joins on the title while the ACTIVATION carries the id exactly; a click after the app has exited D-Bus-activates it but the action target reaches no one — S9 MEASURED WHY (2026-09-08): the portal route's default action was spelled bare, a portal-scope name whose ActionInvoked goes to the dead posting connection, while `app.notify-activated` makes the backend call ActivateAction on the relaunched process (docs/tasks-s9-plan.md R4); and `gapplication action … 12` is refused by GLib as INT32 (the CLI parses a bare parameter with no type hint), which is why both actions take the identifier string — pinned by `gtk::notify_tests` against gapplication's own parser, run by name from tools/check-gtk.py with two watched negatives (a bare number, an unquoted identifier). KEY: org.gtk.Notifications, portal Registry, notify-activated, kaya-portal, notifyd.py, schedrec.py, gapplication.
  - ~~**DEPTH STUB: notify on winui**~~ — CLOSED 2026-09-08, through the OLDER WinRT toast API and not the App SDK's manager, which is the day's measurement and not a preference: on the VM (Win 11 Pro 26200 arm64) `AppNotificationManager.Register()` from an unpackaged exe returns Ok and files the app under an AUMID derived from the EXE PATH (the API takes no id, so the manifest's `id` has no sink there), then `IsSupported()` false, `Setting` Unsupported, `Show()` Ok delivering nothing, `GetAllAsync()` a null vector — the Windows App Runtime's Main and Singleton packages are not deployed for an unpackaged app and the notification stack lives in the Singleton; `DeploymentManager.GetStatus()` fail-fasts the process (docs/traps.md, "The Windows App SDK's notifications are a silent no-op"). The arm posts, reads, schedules and removes through `ToastNotificationManager` under the manifest's `id` as the AUMID (`SetCurrentProcessExplicitAppUserModelID` first, so the process and its toasts wear one identity), NO provisioning of any kind — proven by deleting the hand-made AUMID key and re-running with the VM's global toast toggle OFF, which suppresses the banner and not the platform's record: `expect_notification` reads `ToastNotificationHistory.GetHistoryWithId` by tag with the title out of the toast's own payload, cancel removes from history AND schedule, a future `at` is a `ScheduledToastNotification` (measured firing into history five seconds on), activation is one funnel both `ToastNotification.Activated` and the harness verb enter (N5's Windows carve-out). New diagnostic `notification_miss_census` prints, once per id after two seconds of missing, the app id asked under, the tag wanted and every tag the history held, since one sentence had served three causes. The windows lane ALL PASS at 273 legs (suites 452s under 600), `notify_rust` 2s, `tasks_rust` 17s with the reminders firing as toasts. OPEN: WINDOWS DRAWS A TOAST ONLY FOR A REGISTERED AUMID (measured 2026-09-08 for the review page: with the VM's banners turned on, the unregistered app's post stayed invisible and the centre read "No new notifications" while the history held it; `HKCU\Software\Classes\AppUserModelId\<id>` with DisplayName and IconUri from the manifest made the banner and the centre entry, name and mark both) — the arm should write that key itself at first post, the mac wrapper's plist one platform over, with the mark written under LocalAppData for the IconUri; KEY: AppUserModelId, DisplayName, IconUri. Also OPEN: a click on a toast after the app has EXITED lands nowhere on the classic route (no COM activator; S9's Windows piece), the platform's own `Activated` callback is unmeasured since nothing can click a toast on the VM, and RULED 2026-09-08: the COM activator (docs/tasks-s3-plan.md §3's Windows amendment; the S9 entry). KEY: AppNotificationManager, Singleton package, ToastNotificationManager, AUMID, notification_miss_census.
  - ~~**DEPTH STUB: notify on compose**~~ — CLOSED 2026-09-08. The Compose backend posts, schedules, cancels and reads back local notifications, and the android lane carries the `notify` scene: one channel ("kaya-reminders", named with the package's own label from guests/assets/identity.toml), `Notification.Builder` on the platform API (minSdk 26 makes NotificationCompat a wrapper over the same call), the platform key carried in the TAG (`kaya-<id>`) because a kaya id is u64 and `notify` takes an int, a content PendingIntent addressed EXPLICITLY to the app's launcher component with NEW_TASK|CLEAR_TOP|SINGLE_TOP (an ACTION_MAIN intent to a live task brings the task forward with no `onNewIntent`), and `AlarmManager.setExactAndAllowWhileIdle` into a manifest-declared `KayaNotificationAlarm` for a future `at`. THE CAPABILITY IS MEASURED AT ATTACH, in crates/kaya/src/android.rs's two attach entries and not at mount — `attach` spawns the app thread and returns, so a grant on the mount path races the guest's first `capabilities()` read — by the platform's `areNotificationsEnabled()`, and `KayaPresent.grantCapabilities` moves it when a permission is granted mid-run. THE PERMISSION IS ASKED AT THE FIRST POST and answered from the permission STATE at the next ON_RESUME rather than a one-shot launcher callback, since a launcher belongs to the Activity instance that registered it and a recreation mid-prompt loses the answer (89338ee6's class; tools/check-harness-ceiling.py holds one `register` call). `notification_activate` drives a REAL tap: the verb prints `KAYA_REQUEST: notify_tap <seq> <title>`, the runner expands the shade, finds the row by title in a uiautomator dump, taps it and collapses (2.7-2.9s, the row on the first dump), and the verb waits for the activation to reach the core before acking — the `drag` verb's one hand, one feature over. Measured on emulator-5560: `activeNotifications` lists a post in the same millisecond band as the statement after it; Allow and Don't-allow both answer their post (granted posts, refused otherwise) and the capability reads false at attach with the permission revoked; an exact alarm asked for 1788836880 fired 459ms late into a process that had ALREADY EXITED, posting through the receiver — S9's Android piece proven early. The android lane ALL PASS at 141 legs with `notify-compose` (4s) and `tasks-compose` (15s) through the shade tap; `notify-compose` joins the lane's EXCLUSIVE set. OPEN beside it: no leg on any lane covers the FUTURE-`at` route (the tasks guest's fixed today puts every reminder in the past, so the scenes exercise `at == 0` only; the alarm, calendar-trigger, scheduled-toast and timer routes are hand-measured), and the runner leaves `/sdcard/kaya-shade.xml` behind after a tap, overwritten per tap and deleted by nothing. AND THE SMALL ICON IS BLANK in the shade: the arm passes `context.applicationInfo.icon` (the adaptive launcher icon) to `setSmallIcon`, which Android renders as an empty square — a small icon is a monochrome alpha drawable, so the identity build should write one from the mark beside the mipmap; the review page's Android shade capture shows it. KEY: setSmallIcon, small icon.
  - iOS's driver: the shade tap for `notification_activate` and SpringBoard's "Allow" — the interpreter arm is shared with the mac already.
...
## ~~POLISH — the GTK sections sidebar is the narrowest of the five (the maintainer, from the S4 review captures, 2026-09-09)~~ CLOSED 2026-09-09: the `sidebar` presentation's row list takes libadwaita's OWN band, already in the tree as `protocol::leading_pane_width` (a quarter of the window clamped to 180..280) as its width request, re-read on the toplevel's `default-width` notify, primary and aux alike — measured in the lane's image at the tasks app's 960x640: 119px -> 239px (12.4% -> 25%, between the mac's 16% and Windows' 27%), 279px at 1400 and 179px at 640, each photographed from a fresh window (docs/traps.md, the bare-Xvfb root grab); no observable reads a pane's width, so the guard is four watched census entries in tools/check-gtk.py, the sharpest the UNCLAMPED fraction (the same 240 at the declared width, wrong at both ends)

KEY: GTK sidebar width, sections sidebar, sidebar_row_list, leading_pane_width, kaya's own row list
- Measured off the S4 review captures at the tasks app's declared 960x640: GTK's sections sidebar is 125px of 960 (13%), the mac's is about 145pt of 900 (16%, the platform's own NavigationSplitView column), Windows' is 242px of 902 (27%, the NavigationView pane). GTK's is kaya's own row list (the chrome the `sidebar` presentation builds when libadwaita has no idiom for it), and its width is whatever the rows measure — "Upcoming" sits a few pixels from the pane's edge. The maintainer asked whether it reads as excessively narrow beside the others; it does. The fix belongs to the sidebar's own width rule (a floor in the 180-280 band protocol::leading_pane_width already uses for panes, or the platform's own sidebar width where libadwaita offers one), measured on the same capture; not S4's.

## ~~POLISH — GTK draws some button labels and the header title bold and wide, unlike the Plex Sans the lane installs (the maintainer, from the S4 review captures, 2026-09-09)~~ CLOSED 2026-09-09, one half fixed and one half answered: MEASURED FIRST (GTK 4.18.6, libadwaita 1.7.6) — the fixture supplies all sixteen Plex Sans faces (`fc-match 'IBM Plex Sans:style=Bold'` -> Bold), nothing is synthesized, and kaya applies no bold class: a plain label resolves `IBM Plex Sans 14.667px` while EVERY GtkButton label resolves Bold (bare, `.flat`, `.suggested-action` alike — Adwaita's own stylesheet) and the header title reads Bold off libadwaita's `.title`. The title STANDS (bold is libadwaita's header-bar idiom and the mac's title is bold too — chrome following its platform); the `plain` role does NOT — the low-emphasis rung (docs/tasks-plan.md §4 R6) sat at the same bold as the other two roles, so it wears libadwaita's own `.body` typography class now (400 at 1rem; measured), removed with the others on a re-applied role; SwiftUI and WinUI were already regular and Compose Medium, each its platform's low rung, nothing on the wire moved; `Add`, a roleless button, stays bold as Adwaita draws it. No observable reads a weight, so the guard is two watched census entries in tools/check-gtk.py (the role constant, and the class NAME, since a class GTK does not know is inert). AMENDED THE SAME DAY BY THE MAINTAINER'S RULING: kaya on GTK matches kaya's OTHER backends, not GNOME's Files/Settings weights — one kaya-owned sheet (`WEIGHT_CSS` in gtk.rs, application priority: `button, button label { font-weight: normal }` and `headerbar label.title, windowtitle label.title { font-weight: normal }`) takes every button (bare, flat, suggested, destructive) and the header title to regular, measured through the resolved Pango descriptions with `.heading` still Bold, `.caption`, `.monospace` and `.title-1` untouched; the `.body` class on the plain role is retired into it; three watched census entries replace the two (a selector nothing wears, a one-word slip onto an already-regular label, the sheet at FALLBACK priority — each a rule that still loads and does nothing), check-gtk 49/49, nine legs green on both regimes. ROUND THREE THE SAME EVENING: the maintainer moved buttons and the heading role to Material's Medium (500) with the title SemiBold, the first app-links matrix then killed all sixteen typeface legs — Sora's fvar has no 500 named instance and fontconfig matches instances, not the axis, so Pango ABORTS (docs/traps.md) — and the fix is a clamp in gtk.rs rather than a font edit the OFL forbids: the sheet holds wishes and lowers each onto the brand font's nearest named instance (ties to the heavier; 500 -> 600 on Sora, 500 stays on Plex Sans), re-lowered on every SetTypeface; check-assets C12 holds the wishes, the clamp and every literal weight against every vendored variable font (seven watched negatives, swept), check-gtk's container half runs `gtk::weight_tests` over the real bytes; sixteen typeface legs green again. KEY: WEIGHT_WISHES, wght_named_instances, nearest_weight, weight_css_for, check-assets C12, weight_tests

KEY: GTK bold labels, Details button weight, header title weight, Plex Sans fixture, font-family root rule, synthesized bold
- In the Linux captures the row buttons ("Details") and the header bar's title ("tasks") render in a bold, wide face, while the mac, Windows and the phones draw the same labels regular. The lane's system-font fixture is IBM Plex Sans (tools/linux/Dockerfile, the whole family) and gtk.rs installs one `:root { font-family }` rule, so two candidates, to be told apart by reading the widgets' resolved Pango font description: libadwaita's own `.title` (bold by design) and a bold label class on the link-shaped button that the other backends do not apply, or a bold weight the fixture cannot supply (a synthesized bold reads wide). Either way the other four backends draw those labels at the regular weight, so the GTK arm should too, or the divergence is stated. Not S4's; the captures only made it visible.

## WATCH — the windows portfolio_python leg re-sighted `band.UpdateLayout: (0x88000FA8)` once after five green matrices (2026-09-10)

KEY: 0x88000FA8, band.UpdateLayout, row window report, portfolio_python, IsLoaded gate, table_report_once
...
## ~~DEFER — the screen raster's threads: vello_cpu multithreaded above a megapixel, on a reused context (2026-09-10)~~ CLOSED 2026-09-11, BUILT (`Context::Screen` in crates/kaya/src/canvas.rs: a context kept per thread and resized in place, `screen_threads(pixels)` above `SCREEN_THREADS_ABOVE`; check-canvas-blit clause 6 holds the rule with a sixth negative; the iPhone record is the reason it matters — 19.5ms on one thread against 9.5 on five for a heavy frame)
...
KEY: web links, universal links, App Links, appUriHandler, apple-app-site-association, assetlinks.json, windows-app-web-link, associated domains, autoVerify, [links] hosts
- The app links slice (f94353a5) generates every platform's web-link registration from `[links] hosts` — the associated-domains entitlement on Apple, the `autoVerify` intent filter on Android, `windows.appUriHandler` in the MSIX manifest — and NO lane drives one, since each platform verifies the claim against a file the DOMAIN must serve over HTTPS: Apple's `/.well-known/apple-app-site-association` (the team id + bundle id, and the paths), Android's `/.well-known/assetlinks.json` (the package name + the signing certificate's SHA-256), Windows' `/.well-known/windows-app-web-link` (the package family name). Unverified, every platform opens the browser instead, silently. The maintainer HAS a domain and serving the three files is work on his side (2026-09-10); until then the generated declarations are checked by shape (check-app-identity's plist clause, check-jni's manifest census, check-staging's manifest clause) and this entry says so. WHAT A MEASUREMENT NEEDS, once the files are up: the tasks guest declaring the host, a signed mac bundle with the entitlement (ad-hoc signing does not carry associated domains — a real team identity does), `open https://<host>/tasks/t1` on the mac and `simctl openurl` with the https URL on iOS, `pm verify-app-links --re-verify` then `am start -a VIEW -d https://…` on the emulator (Android 12+ can force the state without the network for a probe), and on Windows the packaged app with the handler declared; each measured ONCE by hand and recorded here, since no lane can host the files. The custom-scheme half is what every lane drives.

## ~~BUILD — app links: a URL opens the app on the thing it names~~ SHIPPED (rulings TAKEN 2026-09-09, docs/app-links-plan.md L1-L6 with L1 and L2 amended by the maintainer; built the same day on five agents and SHIPPED: matrix #2 on the frozen tree green on every leg of every lane — mac 440, linux 765, windows 277, iOS 134, android 143, 1,759 legs, the 58-gate sweep green — with the mac and iOS lanes seconds over ceilings moved with their records, and matrix #3 after the mac window fix ALL PASS on all five lanes and 58 gates, 1,759 legs in 1104s; matrix #1 had found the GTK weight abort on the typeface legs, fixed by the clamp; the review page https://claude.ai/code/artifact/e6495cf6-00cb-4fc3-8e8f-4505ad88e880)

KEY: app links, link route, declare_link_route, link_opened, links.rs, links::opened, open_link, relaunch link, links.steps, sole claimant, links_startup, links_declared, Identity.scheme, kayaLinkScheme, link-leg.py, relaunch-link.ps1, route 0, on_link_miss
- THE SURFACE, as the maintainer asked for it: the app declares a route on the handler it already has (`Messages::link("task/{key}", |params| Msg)` in Rust, the same registrar shape beside `on_notification_activation` in the other eight), the core matches every URL once and hands the handler its captures, and the scheme defaults to the declared id so the manifest needs nothing new (`[links] scheme` overrides, `[links] hosts` declares web links, generated and not driven). The pattern rides `declare_link_route`; a malformed or repeated one faults at apply in crates/kaya/src/links.rs with the whole sentence, and check-sugar-surface freezes that `kaya: link route` is spelled in exactly one file (858 sources read). ROUTE 0 HAS NO REGISTRAR: a URL no route took is announced by the core naming the patterns it tried and every binding is silent on it. THE CATCH-ALL CAME THE NEXT MORNING (2026-09-10, the maintainer's question "what does a registrar for unmatched links entail?"): `*` as the whole pattern is a route matched LAST whatever its declaration order, its one param `url` the URL as delivered — through the `link` every binding already has, no new surface; `*` inside a path and a second `*` refused at the declaration; the tasks app lands one on Inbox with `no route for <url>` and links.steps asserts it on all five lanes. THE DOORS, from the measurements (docs/traps.md, the 2026-09-09 app-links entries): macOS is TWO doors — the delegate's `application(_:open:)` for a URL launch (a raw Apple-event handler installed early swallows the launch and no window ever opens, measured three ways) and the raw kAEGetURL handler installed in didFinishLaunching for a running app; iOS `onOpenURL` with the simulator's one-time approval answered by the lane's driver and the leg's bundle made the SOLE claimant on its device; Android the single-task activity's intent on both `onCreate` and `onNewIntent` (`setIntent` first, consumed once) with the KAYA_* env mapping moved so a warm start still maps it, both doors narrowed by package; Linux GApplication's `open` with `HANDLES_OPEN` over D-Bus (`gio open`, never xdg-open; DBusActivatable AND the service file, refused apart) into the leg's own XDG home; Windows the App SDK's protocol activation with `FindOrRegisterForKey` and the redirect, where AN APP THAT DECLARES NO ROUTE CLAIMS NO SCHEME AND TAKES NO KEY (`links_startup()` reads only the activation kind, `links_declared()` claims on the first route) — the narrowing of L3 the pooled lane forced, recorded for the maintainer: every guest declares one id and the legs sharing python.exe, node.exe, java.exe and dotnet.exe would otherwise redirect-and-exit at startup. The one thing every lane had to meet: a scheme claimed by more than one installed app resolves silently to a chooser or a pick (Android measured it with two probe APKs, iOS with the lane's own bundles, Windows with the App SDK's per-executable key), so each lane targets its own guest and verifies the process the door started. THE HARNESS: `open_link "<url>"` through the platform's own open from inside the process (an ACTION verb, quiet-wait and answer-wait in all three runners), `relaunch link "<url>"` the cold door on every lane (`RELAUNCH_DOOR["links"] = "link"`), tools/scenes/links.steps on the tasks guest — a warm open on t2's details, back, a section link, an unknown key answered by Inbox's caption, then the relaunch on t1 reading its details and reference. THE TASKS APP routes `task/{key}` and `{section}`. GUARDS BY LANE: check-app-identity C14 (the plists' scheme is the manifest's rule) and a check-steps clause over the iOS door (sole claimant, SpringBoard answered, the door pushed again after the tap); check-jni's nine-negative host census plus a reader-agreement clause holding gradle's copy of the scheme rule equal to identity.py's, `apk_link_verify` on the compiled manifest at the assemble funnel, a host-JVM one-shot test; check-gtk's entries for the flag, the signal, the verb arm and the door plus link-leg.py's eleven-clause self-test and linux.py's four activation refusals; winui::tests 31 on the guest (the dotted registration read back off a real registry with the unregister leftover pinned, the claim ORDER held by a source-reading test) and check-staging's two clauses with N2d (the lane's MSIX claims no scheme, since a packaged claim beats an unpackaged one); check-sugar-surface's six-part clause in nine, fifteen runtime checks in each of the python and JS suites, the six runnable suites' `link-route: OK`, a check-abort negative cutting the route-0 guard; and `wire::tests::a_link_body_reads_back_at_the_offsets_the_bindings_use` — the generator's check decodes nothing, and the new occurrence's reader branch was missing in all nine until a hand-packed record died (docs/traps.md). FOUND AFTER THE MATRIX BY THE CAPTURE, NOT THE HARNESS: a macOS URL launch registered the window and never composited it — every harness read answered off the model and act two published OK with zero windows for the process's life (docs/traps.md); the register path orders the window on screen one main-queue turn later now (the delegate proxy that vouched for a weakly held original crashed on the first ordering and holds it strongly), the mac link door refuses a verdict unless the relaunched pid owns a layer-0 window (watched: the scene green and the guard red under `orderOut`), and act two opens with `settle 800` because the window composites at 0.52 s. OPEN: web links unmeasured for want of a served domain (the entitlement, the autoVerify filter and windows.appUriHandler generated by shape); Windows' `RegisterForProtocolActivation` writes the exe path unquoted, pinned by a test and not worked around; the mac lane's protocol registration is left on the machine as the AUMID key is.

## ~~BUILD — S4: what survives a relaunch~~ SHIPPED (rulings TAKEN 2026-09-09; BUILT the same day on five agents and SHIPPED: matrix #2 on the frozen tree green on every leg of every lane — mac 439, linux 763, windows 276, iOS 133, android 142, 1,753 legs, the 57-gate sweep green — with the windows lane 39s over a ceiling that had one second of headroom, moved to 1200 with its record; the review page https://claude.ai/code/artifact/1e2411f6-ac7a-4dbc-a733-3fdda408afda)

KEY: S4, app_data_dir, prefs, preferences store, remember_frame, window memory, taskspersist, relaunch launch, expect_pref, selftest domain, rusqlite
- THE RULINGS (docs/tasks-s4-plan.md P1-P7, §4): the app's data is a SQLite database under `app_data_dir()` through the language's own binding — the maintainer's question ("isn't the standard way for app data stores to work like sqlite?") moved P1 off a guest-owned text document — and kaya answers the directory alone; settings are a typed `prefs()` store on the platform's own backing (UserDefaults, SharedPreferences, a keyfile on the two desktops); window memory on the desktops under the reserved `kaya.window.<id>.frame` keys with `remember_frame(false)` the opt-out; the plain door `relaunch launch`; recording mode films act two; the tasks app on rusqlite with the engine compiled in, its settings and appearance as prefs. Under the harness both stores are scratch (`<id>.selftest`, `<state>/selftest/<id>/data`), emptied by act one and kept by act two. The scene is tools/scenes/taskspersist.steps.
- BUILT 2026-09-09 ON FIVE AGENTS, every lane green by hand before the freeze. CORE: the ten floor functions over crates/kaya/src/prefs.rs with four backings (the keyfile for Linux and Windows; NSUserDefaults reached from the core through objc2 on macOS and iOS; SharedPreferences through JNI on Android), `kaya::prefs()` / `kaya::app_data_dir()` / `remember_frame`, `expect_pref` / `expect_no_pref` / `relaunch launch` in harness.rs and the SwiftUI interpreter, the tasks app on rusqlite (a dev-dependency, so libkaya carries no engine anywhere), tools/scenes/taskspersist.steps, recording mode filming both acts, 554 unit tests. BINDINGS: `app_data_dir` and the nine-method prefs handle in all eight, do/DO everywhere, three byte-frozen refusal sentences compared flattened in nine (reserved key, empty key, no directory — the last RULED uniform after a first draft had Rust panicking where eight answered absent), 40 runtime checks in the python and JS suites, and the S9 open item closed (runnable notify-order proofs in C#, Java, Swift, OCaml and Haskell, baked into the sweep from generated projects over copies). ANDROID: KayaPrefs.kt over `getAll()` so no typed getter meets a key of unknown type (an f64 is a one-element Set<String> of Rust's own Display text — putFloat is 32-bit), `commit()` never `apply()`, `KAYA_PREF_DOMAIN` exported at attach so the interpreter spells no domain rule, the plain door as force-stop + `am start` with no extras, a two-block phone drop, and three guards (`check_real_prefs` refusing the app's REAL preferences domain on any device after every suite; check-jni's class census derived from the tree; KayaPrefsTest on the host JVM). LINUX: GTK window memory (size alone — GTK4 has no position), tools/linux/persist-leg.py pushing `gio launch` on the app's own desktop entry, both regimes green, 42 census entries in check-gtk with a frame unit test in the container. WINDOWS: WinUI window memory through `AppWindow.Changed` and `MoveAndResize` clamped to a DisplayArea, the plain door through tools/guest/relaunch-launch.ps1, rusqlite under cargo-xwin after `llvm-lib` joined the shell, the sampler LEAK fixed at its cause, AND a months-old false green found: tools/deploy-win.py had DISABLED the notification platform on every deploy (the two toast globals at 0 for the foreground trap) and the platform reads them lazily, so the legs passed until the session re-read — delivery is on now, the BANNER is refused per AUMID (`ShowBanner=0`, `ShowInActionCenter=1`), the user notification service is restarted before any leg, and `notify_ready()` asks the PLATFORM (a toast posted and read back from history) before the run. THE FIRST MATRIX THEN FOUND THE POOL RACE (docs/traps.md 2026-09-09: one scratch tree per app under many concurrent legs; the mac's taskspersist read `attempt to write a readonly database` when a neighbour's act one emptied it) and every desktop lane gives each leg its own state home now — the mac's leg_env, the windows launchers' `XDG_STATE_HOME=C:\kaya\legs\<leg>\state` above their KAYA_SELFTEST line (274 launchers, 274 distinct homes, held by check-staging with a 200-launcher floor and three watched negatives; the com door LENDS the variable through the user environment for one activation because a LocalServer32 takes the session's block), the linux leg scripts as before — while the core's preference domain carries a tag derived from the home. KEY: prefs.rs, prefs_keyfile.rs, prefs_android.rs, KayaPrefs.kt, KAYA_PREF_DOMAIN, persist-leg.py, relaunch-launch.ps1, notify-ready.ps1, notify_banner_off, frame_memory, kayaMemoryHoldsSize.
- OPEN, ON THE RECORD (2026-09-09): recording mode's extracted stills are BLACK on this host for the two-act scene — the crop rect and the times are right (a hand extraction at the same rect shows act two's relaunched window), but the film is sparse VFR and harness-extract takes the COVERING frame, so a step before the app's next frame gets the gap; act one is black too, so it predates this slice. Worth its own session. KEY: covering frame, harness-extract, sparse VFR, KAYA_WIN_SLOT. ~~AND the SwiftUI side of the window-memory by-value rule has no unit test where GTK and WinUI each have one.~~ CLOSED 2026-09-09: `tools/check-window-memory.py` is the macOS third of that rule — check-pane-ladder's compiled-probe shape one file over, the interpreter's own `// MARK: - Window memory` block cut out by the gate and compiled with tools/checks/swiftui-window-memory.swift (nothing factored out, no second copy to drift), driving the six by-value cases, both frame spellings and six malformed lines, the clamp against a real NSScreen, the save's coalescing and dedup, the restore and the opt-out; 19 watched negatives, counts printed; the static clause holds the save to `DispatchQueue.main.async` and the restore to the accessor's register(). KEY: check-window-memory, kayaMemoryHoldsSize, window memory.

## ~~BUILD — S9: the closed-app click on every platform~~ SHIPPED (rulings TAKEN 2026-09-08; built the same day and night on five agents, every lane green through both acts — mac 438, iOS 42 of the rust suite, linux 761, android 141, windows 275; the plain matrix on the frozen tree is the line below)
...
- THE ANDROID DOOR IS BUILT AND MEASURED (2026-09-08): the Compose harness carries R6's second act — `kayaStatements` flattens the script once, the `"relaunch" ->` arm takes everything after it as act two, writes `<KAYA_ACT2_DIR>/marker` (the core's path: crates/kaya/src/act2.rs computes `<state>/act2/<id>` at attach, on Android out of `context.filesDir`, which `Kaya.attach(activity, stateRoot)` and `KayaRing.attach(activity, stateRoot)` now take), prints act one's own verdict `KAYA_SELFTEST: ACT 1 OK (…)` and the door line `KAYA_RELAUNCH: door notify_tap notification=<id> title=<t>`, and leaves; the process the shade tap starts consumes the marker in the core BEFORE the app thread exists, runs the remaining steps and writes the ordinary verdict to `KAYA_ACT2_VERDICT` as well as logcat, and tools/android/run-emulator.py joins the two as ONE leg (act one's line, `pidof` empty, the tap through the existing `notify_tap` machinery, `act2.verdict` read back through `run-as`). THE COLD ACTIVATION NEEDED NO NEW CODE: the relaunched process (a new pid) logs `KAYA_NOTIFICATION_ACTIVATED` at its mount 411ms before the harness's first step, so R3's queue delivers it ahead of the app thread's first read. The arm refuses unless the platform holds EXACTLY ONE delivered kaya notification, naming the ids and titles it saw; the lane refuses a two-act scene whose `RELAUNCH_DOOR` entry is missing or names another door, and a door named for a scene with no `relaunch` (which fired on its first run). MEASURED: `am force-stop` cancels the app's notifications while a clean exit keeps them, and `run-as … cat` of a missing file exits 0 with cat's error on the stream (docs/traps.md, both). Android lane ALL PASS, 141 legs, `tasks-compose: two acts — ACT 1 OK + KAYA_SELFTEST: OK (entries 1, href "https://example.com/tasks/t1")` in 20-23s. KEY: relaunch, act2, KAYA_ACT2_DIR, KAYA_ACT2_VERDICT, ACT 1, KAYA_RELAUNCH, RELAUNCH_DOOR, relaunch_count, stateRoot.
- THE LINUX DOOR (2026-09-08): the closed-app click works on both routes and through ONE call. §2.2's measurement corrected R4's premise — S3 read `Activate` alone not because kaya listened on the broadcast but because kaya's portal arm spelled the default action BARE; with the app's own namespace, xdg-desktop-portal-gtk calls `org.freedesktop.Application.ActivateAction("notify-activated", [<'kaya-<id>'>], {})` on the app's bus name, D-Bus activation starts the app through the service file the desktop entry's arm installs, and the relaunched process's `notify-activated` GAction hands the core the occurrence on its own OccSink (created before the app thread, so it is queued and delivered at the guest's first read). GNOME's `org.gtk.Notifications` route sends the identical call, so one door serves both. The GAction registration moved ahead of the main loop. R5 has a state on Linux now and it is measured, not argued: the ruled floor refuses every route that cannot relaunch, so the only silent drop left is an app installed with the desktop entry and no D-Bus service file — `notification_route()` reads `org.freedesktop.DBus.ListActivatableNames` and the post prints `KAYA_DIAG notification <id> posted with no relaunch door: a tap after exit lands nowhere — <id> is not among this session bus's activatable names …`, watched printing against a doctored copy of the leg. THE RUNNER: tools/linux/act2.py drives the second act from inside tools/linux/notify-leg.sh — the recorder's `Invoke` is the tap, nothing here starts a process — and polls `act2.verdict` beside the marker; notify-leg.sh gives each leg its own `XDG_STATE_HOME` (the marker path is one per APP and the two protocols pool concurrently), strips KAYA_SELFTEST from the activation environment (an activated service inherits the bus daemon's environ and act2.rs adopts a marker only when it is unset), and puts tools/linux/act2-exec.sh in front of the leg's own launcher so the relaunched process's output survives — a D-Bus-activated process's stdio is the daemon's, and dbus-launch detaches it. `RELAUNCH_DOOR_TASKS=portal-action` names the door in tools/linux/run-suites.sh. Act two answers 0.2s after the click on both protocols. FOUND ON THE WAY: the Exec line's ARGUMENTS are resolved by whoever starts the app (docs/traps.md), which the first full lane caught and act2.py's own sentence named. KEY: act2.py, act2-exec.sh, RELAUNCH_DOOR_TASKS, notify-activated, ActivateAction, app.-prefixed default action, ListActivatableNames, XDG_STATE_HOME per leg, KAYA_ACT2_LOG.
- S9 CORE + HARNESS + APPLE (2026-09-08). The closed-app click continues a scene in the process the platform starts. `Messages::on_notification_activation` is the process-level registration beside the one-shot `on_notification` (R1): the per-id handler wins, then this one, then an announced drop — `kaya: notification {id} outcome {outcome} reached no handler — none was bound at the show and no process-level handler is registered (Messages::on_notification_activation)`, one stderr write, the same sentence in all nine bindings with each registrar's own name in the parentheses. A notification result emitted before any consumer exists waits in `EARLY_NOTIFICATIONS` and is the first thing an mpsc sink reads (R3, capi.rs, one unit test); single delivery is structural, since a process that installs a sink never reads the ring and a process that reads the ring never installs a sink. The tasks guest derives a reminder's notification id from the task's key (R2), so the counter and its map are gone and a re-post replaces by construction. THE SECOND ACT (R6/R6a) is a `relaunch` line: act one ends there, leaves `<state>/act2/<id>/marker` (line 1 the scene, the rest the steps) and publishes `KAYA_SELFTEST: ACT 1 OK (…)`; the runner pushes the platform's door; the second process is adopted by crates/kaya/src/act2.rs BEFORE the app thread exists — the core alone consumes the marker and hands the path over in KAYA_ACT2_DIR / KAYA_ACT2_VERDICT, so no interpreter spells `<state>/act2/<id>`. The mac and iOS door is the carve-out, `KAYA_LAUNCH_NOTIFICATION` into the centre delegate's own funnel; the OS's own relaunch is MEASURED once (below) and not driven by a leg. Guards: tools/check-steps.py's second-act block (shape, doors, one path; seven watched negatives) and check-verbs' verb census. KEY: relaunch, act2, ACT 1 OK, KAYA_ACT2_DIR, KAYA_ACT2_VERDICT, on_notification_activation, EARLY_NOTIFICATIONS, RELAUNCH_DOOR, notification_id, state root. OPEN, ON THE RECORD: recording mode films act one only (validate-mac's run_recorded judges a leg by the guest's exit and returns before the join, so a KAYA_RECORD run of a relaunch scene gets no tile for the second process; the matrix never sets it); a FOREIGN guest cannot run act two, since the adoption sets KAYA_SELFTEST in-process before the app thread while a python or java guest picks its scene in its own main before kaya_run — tasks is Rust by design on every lane, so nothing is blocked, and a second relaunch scene in another language would consult the marker itself; and the act-one/act-two JOIN is per runner, held by the legs rather than a gate.
- WINDOWS: DONE (2026-09-08). The closed-app click goes through the COM activator the maintainer ruled. crates/kaya/src/winui/mod.rs implements `INotificationActivationCallback` and an `IClassFactory` for it, registers the class object with `CoRegisterClassObject(CLSCTX_LOCAL_SERVER, REGCLS_MULTIPLEUSE)` on the notification apartment at launch in every process, and — unpackaged — writes `HKCU\Software\Classes\CLSID\{…}\LocalServer32` = `"<exe>" -ToastActivated` beside the identity key's new `CustomActivator` value; packaged, tools/lib/packaging/windows.py declares the same class in the manifest's `windows.toastNotificationActivation` and `windows.comServer` extensions, both of which are required and neither of which works alone. THE CLASS ID IS DERIVED, NEVER TYPED: RFC 4122 version 5 (SHA-1 through the platform's own BCryptHash, so no crate joins the tree) in the DNS namespace over the AUMID the process posts under — the declared id unpackaged, `<id>!<Application Id>` packaged, because one MSIX carries N entry points while a CLSID names ONE server AND because HKCU\Software\Classes would otherwise shadow the package's own registration. `Activate` parses the toast's `kaya=<id>` launch string with the shipped parser and enters the backend's one activation funnel, whose pending queue already covers an activation that arrives before the app thread exists. The runner's door is `CoCreateInstance` of that class id from the console session (tools/guest/ relaunch-com.ps1, driven through schtasks), and tools/deploy-win.py joins act one's `ACT 1 OK` with the `act2.verdict` act two writes into ONE leg line; tasks_rust and taskspkg_rust both run both acts. R5's sentence is the Windows arm's alone and prints only on a real miss, watched printing under a withheld registration. KEY: activator_clsid, INotificationActivationCallback, CustomActivator, LocalServer32, toastNotificationActivation, relaunch-com, RELAUNCH_DOOR, second_act, relaunch_door_why_not. deploy-win ALL PASS, 275 legs, suites 563s; tasks_rust act two in 475ms, taskspkg_rust in 865ms. OPEN, seen in the captures and not S9's: the Windows Details screen shows NO task title where the other four backends draw one — reached the ordinary way it is identical, so it is pre-existing WinUI rendering of a pushed entry's title (KEY: pushed entry title, WinUI caption, Details title).
- THE SUGAR IS IN ALL NINE (2026-09-08): `on_notification_activation` / `onNotificationActivation` / `OnNotificationActivation` registers the PROCESS-LEVEL notification handler in each binding's own app-level idiom (Python and JS module-level functions, Go/C#/Java/Swift an app registrar beside the click ones, OCaml a labelled function taking the app, Haskell an App action, Rust `Messages`), and it receives the ID as well as the outcome because a relaunched process has no registration to correlate an id with; JS gets NO promise twin though its alert family has one, since a promise settles once and this handler does not retire. THE ORDER IS THE SEMANTICS and no lane can see it — the per-id one-shot handler first, retiring; else the process-level one, which does not; else the drop announced naming the id — because a scene registers one or the other, never both, and reads the same label back whichever answered; tools/check-sugar-surface.py reads all three positions out of each binding's own arm and its negative SWAPS a pair rather than deleting one, nine times. Go's decision moved into `App.notificationResult`, since `Serve`'s occurrence switch has no seam a test can reach (the ring is C memory through cgo), and the gate holds the switch arm to calling it. FOUND BY THE GATE ON ITS FIRST RUN: the core rendered the outcome as a WORD (`activated`/`refused`) and the eight bindings printed the raw NUMBER — one sentence, two readings, invisible to every lane — so all nine are compared FLATTENED now, 18 watched negatives on top of the 9 order swaps. Three runtime cases in the suites a gate actually runs (Python, JS, Go): a one-shot handler wins, an unknown id reaches the process handler, the process handler does not retire, plus the announced drop compared in full. ~~OPEN: C#, Java, Swift, OCaml and Haskell have call-site probes and known-red canaries only; their runnable three-case proofs belong beside the exercisers under tools/checks and guests/.~~ CLOSED 2026-09-09 (the S4 bindings sweep): all five have runnable four-case proofs — tools/checks/java-notify, tools/checks/swift-notify, guests/csharp/NotifyOrderCheck.cs, bindings/ocaml/checks/notify_order_check.ml, guests/haskell/NotifyOrderCheck.hs — run by tools/java-typecheck.py and tools/check-abort.py, each echoing its own verdict; the five bindings' decisions moved into methods (Go's precedent) so a test can reach them, and check-sugar-surface's ARM_CALLS table holds all six arms to calling them. KEY: notificationResult, NotificationResult, notification_result, ARM_CALLS, notify-order, NotifyOrderCheck. KEY: on_notification_activation, onNotificationActivation, OnNotificationActivation, notificationResult, notification_dropped, NOTIFICATION_ORDER, NOTIFICATION_SPELLINGS.
...
## ~~LEAK — the windows lane's flight-recorder sampler outlives the lane (measured twice 2026-09-08)~~ FIXED AT THE CAUSE 2026-09-09 (the S4 Windows agent): the recorder's own cleanup line was the repo's cmd-precedence trap — `if not exist C:\kaya\flightrec mkdir … & echo stop > …\ALL.stop` ran the echo INSIDE the if, and the directory always exists, so the stop file was NEVER written; the line is split, the lane refuses a verdict while a sampler of its run (or one nobody can attribute) is alive on the guest, watched with a decoy, and check-steps' cmd-precedence clause now reads `if not exist` and the recorder's `self._ssh` too (it had matched `if exist` in deploy-win's must_ssh alone)

KEY: flightrec.ps1, -Mode sample, ALL.stop, sampler leak, windows lane exit path
- tools/guest/flightrec.ps1 runs as a `-Mode sample -Leg lane` poller for the whole lane and is meant to stop when the runner drops `C:\kaya\flightrec\ALL.stop` on its exit path (the next run deletes the file at startup). Measured: the packaging agent reaped FIVE of its own after hand runs, and the plain matrix of 2026-09-08 21:34 left one polling (pid 22144, started 21:34:53) after its windows lane had finished at 04:51:23Z with every leg PASS — stopped by hand with the stop file an hour later. So the exit path does not reliably reach the sampler: either the lane's exit skips the drop on some route (a verdict path that returns before it), or a run that starts as another ends deletes the stop file before the sampler polls it. Remedy: the runner WAITS for the sampler's exit (tasklist by pid until absent, with a ceiling and a sentence) rather than dropping a file and leaving; and the matrix's cleanup proof lists the VM's powershell processes the way the agents' do. Not fixed in the packaging slice. SIGHTED AGAIN after S9's two plain matrices (2026-09-09): two `flightrec.ps1 -Mode sample -Leg lane` samplers alive on the VM, one per matrix, with the windows lane green both times; killed by pid (the stop file's directory was present, so the exit path is what does not reach them). Three matrices, three leaks: the remedy above is due before the next slice's matrix. RESOLVED 2026-09-09: the cause was never the exit path's ROUTE but its SPELLING — measured on the VM by running the exact cleanup string with the directory present (no ALL.stop) and the split spelling with it present and absent (ALL.stop both times). Two more findings on the proof: tools/guest/flightrec.cmd forwarded `%1 %2` only, so the run token never reached the script (it forwards `%3` as -Run now); and the runner starts the sampler BEFORE it ships the artifacts, so a lane's first run after an edit to flightrec.ps1 runs the PREVIOUS script with no pid file — the census therefore has TWO nets (pid files, which alone carry run identity, and the live process list filtered to a powershell naming `flightrec.ps1 -Mode sample`), unioned by pid, and the lane refuses on its own token AND on an unattributed one. KEY: lane_end, -Mode list, notify-ready, seen=pidfile+process.

## ~~BUILD — packaging part 1: a complete app from one manifest~~ SHIPPED 84e2f3c0 (2026-09-08; matrix ALL PASS 1,747 legs; the review page https://claude.ai/code/artifact/67f6fb6d-170b-416b-8d5c-5a2831e3092d) (rulings TAKEN 2026-09-08, docs/packaging-plan.md P1-P7; the shared core and four arms green the same day)
...
- THE SHARED CORE AND FOUR ARMS (2026-09-08): one manifest reader (tools/lib/packaging/identity.py) replaced five — tools/lib/lanes/mac.py's `app_identity`, tools/ios/run-sim.py's and tools/android/run-emulator.py's module-level reads, tools/linux/notify-leg.sh's inline `tomllib`, tools/linux/identity-class-leg.py's and tools/linux/run-suites.sh's — and check-app-identity's C9 refuses a sixth. The mark is a 1024x1024 SOURCE now and every platform size is DOWNSAMPLED from the file the manifest names by mark.py's box filter, which knows nothing about the picture and refuses to enlarge one; `render_png` survives only to regenerate the example asset (tools/regen-mark.py) and C11 refuses any other caller, holding the committed source byte-identical to the description's picture. The mac lane's bundle wrapper became the mac arm (P4, `stage_rust` calls it), the notify leg's hand-written desktop entry became the Linux arm's (P5, through tools/linux/install-desktop.py), the iOS bundle's icon family and launch picture and the APK's five density mipmaps became the phone arms (P6), and `tools/package.py` is the command. The assets scene stopped depending on the declared icon's size (its image widget shows the scenery picture; its label reports the mark present). FOUND ON THE WAY: a 1024 texture is dropped by X11's `_NET_WM_ICON` with no error (the GTK arm caps at 128; docs/traps.md), the wayland witness parses the lowering sentence (the phrase lives in tools/lib/identity_phrases.py under C13), AAPT2 stamps density qualifiers `-v4` and AGP's merge is incremental (docs/traps.md), and the iOS bundle's 1x icon entry must stay the declared file's BYTES because `kayaIOSAppIconWhyNot` compares bytes. Validated: check-app-identity with 27 watched negatives, the fast gates, validate-mac ALL PASS 438 legs, validate-linux ALL PASS 761 legs, both on a quiet tree; the iOS suite was stopped under a host load of 557 and is the matrix's.
- THE WINDOWS HALF (2026-09-08): a kaya app on Windows is two runtime situations and one declaration (docs/packaging-plan.md P3). tools/lib/packaging/windows.py writes an MSIX staging directory from guests/assets/identity.toml alone — one `<Identity>` carrying the declared reverse-DNS id, one `<Application>` per entry point with the declared name and launch background, the 44/150/50 tiles resampled from the declared 1024 icon and the 310x150 tile and 620x300 splash composed on the launch background, and the exe, kaya.dll, the App SDK bootstrap, an MRT index and the whole asset root beside each other — and tools/deploy-win.py's `package` phase packs, signs and installs it on the VM through tools/guest/pkg-install.ps1 (2s). Two legs run the Rust guests out of the installed package beside their unpackaged twins (`notifypkg_rust`, `taskspkg_rust`; `PACKAGED_LEGS` in tools/lib/lanes/win.py; the harness environment is set in the INNER `pkg_<leg>.cmd`, since `Invoke-CommandInDesktopPackage` carries no caller environment and grants no identity without `-PreventBreakaway`). UNPACKAGED, the library registers its own identity at startup — `HKCU\Software\Classes\AppUserModelId\<id>` with the declared DisplayName and an IconUri naming the mark written under LocalAppData, idempotently, read back by a guest-side unit test (28/28 on the guest) — and the unpackaged toast is ATTRIBUTED with nothing done by hand (the S3 page's finding, closed). PACKAGED, it writes nothing and takes the platform's AUMID, which forced packaged overloads of the three toast calls and an early return from the bootstrapper, since `MddBootstrapInitialize2` refuses a process with package identity (0x80070032) and the manifest's `<PackageDependency>` on the WindowsAppRuntime framework resolves it instead (its major.minor is `WASDK_MAJOR_MINOR` in winui/mod.rs, held equal by check-steps). MEASURED (docs/packaging-plan.md §3, docs/traps.md): the unsigned MSIX route does not exist on Win 11 26200 and Developer Mode does not open it, so the lane provisions ONE self-signed CN=kaya certificate into LocalMachine\TrustedPeople (pkg-install.ps1 mints and trusts it when absent; undo: remove the CN=kaya certs from TrustedPeople and CurrentUser\My); the install runs only in the console session (PLM); the installing shell's own "installed" toast steals the foreground and the toast toggle does not suppress it, so pkg-install.ps1 stops ShellExperienceHost after the install; the App SDK's notification stack is still Singleton-gated and packaging does not deploy one (the 2.x family has none), so S9's Windows closed-app click STAYS A RULING (a `windows.toastNotificationActivation` COM activator in the manifest on the classic route, or the App SDK's Main+Singleton deployed on every machine); a cross-built exe CAN carry an icon resource (`llvm-rc -no-preprocess` + `-Clink-arg`, the quadrant colours read back), docs/app-identity-plan.md's own "Windows before launch" row and not this slice's. THE TILE-SLOT RULE: the tasks scene drags real screen pixels and the tiling's slots 4 and 5 sit at y=786 on the VM's 800-tall screen, so a pointer-driving leg is never pooled — `POINTER_VERBS` read out of the scene scripts, the runner refuses to pool one, check-steps holds it, and `tasks_rust` (passing on submission order alone) moved to its own block. NEW GUARD CLASS: a guest .ps1/.cmd is read in Windows-1252, so an em dash inside a string closes it; check-steps refuses a non-ASCII byte outside a comment in tools/guest with the shipped defect as its negative. deploy-win ALL PASS, 275 legs, suites 535s. KEY: PACKAGED_LEGS, notifypkg_rust, taskspkg_rust, pkg-install.ps1, pkg-run.ps1, PreventBreakaway, WASDK_MAJOR_MINOR, POINTER_VERBS, unsigned namespace, ShellExperienceHost.

## ~~BUILD — task manager S2b: the app-wide appearance switch~~ SHIPPED dbc07ad6 (2026-09-07) (rulings TAKEN 2026-09-07; depth green on the mac the same afternoon)
...
- REVIEW POLISH (2026-09-07 evening, the maintainer's findings on the S2b page): GTK's sidebar pane tinted (`sidebar-pane`) with a separator to the content, and WinUI's pane at 240 instead of the 320 default (docs/tasks-s2b-plan.md §3).
...
## ~~BUILD — task manager S2: switches, the Today badge, a link in notes, the launch slot~~ SHIPPED b8be1a25 (2026-09-07) (rulings TAKEN 2026-09-07; depth green on the mac the same day)
...
## ~~WATCH — an explorer PopupHost held the VM's foreground and the `type` verb could not take it, once (first sighting 2026-09-15)~~ CLOSED 2026-09-18 after eight reds: the trigger is gone from the lane and the wait loses legibly — a notification leg never runs in the pool an exclusive funnel drains (tools/lib/lanes/win.py's NOTIFICATION_VERBS census, held by check-exclusive's clause 5), and the guest's wait dances from its FIRST turn with the AttachThreadInput route beside SetForegroundWindow, takes and prints the window's own readings and the database's, and ends the notification host at its bounded expiry, the only route measured taking the foreground back from that window (eleven driven against it; the kill is harness-only, guarded to the host's class AND title, and Windows restarts the host on demand); the forced state read FAIL (35s) twice before and PASS (32s) after, two windows lane runs ALL PASS at 282 legs, and check-harness-ceiling's clause C holds the dance inside the wait, both bounds, the reading and the attach-and-detach (eight watched negatives). THE CHARGE'S TWO PREMISES WERE MEASURED FALSE and are on the record beside the fix (the job's tmp/idiom/notes-phantom.md; docs/traps.md): the notification database cannot tell a phantom from a banner, because a CLEARED row is still in the file's bytes and a notify leg's own cleared toast reads as fresh two seconds later; and AttachThreadInput is REFUSED by that window (the call answers 0). STILL OPEN INSIDE THE CLOSE: the empty window's own Win32 readings have never been taken — six single-lever recipes never raised it and the one state that did (eight CPU spinners beside a stack of reminder banners) was a real banner — so the 30s is still spent when it is met, green at the end instead of red; the instrument that takes those readings is in the wait now, and the next sighting's bundle buys the discriminator that skips the wait. KEY: phantom window readings, cloaked, take_down_notification_host, PHANTOM_HORIZON_MS
