...
var _te = (typeof TextEncoder === "function") ? new TextEncoder() : null;
function utf8(s) {
...
// realistic kaya batch sizes: the portfolio refresh is 15,003 inserts
// (docs/portfolio-plan.md); an ordinary handler emits a few dozen records.
function timed(fn, arg, reps) {
  var best = Infinity, t, i;
  for (i = 0; i < reps; i++) { t = now(); fn(arg); t = now() - t; if (t < best) best = t; }
  return best;
}
// warm the tiers the way a real app does before the big batch
timed(packBatch, 200, 50);
var big = 15003, reps = 40;
var t = 0, i;
for (i = 0; i < reps; i++) { var s = now(); packBatch(big); t += now() - s; }
out("portfolio15003\t" + (t / reps).toFixed(2) + " ms/batch\t" + ((t / reps) * 1000 / big).toFixed(3) + " us/record");
var small = 20, reps2 = 20000, t2 = now();
for (i = 0; i < reps2; i++) packBatch(small);
t2 = now() - t2;
out("handler20\t" + (t2 * 1000 / reps2).toFixed(2) + " us/batch\t" + (t2 * 1000 / reps2 / small).toFixed(3) + " us/record");
