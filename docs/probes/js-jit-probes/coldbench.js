...
// cold vs warm: what a JIT costs you on the FIRST batch, and how fast it pays back
var t0 = now(); packBatch(15003); var cold = now() - t0;
var trace = [];
for (var k = 0; k < 12; k++) { var s = now(); packBatch(15003); trace.push(now() - s); }
out("cold15003\t" + cold + " ms");
out("warm-trace\t" + trace.join(" "));
