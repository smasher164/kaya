...
> `-L "$TARGET_DIR" -lkaya` (`run-sim.sh:1343`) makes ld64 prefer the dylib.
...
target/ios-bundles/<name>.app/
  ...
  <name>              # the guest executable, cp'd in (run-sim.sh:101)
  libkaya_swiftui.dylib   # cp'd to the BUNDLE ROOT, not Frameworks/ (run-sim.sh:1366, :1384)
```
...
`crates/kaya/include/kaya.h` (1693 lines, generated by `tools/gen-header.sh`)
...
| | iOS | Android |
|---|---|---|
| Script in | `SIMCTL_CHILD_KAYA_SELFTEST_SCRIPT` env (`run-sim.sh:1093`) | `am start --es KAYA_SELFTEST_SCRIPT` intent extra, then `Os.setenv` in onCreate (`run-emulator.sh:571`, `MainActivity.kt:18-25`) |
| Newlines | real newlines | **not possible** — scripts fold to `;`, "the intent-extra transport cannot carry newlines" (`run-emulator.sh:713-715`; grammar accepts `;`, `KayaCompose.kt:1709-1710`) |
| Verdict out | stdout, read via `simctl launch --console-pty`, matched by `grep -q "KAYA_SELFTEST: OK"` (`run-sim.sh:1092-1095, 1118`) | `Log.i("kaya", …)` → `adb logcat -s kaya:* -e 'KAYA_SELFTEST: (OK\|FAILED)' -m 1` (`KayaCompose.kt:4711`, `run-emulator.sh:595, 622`) |
...
| Path | What it is |
|---|---|
| `run-sim.sh` (1627 lines) | the whole lane: gates, dylib build, per-guest builds, bundle assembly, simulator pool, legs |
| `Info.plist.in` | the one bundle template; `@EXECUTABLE@`/`@BUNDLE_ID@`/`@NAME@` substituted in `make_bundle` (`run-sim.sh:95-100`) |
| `simdrive/{build.sh,main.swift}` | host-side driver for the document picker (built once per run, `run-sim.sh:83`) |
| `clipctl/{build.sh,main.swift}` | in-simulator clipboard reader, run under `simctl spawn` (`run-sim.sh:84-88`) |
...
with three things in it (`make_bundle`, `run-sim.sh:90-103`). The only
...
| Path | What it is |
|---|---|
| `run-emulator.sh` (1184 lines) | the lane: `cargo ndk` builds, jniLibs copy, `gradle assembleDebug`, `adb install`, `am start`, logcat verdict |
...
**iOS** — three insertion points, all inside `run-sim.sh`:
...
1. a build step producing a **Mach-O executable for `arm64-apple-ios-sim`**
   ...
   `run-sim.sh:1337-1346`) or carrying kaya internally (the Rust leg's shape,
   `:1381`);
...
- *native-guest shape* (Rust's): produce `lib<name>.so` for `arm64-v8a`
  ...
  → `run_apk` (`run-emulator.sh:741-760`). The `.so` must be self-sufficient:
...
- *JVM-guest shape* (Kotlin's): `cp target/aarch64-linux-android/debug/libkaya.so`
  ...
  (`run-emulator.sh:1043-1052`; `milestone2kt/build.gradle.kts:31-40`).
...
- **Every artifact carries a build id and is `--verify`'d before it runs.**
  `tools/build-id.sh --verify` is applied to `libkaya.a` (`run-sim.sh:1307-1308`),
  the SwiftUI dylib (`:1299-1300`), the copied `.so` (`run-emulator.sh:752`),
...
- **`jniLibs` is a checked-in directory that accumulates.** The APK measured
  ...
  (`run-emulator.sh:741-749`). `[M]` A second language adding its own `.so` to
...
| A2 | iOS: that executable **carries kaya inside it** — statically linking `libkaya.a`, or (Rust) the rlib. Linking `libkaya.dylib` is a **fail**: today's Swift leg does exactly that and produces a bundle that only runs in the Simulator (§1.2). Test it with `otool -L`: nothing but system dylibs and `libkaya_swiftui.dylib` may appear. | `[M]`; `Cargo.toml:9-12`; `run-sim.sh:1343` |
| A3 | iOS: the executable runs **unsigned in the Simulator** (the lane never signs anything). | `run-sim.sh:40-43` |
| A4 | **Android: can produce an `arm64-v8a` ELF shared object** exporting a JNI-callable entry, **or** run as JVM bytecode in the app's dex. | `run-emulator.sh:748-749, 1046-1047` |
| A5 | The artifact can be **stamped with a build id** and pass `tools/build-id.sh --verify`. | `run-sim.sh:1307`; `run-emulator.sh:752` |
...
| # | Requirement | Evidence |
|---|---|---|
| E1 | The bundle is **assembled by hand**: `Info.plist` from a template, the executable, and `libkaya_swiftui.dylib` at the bundle root. No Xcode project. | `run-sim.sh:90-103, 1363-1366` |
| E2 | `kaya::run`/`kaya_run` `dlopen`s the interpreter from `$KAYA_SWIFTUI_LIB`; the runner points it at the installed container. The guest does nothing here. | `swiftui_host.rs:233-247`; `run-sim.sh:1094` |
| E3 | Reads env vars set by `SIMCTL_CHILD_*` at launch. | `run-sim.sh:1092-1095` |
| E4 | Writes the interpreter's verdict to **stdout** and lets it `exit()` — `--console-pty` returns only when the process exits. | `run-sim.sh:1110-1111, 1118` |
| E5 | **One bundle per scene** (unlike Android): the guest need not switch on `KAYA_SELFTEST`, but the runner still sets it. | `run-sim.sh:1323, 1363-1372` |
