...
| I want to… | the route |
|---|---|
| compile Swift | `source tools/lib/swift-toolchain.sh` first — bare `swiftc` is a shim in the dev shell that says so and exits. THE BINDING IS A PACKAGE TARGET (Package.swift, Swift 6 language mode): build it with `kaya_swift build --disable-automatic-resolution --scratch-path target/swiftpm`, then compile a guest against it with `kaya_swift_guestc -I target/swiftpm/debug/Modules -I bindings/swift/CKaya <guest> target/swiftpm/debug/libKaya.a -L target/debug -lkaya`. For the simulator add `--triple arm64-apple-ios16.0-simulator -Xswiftc -sdk -Xswiftc <iPhoneSimulator SDK> -Xcc -isysroot -Xcc <same>` and read the output through `target/swiftpm-ios/debug/` — `--triple` alone picks the macOS sysroot and cannot load the standard library. The SwiftUI interpreter is NOT in the package: it keeps `-import-objc-header crates/kaya/include/kaya.h` (tools/swiftui/build-dylib.sh). Guests use `KayaApp.run { app in ... }` and `kaya_swift_guestc`, which selects Swift 6 through the app-thread executor. The interpreter keeps Swift 5 and uses `kaya_swiftc`; tools/check-pins.py holds the separate routes. See docs/measurements/swift-executor-2026-09-18.md |
| compile the Java tier by hand | `javac --release 21 -encoding UTF-8 -d <out> bindings/java-desktop/dev/kaya/KayaRing.java bindings/java/dev/kaya/*.java guests/java/dev/kaya/guests/*.java` — the dev shell's JDK is zulu 21, and `--release 21` is not optional: every javac in the repo states it and tools/check-pins.py refuses one that does not, because a bare javac compiles at whatever level the JDK on PATH defaults to. The Android half is one language level's worth narrower than the desktop's — ART has no `Thread.ofVirtual()` at any compileSdk — so a shared guest that compiles here can still fail `:javahost:compileDebugJavaWithJavac` |
...
| photograph a guest window on GTK | `tools/linux/shot-gtk.py <scene> <out.png> [--layout appmenu:close] [--hold-after N] [--guest tasks] [--env KAYA_LOCALE=ar-EG]` — `--guest` names the example when the scene runs another's (tasksrtl and tasksbig run tasks), `--env` hands the guest a knob (the compliance page's Arabic and 200% captures); checked build and core verification, then the scene's Rust example under Xvfb, held after step N, shot and cropped into per-run files; `<out.png>.log` retains build, capture and guest output even on failure. A failed build or capture leaves any previous requested image untouched, not newly validated. `--layout` selects desktop controls; without it, kaya's fallback (docs/gtk-chrome.md). View every capture before publication. |
...
| build one platform's app artifact by hand | `tools/package.py mac\|linux\|windows\|android\|ios [<exe>] [--out DIR]` (docs/packaging-plan.md): the arms are tools/lib/packaging/*.py, the manifest reader is identity.py, and `--install` on linux copies the staged XDG tree under ~/.local/share. |
| regenerate the example app's mark | `tools/regen-mark.py` (`--check` compares without writing), the ONE caller of mark.py's `render_png`; every packaging step resamples the committed 1024px file instead. |
| build the Windows package by hand | `tools/package.py windows <exe> --out DIR` writes the MSIX staging directory (tools/lib/packaging/windows.py); makeappx and signtool live only on the VM, so the pack, the signature and the install are `tools/guest/pkg-install.ps1`, started through `schtasks /it` (tools/deploy-win.py's `package` phase, run for `all` and for a packaged leg by name). The ssh session is elevated, but `Add-AppxPackage` never works there (docs/traps.md, the PLM entry). |
| run a packaged Windows leg | `tools/deploy-win.py akhil@192.168.64.2 notifypkg_rust` (or `taskspkg_rust`): the same guests out of the installed MSIX. Their launchers are a PAIR — `run_<leg>.cmd` is the lane's door, `pkg_<leg>.cmd` runs inside the package and sets the harness environment, since `Invoke-CommandInDesktopPackage` carries none of the caller's. |
| compile a Windows resource script | `nix shell nixpkgs#llvm -c llvm-rc -no-preprocess` — the bare `llvm-rc` runs the nix-wrapped clang as its preprocessor and dies on `-fPIC` for an MSVC target. |
| drive a two-act scene on a warm emulator by hand | the ordinary `am start … --es KAYA_SELFTEST <scene> --es KAYA_SELFTEST_SCRIPT '<steps with a bare relaunch in them>'`; once logcat carries `KAYA_SELFTEST: ACT 1 OK` and `pidof <pkg>` is empty, tap the row the app named in `KAYA_RELAUNCH: door notify_tap … title=<t>` (`cmd statusbar expand-notifications`, `uiautomator dump`, tap the row's centre, `cmd statusbar collapse`). NEVER `am force-stop` between the acts: it cancels the app's notifications and takes the door with it (docs/traps.md). |
| read a second act's artifacts on Android | `adb -s <serial> exec-out run-as <pkg> ls files/act2/<the manifest's id>` and `… cat files/act2/<id>/act2.verdict`. The cat exits 0 on a missing file with its own error on the stream, so read the content, never the code (docs/traps.md). |
| hand-run one notify or tasks leg with its second act on Linux | `docker run --rm -v <repo>:/work kaya-linux bash -c '…'` with a display and the scene dir; `tools/linux/notify-leg.sh portal [tools/linux/a11y-leg.sh] <guest>` does the rest and drives act two itself whenever act one left a marker (tools/linux/act2.py, the recorder's Invoke as the tap). |
| hand-run the taskspersist leg with its plain door on Linux | the same container shape, running `python3 tools/linux/persist-leg.py <guest>` under `KAYA_SELFTEST=taskspersist`: it stages the app's desktop entry into its own XDG home, runs act one, pushes `gio launch <entry>` and joins act two, printing it under `--- act two ---` as the notify leg does; `--self-test` watches its six refusals. |
| hand-run the links leg on a lane | mac: `tools/run-leg.py links rust --build` (the cold door is `open -a` on the leg's own bundle); linux: `python3 tools/linux/link-leg.py <guest>` in the container under `KAYA_SELFTEST=links` (`gio open`, never xdg-open; `--self-test` watches eleven refusals); android: `KAYA_ONLY=links tools/android/run-emulator.py` (the door is `am start -a VIEW -d <url> -p <package>`); windows: `tools/deploy-win.py akhil@192.168.64.2 links_rust` (tools/guest/relaunch-link.ps1 reads the URL from `C:\kaya\legs\<leg>\relaunch-url.txt` and refuses by name when the shell started another exe); iOS: the rust-swiftui suite's links leg, whose device preflight uninstalls every other dev.kaya.* bundle (the sole-claimant rule) and answers SpringBoard's one-time `Open in …?` through the driver. |
| hand-run one android scene | `KAYA_ONLY=<scene> tools/android/run-emulator.py` on the warm pool takes every suite's leg for that scene — a filter that matches no leg is refused rather than printing a verdict over an empty run; `run-as dev.kaya.rusthost ls shared_prefs` reads the scratch store back, and `check_real_prefs` after every suite refuses the app's REAL domain on any device. |
| read a matrix's lane logs after the next matrix has run | `target/validate-lanes/runs/<stamp>/<lane>.log`, one directory per validate-all run (newest 20 kept) beside the overwritten `target/validate-lanes/<lane>.log` — a duration anomaly is read PER LEG against the run before it, which the S4 windows reading could not be. |
| read a two-act leg's second half on Linux | it is in the leg log under `--- act two ---` (the relaunched process's own stdout, kept by tools/linux/act2-exec.sh) with act2.py's join above it; a D-Bus-activated process's stdio is otherwise the bus daemon's and is thrown away. |
| measure the macOS closed-app relaunch | `tools/mac/notifyprobe/build.sh`, then the executable with `NOTIFYPROBE_PROVISIONAL=1 NOTIFYPROBE_EXIT_AFTER_POST=1`: it posts and exits with the notification delivered, and every launch appends to `/tmp/notifyprobe.log`. Tap the row through the accessibility API on an idle host (docs/traps.md) and the relaunched pid's `ACTIVATED` line is the answer. |
| run one mac leg whose scene has a `relaunch` | `tools/run-leg.py tasks rust --build` runs BOTH acts and joins them: act one publishes `KAYA_SELFTEST: ACT 1 OK`, the runner starts the same bundle with `KAYA_LAUNCH_NOTIFICATION`, and act two's verdict is read back out of `<state>/act2/<id>/act2.verdict`. |
| open the closed-app door by hand (windows) | `ssh akhil@192.168.64.2 'schtasks /create /tn kaya_door /tr "C:\kaya\relaunch-com.cmd <leg> {<clsid>} <aumid> kaya 1 dev.kaya.aurora.notes -" /sc once /st 00:00 /it /rl highest /f && schtasks /run /tn kaya_door'`, then `type C:\kaya\out_<leg>-act2.txt`. The class id is `python3 -c "import uuid;print(uuid.uuid5(uuid.NAMESPACE_DNS,'dev.kaya.aurora.notes'))"` unpackaged and `…'dev.kaya.aurora.notes!tasks'` packaged. NEVER over ssh: COM starts the server in the caller's session and session 0 has no window station. |
| open the plain relaunch door by hand (windows) | the same schtasks shape running `C:\kaya\relaunch-launch.ps1` (tools/guest/) for the leg, which refuses BEFORE launching when act one left no marker or the exe is not on the guest, and prints `code ?` when Start-Process's own object will not answer; `type C:\kaya\out_<leg>-act2.txt` reads act two. |
| ask the Windows notification platform whether it delivers | `tools/guest/notify-ready.ps1` through a schtasks /it task (the shape `notify_ready()` in tools/deploy-win.py runs before any leg): it posts one toast under the app's AUMID, reads the history back and removes it, printing `setting=<Enabled|DisabledForUser> delivered=<n>`. NEVER read the two toast globals back from the registry — the platform reads them lazily (docs/traps.md). |
| read the toast activator's registration | `ssh akhil@192.168.64.2 'reg query "HKCU\Software\Classes\CLSID\{E92B7311-E2B0-5787-BCEA-7DD6B24F166B}\LocalServer32"'` and `reg query "HKCU\Software\Classes\AppUserModelId\dev.kaya.aurora.notes"` (DisplayName, IconUri, CustomActivator). |
| probe macOS notifications without a lane | `tools/mac/notifyprobe/build.sh` builds a bundle-wrapped probe and prints its executable; `NOTIFYPROBE_ID=<id> NOTIFYPROBE_PROVISIONAL=1 <exe>` posts, reads the centre's delivered list back and prints the authorization it measured. A banner request from a headless process is refused AND burns the id for good (docs/traps.md, "A headless macOS notification request burns its bundle id") — probe with a fresh id, and provisionally. |
| probe iOS notifications without a lane | tools/ios/notifyprobe is the mac probe's twin, a minimal simulator .app that asks provisionally, posts and reads the centre's delivered list back; the XCUITest driver's `shade` verb pulls the cover sheet, reveals the list and prints every notification cell with its subtree, and `sb_drag` drags SpringBoard by coordinate. A screen-coordinate tap lands nowhere in that sheet, and no route activates a cell on the simulator (docs/traps.md, "The iOS simulator's shade will not activate a notification"). |
| measure the canvas rasterizers on a paired iPhone | `tools/ios/gpuprobe/build.py --out FILE` builds the probe crate (tools/ios/gpuprobe/probe: vello_cpu, vello_hybrid and classic vello on the same op streams) for aarch64-apple-ios, signs it with the Apple Development identity against the one downloaded profile (which decides the bundle id, scopeprobe's rule), installs and launches it with the console attached and copies the report back; `--mac` runs the same probe on this host. The phone must be unlocked at launch. Records: docs/measurements/canvas-gpu-timing-iphone-2026-09-10.txt |
| drive the rich text protocol against automerge | `cd tools/richtext/automerge-probe && cargo run --release` (its own workspace; automerge never enters libkaya): kaya's mirror model beside an automerge document in UTF-8 bytes, through typing, toolbar acts, two peers merging and the caret oracle, every step compared with `spans()`. Record: docs/measurements/richtext-automerge-2026-09-11.txt |
| run a mac scene that posts notifications | the lane wraps it in a minimal .app (`BUNDLED_SCENES` and `stage_rust` in tools/lib/lanes/mac.py: Info.plist from guests/assets/identity.toml's `id`, ad-hoc signed, LaunchServices-registered); `tools/run-leg.py notify rust --build` does the same for one leg. A machine whose id was burned reads `refused` on every notify leg until System Settings › Notifications re-enables it. |
