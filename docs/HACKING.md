...
| I want to… | the route |
|---|---|
| compile Swift | `source tools/lib/swift-toolchain.sh` first — bare `swiftc` is a shim in the dev shell that says so and exits. THE BINDING IS A PACKAGE TARGET (Package.swift, Swift 6 language mode): build it with `kaya_swift build --disable-automatic-resolution --scratch-path target/swiftpm`, then compile a guest against it with `kaya_swiftc -I target/swiftpm/debug/Modules -I bindings/swift/CKaya <guest> target/swiftpm/debug/libKaya.a -L target/debug -lkaya`. For the simulator add `--triple arm64-apple-ios16.0-simulator -Xswiftc -sdk -Xswiftc <iPhoneSimulator SDK> -Xcc -isysroot -Xcc <same>` and read the output through `target/swiftpm-ios/debug/` — `--triple` alone picks the macOS sysroot and cannot load the standard library. The SwiftUI interpreter is NOT in the package: it keeps `-import-objc-header crates/kaya/include/kaya.h` (tools/swiftui/build-dylib.sh). NEVER pass `-swift-version` to a guest or to the interpreter: the package's `.swiftLanguageMode(.v6)` is the only language mode this repo declares, because a guest built in Swift 6 SIGTRAPs at its first handler (top-level code is `@MainActor`, kaya's app thread is not main) — tools/check-pins.py refuses it by name |
...
| photograph a guest window on GTK | `tools/linux/shot-gtk.py <scene> <out.png> [--layout appmenu:close] [--hold-after N]` — the scene's rust example in the lane's container under Xvfb, held by a settle after step N, shot and cropped; `--layout` injects a GTK settings file so the capture shows a desktop's own window controls, and without it kaya's fallback (docs/gtk-chrome.md). View every capture before it is published. |
