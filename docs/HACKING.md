...
| photograph a guest window on GTK | `tools/linux/shot-gtk.py <scene> <out.png> [--layout appmenu:close] [--hold-after N]` — checked build and core verification, then the scene's Rust example under Xvfb, held after step N, shot and cropped into per-run files; `<out.png>.log` retains build, capture and guest output even on failure. A failed build or capture leaves any previous requested image untouched, not newly validated. `--layout` selects desktop controls; without it, kaya's fallback (docs/gtk-chrome.md). View every capture before publication. |
