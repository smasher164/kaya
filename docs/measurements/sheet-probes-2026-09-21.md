...
| linux, both regimes, after the AdwWindow adoption | the dialog's root is the parent AdwApplicationWindow (the sheet_count read requires it); the sheet and tasks legs | the windows became AdwWindows so the dialog is hosted in the parent; Esc goes to the parent's xid: see the closing commit's counts |
