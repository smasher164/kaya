...
| Android | `LocalDensity provides Density(density, fontScale)` and `fontScale` in the forced `Configuration`, beside the night bits (KayaAppearance's shape) | `LocalDensity.current.fontScale` at the root, and `resources.configuration.fontScale` | 2.0, Android 14's largest |
...
| WinUI | none exists per process: `UISettings.TextScaleFactor` is the user's system setting (1.0–2.25) and XAML applies it by itself. **U3** decides the lane's route | `UISettings.TextScaleFactor` | 2.0 if U3 allows |
...
| Android | `setLocales` on the forced `Configuration`, and `Locale.setDefault` for the formatter | `LocalLayoutDirection provides Rtl` beside it, and `setLayoutDirection(locale)` on the configuration | the root `Layout`'s `layoutDirection` as measured; `resources.configuration.locales[0]` |
...
| WinUI | `ApplicationLanguages.PrimaryLanguageOverride` before the first element (U5: an unpackaged app) and `Language` on each root | `FlowDirection.RightToLeft` on each window's root element | the root's `FlowDirection`; `ApplicationLanguages.Languages[0]` |
...
| Android | `android.icu.text.DateFormat` / `NumberFormat` through JNI on the JavaVM the android backend already holds | the three lengths map to `SHORT`/`MEDIUM`/`LONG`; U10 measures the 24-hour setting reaching it and the JNI call from a non-UI thread |
| Linux | GLib: `g_date_time_format` under the process `LC_TIME`, `%'d`-style grouping under `LC_NUMERIC`, `strfmon` under `LC_MONETARY` | glibc has `%x` (short) and `%c` (full) and NO medium style, so kaya COMPOSES medium from the locale's abbreviated month name, day and year (`%e %b %Y`, the order chosen per locale from `%x`'s order); that composition is kaya's own rule and gets its own unit test (§4). Percent through the number formatter and `LC_NUMERIC`'s percent sign |
| Windows | `Windows.Globalization.DateTimeFormatting.DateTimeFormatter` with the `shortdate`/`longdate` templates and `NumberFormatting.DecimalFormatter`/`PercentFormatter`/`CurrencyFormatter`, through the `windows` crate | medium composes from the templates (`{month.abbreviated} {day.integer}, {year.full}`), U11 measures unpackaged availability and the user's custom short-date pattern reaching it |
