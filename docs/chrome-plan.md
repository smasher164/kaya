...
| WinUI | CommandBar: primaries from the bit, remainder into SecondaryCommands | **[MEASURED: pinned winmd]** dynamic overflow automatic (width breakpoints, the "…" button, overflow rows re-laid icon+label); 20→16px icon rescaling; labels hidden while closed; IsEnabled follows the one button object; 48px transparent bar = the Win11 modern look by default. |
