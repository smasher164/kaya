...
| WinUI | CommandBar of the primaries, mounted in the `RightHeader` of a `Microsoft.UI.Xaml.Controls.TitleBar` that IS the window's caption (`ExtendsContentIntoTitleBar`); MenuBar keeps its own row below; remainder stays in that MenuBar | **[MEASURED: pinned winmd + the VM]** dynamic overflow automatic (width breakpoints, the "…" button, overflow rows re-laid icon+label); 20→16px icon rescaling; labels hidden while closed; IsEnabled follows the one button object. **Revision ratified 2026-08-17:** the bar no longer hangs in a strip under the caption — it merges INTO the caption row, a MOUNT-POINT change and no new knob. **Revised again 2026-08-17 (one band):** the MenuBar moves INTO the caption's `LeftHeader`, the title into the control's centre slot, and the shell's menu row goes empty (an empty Auto row measures zero). `AppWindowTitleBar.PreferredHeightOption = Tall` IS taken — derived from the same promotion, both directions — because the control sizes its own band and leaves the system's caption buttons in the standard one (upstream #9863), which put two runs of buttons in one row on centres 9 DIP apart. Measured after: system cluster 48x48 cells on centre-y 180, promoted commands 48x48 on 181, menu items 32 tall on 181, both runs pitch 48 gap 0, separated by an 8 DIP strip: `TitleBarMinDragRegionWidth` is 48 in the library's dictionary and kaya overrides it to 8 through a direct entry in `Application.Resources` — the platform's own lightweight-styling route, written at the caption mint before the control exists (`mint_caption_titlebar`). 48 reserved a whole caption button for a drag region the band does not need (the Content slot's TextBlock is never punched out of the drag surface, so hundreds of pixels already drag); 8 is the gap the band's own MenuBar items sit at. Measured before/after: 48 → 8, both drags exact (by the 8px strip AND the title centre), Find fires, Close closes. Drag proved by dragging; the menu proved by CLICKING it in the caption (File opens, Export fires, the guest asserts `exported`); caption buttons close the window. **Revised again 2026-08-17 (the title centres on the WINDOW):** the caption title no longer sits at the middle of the leftover slot between the headers — it is aimed at the window's own centre-x, VS Code's rule, and clamped when a header would collide. The slot's middle is not the window's middle: the right of the band carries the commands, the 8 DIP strip and the system's caption cluster while the left carries only the menu, so the title measured **83 px left of centre** when it was the control's `Title`, and **63 px** in the centre slot after the strip shrank. Measured after, off UIA against `DWMWA_EXTENDED_FRAME_BOUNDS` (the visible frame; the client area is concentric with it, 944 inside 946): **drift 0 px** on the toolbar scene at 946, 1100, 900, 800 and 700 DIP wide, drift 0 on the editor scene (2 menus, a different imbalance) at 1100, 900, 800, 700, and drift 0 after a LIVE resize by dragging the border (946 → 706). Clamped, driven deliberately by narrowing the window: 640 → −8, 600 → −28, 560 → −48, 520 → −64.5 with the title ellipsized to 31 px from its untrimmed 40, always ≥ 8 DIP clear of the `CommandBar` and inside the slot on the menu side, never overlapping either. Mechanically it is one `Thickness` per layout pass on the Content slot's TextBlock — a BIAS, `(d, 0, -d, 0)`, whose halves cancel in the element's desired size, plus `MaxWidth` for the ellipsis. The obvious spelling (margins that carve the wanted rect out of the slot) aims just as well and RATCHETS the window: a margin is part of what an element asks for, so the title's desired width became the slot's, the window's content asked for the widest band it had ever had, and dragging it narrower produced no further layout pass at all. `d` is corrected from a measured position rather than predicted (`d += wanted x − measured x`, snapped to the pixel grid), so it converges in one step and no model of the template's arithmetic can be wrong. Hook: the control's own `LayoutUpdated`, which is the one event that catches all four reasons the geometry moves — resize, menu change, command change including the CommandBar's own dynamic overflow, and a new title. No app surface, no knob. |
