...
func kayaCopyToPasteboard(
    ...
    kayaClipStaging()
        ...
        // AND KAYA'S OWN MARKER, on item 0 beside the payload: the
        // whole of the witness's evidence on this platform, because the
        // change count is not evidence here (kayaClipMarkerType carries
        // the reason). It is one type more than the guest asked for and
        // it can answer no read — every arm of the walk gates on the
        // ACCEPT LIST first, and no accept list names kaya's namespace.
        // A clip with no representations at all still leaves this item:
        // kaya staged an empty board, and the witness has to be able to
        // say whose empty board it is.
    ...
    #endif
    // THE BOARD KAYA NOW OWNS. Anything that moves it after this line
    // was somebody else, and the diagnostics say so by name
    // (kayaClipOwnerClause). Composed by this very function, so on iOS
    // the marker above must be on the board it reads back — and if it is
    // not, the stage says so rather than leaving a witness that can only
    // ever agree.
...
/// ONE LINE WHEN A READ ANSWERS NOTHING, naming what was asked for and
/// what the clipboard was actually holding.
///
/// Empty is a legitimate answer with four indistinguishable causes
/// (denied, unfocused, absent, or nothing this read accepted), so the
/// GUEST cannot report which — but the backend knows what the board
/// offered at that instant, and a step that reads "label#0 reads empty"
/// does not. That gap cost this project two debugging rounds on iOS,
/// where the answer was that the board had been REPLACED by another
/// process's clip (docs/clipboard-plan.md §8 finding 7): the offer list
/// in this line says so at a glance, and it costs nothing on the path
/// that answers.
func kayaClipNote(
...
/// THE PRIVATE TYPE EVERY CLIP KAYA COMPOSES CARRIES ON iOS, and the
/// whole of the witness's evidence there: UIPasteboard hands out no
/// counter a reader may believe, so "is this still the clip this leg
/// staged?" is answered by looking at the CONTENT for something only
/// kaya writes (kayaClipDrifted below; docs/traps.md carries the
/// measurements, and this is also the answer Apple's own DTS gave the
/// 2019 reporter of the focus bump).
///
/// AN ARBITRARY STRING, NOT A UTI: UIPasteboard takes a type string
/// verbatim, slash and all — the same measured property the custom-clip
/// id rides on (docs/clipboard-plan.md §8 finding 1). The namespace is
/// kaya's, and a guest that declares this exact id as its own custom
/// format is asking for kaya's marker.
///
/// macOS DOES NOT CARRY IT: NSPasteboard's count moves on writes and
/// only on writes, so that platform has a cheaper and stricter answer
/// already.
///
/// THE BYTES ARE NEVER READ. Presence is the whole signal, so the value
/// is a constant rather than a payload somebody might start believing.
let kayaClipMarkerType = "dev.kaya/staged"
let kayaClipMarkerBytes = Data("staged".utf8)

/// What the board this leg staged has been replaced by, in the terms
/// the PLATFORM ACTUALLY MEASURED.
///
/// The two platforms measure different facts — a count that means what
/// it says on macOS, a private type on iOS — and a step's failure
/// sentence may only claim the one that was measured (invariant 3). So
/// the evidence travels as a value and each case renders itself; there
/// is no shared sentence with a platform-shaped hole in it, and no `#if`
/// in either of the two functions below that print.
enum KayaClipDrift {
...
/// THE CHANGE COUNT KAYA ITSELF LAST PUT ON THE BOARD — the app's own
/// copy, or a seed that settled. Written from the app thread (the copy)
/// and the harness thread (the seed), so it takes a lock rather than
/// pretending an Int is atomic.
private let kayaClipOwnerLock = NSLock()
private var kayaClipOwnerChange = -1
/// AND WHETHER THAT BOARD CARRIED KAYA'S MARKER — iOS's half of the
/// same record, and the half its witness reads. Set from what the stage
/// SAW on the board, never from what the write intended: a stage that
/// left no marker makes the witness stand down rather than fire, which
/// is the one direction a false verdict here is affordable in.
...
///
/// `composed` is whether the ITEMS were built by a writer kaya controls
/// — this file's own `items =`, or `tools/ios/clipctl` for a seed — as
/// against a clip UIKit or AppKit composed down the responder chain for
/// a native cut or copy. Only a composed clip can be expected to carry
/// kaya's marker, and only iOS reads it.
///
/// IT RECORDS WHAT IT SEES, NOT WHAT THE WRITE MEANT. The marker flag
/// comes off the board's own type list, so a stage whose marker never
/// landed leaves the witness standing down instead of naming a stranger
/// for kaya's own write — this design's only false-positive mode, shut
/// here rather than argued away.
///
/// AND A COMPOSED STAGE THAT LOST ITS MARKER FAILS THE LEG, because the
/// alternative is the vacuous guard this project has been bitten by
/// twice: a witness whose evidence silently stopped being staged never
/// fires again and every leg stays green. It also pins the marker's
/// spelling across the two binaries that write it — this file and
/// tools/ios/clipctl/main.swift — which no compiler spans, on the path
/// every iOS clipboard leg walks five times.
...
/// The record the last stage left, taken under the lock in one go: a
/// caller that read the three separately could compare a count from one
/// stage against a marker flag from the next.
...
///
/// THE TWO PLATFORMS MEASURE DIFFERENT FACTS, and this is the one place
/// that knows it: macOS compares the change count, iOS compares the
/// presence of kaya's marker, and neither borrows the other's evidence.
///
/// The count cannot serve on iOS. `UIPasteboard.general.changeCount` is
/// a PER-PROCESS number that a text field taking focus inflates by 4
/// with the board untouched, and it never resyncs to the server's, so
/// there is no tolerance window that could rescue it. The notification
/// cannot serve either: it does not fire for another process's write at
/// all. Both measured 2026-08-18 and written up in docs/traps.md,
/// "UIPasteboard's changeCount is a PER-PROCESS number" — with the
/// third measurement, the one this arm stands on: the marker held true
/// through the whole focus inflation and went false within 13ms of a
/// stranger's write.
///
/// WHAT THE MARKER CANNOT SEE, said here because a guard nobody has
/// bounded gets believed past its evidence. It sees the staged clip
/// REPLACED. It does not see a stranger who writes a clip CARRYING
/// kaya's marker — a second kaya leg on one device would, and the lane's
/// one-leg-per-simulator shape is the whole of what keeps that
/// hypothetical — nor a writer who APPENDS an item and leaves kaya's in
/// place, which the macOS count does catch. That is the right trade for
/// the threat model: this witness is for a machine-wide resource shared
/// by ACCIDENT (a human pressing Cmd-C, a clipboard manager, a VM's
/// relay, a sibling lane), none of which have heard of
/// `dev.kaya/staged`. It is not a boundary against anyone who is trying.
///
/// Silent before this leg has staged anything, silent inside a stage
/// (the board is mid-write, kayaClipStages), and on iOS silent after a
/// stage that left no marker — a native cut or copy, whose items UIKit
/// composed (kayaClipOwned's `composed`).
///
/// ONE OBSERVATION OF THE BOARD backs both halves of what this returns.
/// A verdict from one read and an offer list from another can disagree,
/// and the sentence built from them would describe a board that never
/// existed.
...
/// or "" when the board is still the one kaya left.
///
/// WHY THIS SENTENCE EXISTS. A pasteboard has no "who wrote it", so
/// every failure whose real cause is a second principal — a VM's
/// clipboard relay, a clipboard manager, a sibling lane — arrives as
/// kaya's own step reading the wrong thing, and the session that gets it
/// starts by suspecting kaya. The board's identity is the one piece of
/// evidence that separates the two, and kaya knows what it staged:
/// 2026-08-04's matrix failure was a paste that silently did
...
///
/// AND ON iOS IT NOW ANSWERS BOTH WAYS. This clause used to say that the
/// question could not be told on that platform, which was true of the
/// COUNT and is not true of the marker: a board still carrying
/// `dev.kaya/staged` is the clip this leg staged, and saying so is a
/// measurement rather than a silence. The middle case — kaya has staged
/// nothing yet, or the stage left no marker — stays quiet, because the
/// only honest thing there is that nothing was measured.
...
/// leg staged. Nothing on the matching path: the board is still the one
/// kaya left and the consumer proceeds exactly as it always did.
///
...
/// a rerun of the matrix. Whatever a platform can measure about the
/// board's identity is the evidence that separates them, and this is
/// where it gets read.
///
/// IT SAYS WHAT IT MEASURED AND STOPS THERE. kaya staged a board, made
/// no write of its own since, and finds a board it cannot recognise — a
/// different count on macOS, its marker gone on iOS — so some writer
/// that is not this leg replaced the content. WHO is not on the
/// pasteboard at all: no API answers it, so this names nobody. The type
/// list is the board as it is now, for the reader who has to guess
/// anyway (a text clip where files were staged reads very differently
/// from an image one).
///
/// ONE SENTENCE, TWO EVIDENCES, and not by an #if here: kayaClipDrifted
/// answers in whichever terms its platform can measure — a change count
/// on macOS, kaya's marker on iOS — and this renders what it was handed.
/// The consumers below call it on both platforms; one call site per
/// consumption, one place that decides what the platform can say.
...
/// What a child tool DID, not merely what it printed.
///
/// A TOOL THAT FAILED AND A TOOL THAT SUCCEEDED SILENTLY ARE THE SAME
/// EMPTY STRING, and this arm could not tell them apart for the whole
/// life of the clipboard scene: the run discarded the exit status and
/// piped stderr into a pipe nobody read, so an `osascript` that refused
/// looked exactly like one that worked. Either way the only symptom was
/// a settle that timed out — a sentence about the pasteboard, said
/// about a process that may never have written it. The status and the
/// stderr ride along here so a failure can quote the tool.
struct KayaToolRun {
    let args: [String]
    let out: String
    let status: Int32
    ...
    /// The tool's own account of itself, in one line, for an error that
    /// has to explain a failure to a session with no context.
    var note: String {
        let name = args.first ?? "?"
...
/// and hand back what it did. The tools are Apple's — `pbcopy`,
...
///
/// NOT `@discardableResult`, and that one missing attribute is the
/// guard: dropping this value on the floor is exactly the defect that
/// cost a matrix debugging round, and now the COMPILER refuses it at
/// the callsite rather than a gate refusing it later. A caller that
/// genuinely has nothing to do with the answer says so by name —
/// `kayaToolNote(kayaRunTool(…))`, which reports a tool that failed.
func kayaRunTool(_ args: [String], stdin: Data? = nil) -> KayaToolRun {
        ...
        // while this thread is still reading stdout to EOF stops writing
        // and never exits — a deadlock built into the very path that
        // exists to report a tool having gone wrong. Reading stderr
        // beside stdout costs one queue hop and removes the class.
...
/// The clipboard as a settle sees it: the changeCount and the type
/// list. THE ONE PLACE THE TWO BOARDS DIFFER, so everything built on it
/// is written once and cannot drift between the platforms this file
/// serves — which is what the settle below relies on.
...
/// Put the content on the clipboard ONCE, by the platform's own means.
/// Answers nil when the attempt went through, or the sentence to fail
/// with when the MECHANISM refused — which is never worth retrying: a
/// tool that will not run and a host bridge that will not answer do not
...
private func kayaClipSeedOnce(kind: String, arg: String) -> String? {
        ...
        // kayaRunTool runs nothing here). The host spawns
...
///
/// ONE BODY FOR BOTH PLATFORMS, and that is not tidiness: the seed's
/// settle is the rule that "the verb returns only when the content is
/// really there", and the two arms drifted apart once already — mac
/// polled the TYPE alone, which three of this scene's four seeds
/// already offered before the seed ran, so those three waited on
/// nothing and only `files` could ever report trouble (§8 finding 6 is
/// the same vacuity, found on the other platform). What differs between
/// the platforms is the WRITE and the board object; the rule does not.
func kayaClipboardSeed(kind: String, argument: String) {
    ...
    // downstream will say so: `set the clipboard to POSIX file
    // "<missing>"` exits 0, prints nothing and leaves the board
    // untouched (measured 2026-08-04), so a path that is wrong or a file
    // the guest has not written yet arrives as a settle that waits out
    // its whole deadline and then talks about the pasteboard. Both
    // path-taking kinds check here, on both platforms, where the answer
    // is one line.
    ...
    // AND WAIT UNTIL IT IS REALLY *NEW*, not merely until the kind is
    // offered: the scene's own copy leaves a union clip carrying nearly
    // every kind's type, so a poll for the type alone is satisfied by
    // the STALE board before the seed lands. The changeCount moving is
    // what proves the board this poll reads is the seed's board; then
    // the type confirms it is the right one. Both polls are prompt-free
    // — a type list and a changeCount cost nothing at any stage — so
    ...
    // 2026-08-18 — kayaClipDrifted carries the numbers), so there the
    ...
    // What says the board is KAYA'S on that platform is neither of
    // them but clipctl's marker, and the stage's close is where that
    // is checked (kayaClipOwned, composed:).
    //
    ...
    // the board inside its clear-then-put window: measured 2026-08-04,
    // 12 of 12 writes vanished against a competitor writing every 10ms,
    // osascript exiting 0 and silent each time, with a 1ms poll that
    // never once saw the type (200 of 200 land with no competitor, so
    // this is contention and not load). That is the matrix-only failure
    // this settle used to report as "never appeared on the clipboard"
    // after fifteen fruitless seconds — and why widening the deadline
    // from 5s to 15s changed nothing. The seed is idempotent by
    ...
    let started = Date()
    // EVERY BOARD THIS WAIT SEES, one entry per distinct clip. A settle
    // that runs out has two very different stories and the bare timeout
    // tells neither: nothing was ever written (the count never moves),
    // or something else is writing this board too (the count moves to
    // types that are not ours, again and again, through every re-issue).
    // This list is that evidence, and it costs one comparison per poll.
    var clips: [String] = []
    var last = before
    var seen = 0
    var attempts = 0
    while true {
        ...
        while true {
            ...
            if board.change != before, board.types.contains(want) {
                ...
                // process that made it: what the verb settled on is the
                // board kaya owns from here. COMPOSED, because the
                // writer on the other side of both platforms' seeds is
                // one kaya controls — so on iOS clipctl's marker has to
                // be on this board, and a settle that closed without it
                // fails the leg instead of disarming the witness.
...
/// ONE LINE WHEN A CLIPBOARD TOOL DID NOT SUCCEED, quoting it.
///
/// A foreign read answers a STRING the scene compares byte for byte, so
/// a tool that failed and a clipboard holding nothing of that kind
/// arrive as the same "" — and the step then reports the guest's state
/// where the harness's belongs. This is kayaClipNote's sibling for the
/// side kaya spawns. Not a failure by itself: `osascript` refusing to
/// write out a png the board does not hold IS the empty answer, and the
/// line says so in the tool's own words.
@discardableResult
...
func kayaPresentSaveDialog(
    ...
    /// it from the host (tools/ios/simdrive) and the app learns nothing
    /// about it except through the delegate. So the four moments that
    /// exist here are said out loud — presented, picked, cancelled,
    /// emitted — and the difference between "the tap never landed" and
    /// "the tap landed and the delegate never fired" stops being a
    /// guess. Same voice and same destination as kayaClipNote, for the
    /// same reason: the guest cannot report what only the backend knows.
        ...
        for raw in kayaSplitStatements(trimmedLine) {
            ...
            case "type":
                ...
                let typed = kayaQuoted(Array(parts[1...]))
                    ...
                    // platform has in process (kayaTypeAtFocus's own
                    // comment carries the deviation and what was
            ...
            case "expect_no_stall":
                ...
                // stall about a HEALTHY app is worse than none, because
                // the line is read as evidence. It shipped that way — the
                // five languages that read the occurrence ring directly
                // reported a stall on every green leg — and this arm is
                // what refuses it. Polled like its sibling: the reading
                // clears on the watchdog's own 100ms poll.
            ...
            case "expect_toolbar":
                ...
                #else
                    // The same invariant off the REAL UINavigationBar:
                    // the promoted set is really among the bar buttons
                    // UIKit built, in catalog preorder, and the
                    // remainder's More menu is really there beside them.
                    let gotChrome = DispatchQueue.main.sync { kayaToolbarIOSChromeRead(0) }
                #endif
                // ONE VERDICT SPELLING FOR BOTH HOSTS. The platforms
                // differ in which object graph answers — NSToolbar there,
                // UINavigationBar here — never in what the scene reads
                // back, and this scene is compared byte-for-byte across
                // every lane. Two copies of this tail is how that stops
                // being true.
            ...
            case "menu_activate":
                ...
                let failure = DispatchQueue.main.sync { () -> String? in
                    ...
                    // BEFORE THE DISPATCH, AND ON BOTH PLATFORMS, because
                    // an inert standard command is the one activation
                    // that succeeds and does nothing (kayaRoleInertNote).
                    // Resolved against the model, which is what the mac
                    // route resolves against too before it walks to the
                    // real NSMenuItem.
            ...
            case "shortcut":
                ...
                let unowned: Bool = DispatchQueue.main.sync {
                    // A CHORD REACHES THE SAME COMMANDS, so it reaches
                    // the same silence: no scene pastes by chord today,
                    // and the session that first writes one must not have
                    // to find this out again.
            ...
            default:
                    ...
                    Date() < stepDeadline
                {
                    ...
                    // THE EVIDENCE MUST OUTLIVE THE PROCESS. The verdict
                    // line at the bottom is authoritative and stays so,
                    // but it is printed LAST — and a scene whose failure
                    // is "the dialog did not resolve" walks straight into
                    // the one-per-process dialog guard three steps later,
                    // which ABORTS. The failure list dies with the
                    // process and the log shows a panic with no reason,
                    // which is exactly the round this cost on iOS: the
                    // host driver had written a self-diagnosing sentence
                    // naming the tap point and what the picker listed,
                    // and nobody ever saw it.
                    //
                    // So a failure is printed the moment it becomes
                    // FINAL — an expect whose deadline has run out, or
                    // any action, which never retries. One line each, on
                    // the same line-buffered stdout as the step trace, so
                    // it lands in order beside the step it belongs to and
                    // survives a kill as well as an abort.
...
func kayaRoleEnabled(_ role: String) -> Bool {
        ...
        switch role {
        case "undo":
            // A4 again, and the same rule: enablement IS the routing
            // answer, asked live. `nothing` is what a disabled Edit>Undo
            // means. The phone's read path (kayaMenuStateRead) answers
            // from the model, which calls straight through to here, so
            // this platform never had the mac's stale-enablement problem
            // — there is no NSMenuItem holding a state it was born with.
            return kayaUndoRoute() != .nothing
        case "redo":
            return kayaRedoRoute() != .nothing
        case "cut", "copy":
...
/// item is INERT, exactly as native chrome leaves a greyed one. For a
/// person that is right and needs no words: they can see the grey. For a
/// scene it is SILENT — `menu_activate "Edit>Paste"` reports success,
/// nothing is emitted, and the failure surfaces up to five seconds later
/// as a label that still reads what it read before, in a step that
/// mentions neither the clipboard nor the command.
///
/// That is exactly the 2026-08-04 matrix failure: a foreign image clip
/// replaced the seeded text 25ms after the seed settled, so both pastes
/// went inert, and the only other line in the log was a stall report
/// about an app thread that was fine. The read's own note
...
// in what the user typed.
    ...
    //
    // EVERY CELL OF §1.3 DIVERGES FROM THE MAC, mechanically rather than
    // cosmetically. The probe measured it against a kaya-shaped app
    // (scratchpad/undo-probe-apple.md) and this arm re-measured every
    // load-bearing cell against THIS interpreter, because §3a exists to
    // say that a premise inherited from another backend is not a
    // measurement (scratchpad/undo-fan-ios.md):
    //
    // - BOTH text kinds carry a PRIVATE `_UITextUndoManager` and the
    //   window's manager is never in play, so "the focused text's own
    //   stack" is the only stack any affordance on this platform can
    //   reach — shake-to-undo included, which is ON by default and was
    //   measured to drive that manager and nothing else. A6's
    //   consequence, documented rather than fought: the core tier is
    //   invisible to shake.
    // - `undo:` down the responder chain performs the focused field's
    //   undo and NEVER FALLS THROUGH, so the second half of D6's routing
    //   — free on macOS, where AppKit walks to the next manager — is
    //   HAND-WRITTEN here, and the route is asked BEFORE the send.
    // - `canPerformAction(undo:)` answers FALSE while undo works, so it
    //   is not the enablement oracle: A4's one named query reads
    //   `undoManager?.canUndo`.
    // - D7 IS FREE: a programmatic write registers nothing and clears
    //   the field's stack by itself, both kinds. The explicit
    //   `removeAllActions()` after a write is a MEASURED NO-OP and must
    //   not be mistaken for the mechanism, so this arm ASSERTS the rule
    //   rather than performing it (kayaNoteQuietTextWrite below) — a
    //   UIKit regression then reports instead of passing silently.
    // - A1's clear is NOT free, and it is the asymmetry a reader is
    //   likeliest to get wrong from the sentence "iOS gets D7 for
    //   nothing": an iOS field's history SURVIVES a focus round trip
    //   (measured; on macOS the field editor takes it with it). So a
    //   core group committing while a field is focused has to clear that
    //   field's stack EXPLICITLY, or §3's keystone invariant — native
    //   stack ⊆ current episode — holds on three platforms and fails on
    //   this one, and the scene's interleave walks back in the wrong
    //   order.

    /// The turn budget this half spends waiting for UIKit to catch up
    /// with a model write: main-queue turns a millisecond apart, so it
    /// is render-latency scale and a wedged sync reports rather than
    /// hangs. Same number and same reasoning as the macOS constant of
    /// this name — they are in mutually exclusive branches, and the
    /// duplicate is deliberate: hoisting it would have rewritten the
    /// committed mac reference mid-fan-out.
    private let kayaUndoSyncTurns = 240

    /// Q2's ledger-quiet bracket, IN THIS PLATFORM'S TIMING — and the
    /// timing is the whole reason it is a scope here and a text match
    /// there.
    ///
    /// §3a asked every arm to measure whether a native undo reaches
    /// kaya's model. Here it DOES — UIKit's undo is an ordinary text
    /// replacement, so SwiftUI's binding setter runs, the node is
    /// written and the ordinary `text_changed` is emitted — and it does
    /// so SYNCHRONOUSLY INSIDE `sendAction`, before this backend can
    /// take its sample. The mac arm's bracket records the sampled TEXT
    /// and lets the emission that follows a runloop turn later consume
    /// it; on this platform that emission has already gone by then.
    ///
    /// AND THE TEXT BRACKET MUST NOT BE USED HERE AS WELL. A record
    /// written after the edit it was meant to silence is never consumed,
    /// and it would sit in the table silencing the NEXT edit that
    /// happened to reach the same string — which is a corruption, not a
    /// miss: the episode's high-water would stop tracking a field the
    /// user is still typing in, and the redo side reads that high-water.
    var kayaRoutedNativeUndoDepth = 0

    /// Perform the platform's own undo (or redo) with the bracket held
    /// open across it, so the edit it provokes is banked ONCE — by
    /// `kayaNoteNativeUndo`'s sample, with the interpreter's own
    /// emission marked quiet.
    ...
    ///
    /// A WALK RATHER THAN AN API, because there is no API: UIKit names
    /// no first responder (this is the gap the clipboard arm's diagnostic
    /// already records in as many words), and the `sendAction`-capture
    /// trick answers with a responder that may not be a view. The walk is
    /// the probe's own technique, public API, and cheap — a kaya scene's
    ...
    /// The focused EDITABLE, or nil when nothing editable is focused —
    /// the iOS half of the mac arm's `kayaFocusedTextResponder`, and the
    /// same trick for the same reason: both kaya text kinds land on a
    /// UIKit control conforming to `UITextInput` (a UITextField for an
    /// entry, a UITextView for a textarea), so the caret move, the text
    /// sample and the undo query are each written ONCE for both kinds
    /// without this layer having to know which one is focused.
    func kayaFocusedTextInput() -> (UIView & UITextInput)? {
        kayaFirstResponderView() as? (UIView & UITextInput)
    }

    /// The same, WAITED FOR — the phone's version of the mac arm's
    /// `kayaAwaitTextWindow`, and it guards the identical trap (§3a's
    /// third finding): kaya's focus is a model fact the instant the
    /// focus command applies, and `expect_focused` reads THAT model,
    /// while UIKit makes the control first responder a render later. A
    /// `type` step that ran in between would report "nothing focused"
    /// for a scene that did everything right.
    ...
    ///
    /// POINT 1 IS SPELLED `insertText`, AND THAT IS THE ONE DEVIATION IN
    /// THIS ARM. The contract names a real key event per platform —
    /// NSEvent, SendInput, gdk_event_put, the instrumented injection —
    /// and iOS is not in that list because it has none: UIKit exposes no
    /// way to construct or post a `UIPressesEvent`, `simctl` has no
    /// typing verb, and driving Simulator.app through System Events
    /// would type at whatever is frontmost, which under a three-device
    /// pool is a flake generator rather than a fix (the same argument
    /// the mac arm made against CGEvent).
    ///
    /// `insertText(_:)` is `UIKeyInput`'s method — the one UIKit's OWN
    /// keyboard calls for a pressed key — so it is the platform's text
    /// input path and not a text write. What the contract is BUYING with
    /// point 1 was then measured here rather than assumed: each
    /// character fills the field's native undo stack exactly as a user's
    /// typing fills it, and each one emits the ordinary `text_changed`
    /// the core banks its episode from (scratchpad/undo-fan-ios.md, M2).
    /// Only the COALESCING differs — a burst from a real keyboard merges
    /// into fewer native steps than one insertText per character does —
    /// and point 5 hands granularity to the platform explicitly.
    func kayaTypeAtFocus(_ text: String) -> Bool {
        guard let input = kayaAwaitFocusedTextInput() else { return false }
        let before = DispatchQueue.main.sync { () -> String? in
            // CONTRACT POINT 3: TYPING APPENDS. iOS is gentler than
            // macOS here — becoming first responder does not select the
            // whole contents — but the contract is a contract and one
            // script is compared byte-for-byte on all five lanes, so the
            // caret goes to the end with nothing selected, explicitly.
            // Before the keys and not between them: a selection change
            // mid-run breaks the field's own coalescing, which is the
            // granularity the delegated tier is made of.
    ...
    /// platform-specific: an action is never retried, and the step after
    /// a `type` is usually `menu_activate "Edit>Undo"` — an action too,
    /// with no retry cover — so a keystroke still in flight reads as a
    /// broken undo rather than as a missed key. Waits for the model the
    /// app sees to MOVE and then hold still.
    ///
    /// A timeout is not a verdict: nothing focused is a legitimate state
    /// under this verb's contract, so this reports and returns rather
    ...
    ///
    /// A programmatic write was measured to clear the field's native
    /// history by itself, so there is nothing here to perform — and a
    /// `removeAllActions()` written anyway would be indistinguishable
    /// from the platform doing it, which means a UIKit regression would
    /// pass silently and the corrupting case (a Cmd+Z that reverts the
    /// APP's write) would ship green. The probe said exactly this: if
    /// the arm wants a guard, the guard is `canUndo == false` after a
    /// write.
    ///
    /// ON THE FAR SIDE OF THE RENDER, for the mac arm's reason: a kaya
    /// write is `node.text = …` on an @Observable class, and SwiftUI
    /// pushes it into UIKit a later runloop turn. Asked at the model
    /// write, this would read the stack as it was BEFORE the write and
    /// answer nothing useful.
    ///
    /// And if the assertion ever fails, the rule still wins: it clears
    /// explicitly and says so. Uniform semantics first, diagnosis loud.
    func kayaAssertNativeUndoCleared(expecting text: String, tries: Int = 0) {
        ...
        if kayaTextInputString(input) != text {
            // SUPERSEDED (the mac arm's case, same shape): the model has
            // moved past the write this assertion belongs to, so the
            // control will never show `text` and the later write's own
            // assertion covers the field.
    ...
    ///
    /// This is the arm's one piece of real work that the "iOS gets D7
    /// for nothing" summary hides. D7's freebie covers a programmatic
    /// WRITE; A1 fires when a core group commits with a field focused
    /// and nothing wrote that field at all. On macOS the field editor
    /// loses its history anyway; here the field keeps it across focus
    /// changes (measured), so without this call the field's stack would
    /// still hold typing from BEFORE the group — and "ask the focused
    /// text first" would take back that older typing ahead of the group,
    /// which is precisely the interleave §3 exists to make
    /// unconstructible.
    ///
    /// MEASURED EFFECTIVE, which the probe could not establish: its only
    /// clear followed a write that had already emptied the stack, so a
    /// no-op and a working clear looked the same. Live, on the private
    /// manager, the star group's commit read canUndo true going in and
    /// false coming out (scratchpad/undo-fan-ios.md M3).
...
func kayaTakeNativeUndoEcho(_ id: UInt64, _ text: String) -> Bool {
        ...
        // emission arrives INSIDE the routed undo rather than a turn
        // after it (kayaRoutedNativeUndoDepth carries the measurement).
        // Same rule in both places — an edit this backend provoked by
        // routing an undo is banked once, by the sample — and the two
        // spellings are the two platforms' delivery orders, not two
        // policies.
...
func kayaFocusedCanUndo() -> Bool {
    ...
    #else
        // THE SAME QUESTION, and NOT the one the platform offers to
        // answer: `canPerformAction(undo:)` on the focused responder was
        // measured FALSE while undo demonstrably worked (§1.3), so
        // asking UIKit's own enablement oracle here would ship a
        // permanently greyed Edit>Undo that nobody could explain. The
        // manager is asked directly instead, which is the same reach the
        // macOS line above makes.
...
///
/// THAT ASSERTION NOW EXISTS (the iOS arm): the phone half takes it on
/// the far side of the render and reports — loudly, naming the
/// measurement it contradicts — if UIKit ever stops doing it, then
/// clears explicitly so the RULE survives the regression that broke the
/// mechanism. A silent `removeAllActions()` in its place would have made
/// that regression unobservable, which is the whole reason the probe
/// recommended the assertion over the call.
...
func kayaClearUndoForGroup(_ window: UInt64) {
    ...
    #else
        // NO RENDER TO WAIT FOR HERE, and that is the difference from
        // the write path rather than an omission: a group commit does
        // not write this field, so there is no value in flight that a
        // later push could re-register behind the clear. What there IS
        // on this platform is a history that would otherwise SURVIVE
        // (measured — an iOS field keeps its stack across a focus round
        // trip), which is why the phone cannot take A1 for free the way
        // it takes D7.
...
func kayaUndoRoute() -> KayaUndoRoute {
    ...
    ///
    /// macOS asks `kayaPresentedMenuWindow` — the key window, which is
    /// what its global menu bar is showing. This platform has no such
    /// variable (a phone presents the primary's catalog, and auxiliary
    /// windows are capability-rejected here), so the activation asks the
    /// same question the EMISSION asks: the window whose ledger this
    /// field's typing was banked into. Falling back to the primary is
    /// the same answer `kayaWindowOf` gives for an unmounted widget —
    /// window 0 always exists, and a coarse ledger is never a lost one.
...
func kayaPerformUndoRole(_ role: String) -> Bool {
    ...
    #else
        // THE TWO-STEP, HAND-WRITTEN. §1.3 measured that `undo:` reaches
        // the focused field's private manager and STOPS: with the field's
        // own stack empty the send is simply refused, and nothing else is
        // ever reached. So the ROUTE decides first and the send happens
        // only on `.native` — which is the same code the macOS arm above
        // runs, for a different reason (there the route is asked to keep
        // this backend from disagreeing with the ledger; here it is also
        // the only thing standing between a core-tier step and silence).
        switch role {
        case "undo":
...
func kayaNoteNativeUndo(_ window: UInt64) {
    ...
    #else
        // ONE CHANNEL LEFT, NOT THREE — and §3a is why that is a
        // measurement rather than a shortcut. Asked here the way the
        // amendment demands, this platform answered YES: UIKit's undo is
        // an ordinary text replacement on the UITextField, so SwiftUI's
        // binding setter ran, the node was written and the ordinary
        // `text_changed` was emitted before this line was reached
        // (scratchpad/undo-fan-ios.md M1 — `text="tea" model="tea"` at
        // the instant `undo:` returned, where the mac's model was still
        // "teas" fifty milliseconds later). The premise §0 states for
        // every backend HOLDS here and fails there, which is exactly the
        // raw-control-versus-declarative-layer line §3a draws: on this
        // platform SwiftUI's TextField IS a UITextField.
        //
        // NOT WRITING THE NODE IS DELIBERATE, not an omission. The mac
        // arm must write it or the next render pushes the stale model
        // back into the control and visibly rolls the undo back; here
        // the model already holds the undone text, and a second write
        // would at best be a no-op and at worst race the binding.
        //
        // What remains is the LEDGER, once. The emission that just went
        // was bracketed ledger-quiet (kayaSendBracketedNativeUndo), and
        // this sample replaces it: Q2's one-reporter rule, with the two
        // platforms differing only in which of the two reports they
        // suppress.
...
func kayaSendToFocusedResponder(_ selector: Selector) -> Bool {
        ...
        // No public API names this platform's first responder, so the
        // sentence below says what it can. The TRIGGER and the meaning
        // are the same on both platforms, which is the part that has to
        // be uniform.
        let offered = ["the app's responder chain"]
    #endif
    // NOBODY TOOK IT, and this was swallowed even though the callsite's
    // own comment claimed otherwise ("a refusal is reported rather than
    // swallowed"): a command that reached no responder looks exactly
    // like a widget that ignored the content, and the scene reports a
    // field that simply stayed empty. A paste sent while the platform's
    // focus is still catching up with kaya's is precisely this, and it
    // is intermittent — the failure you get once a month and cannot
    // reproduce is the one that has to explain itself the first time.
    let note =
...
func kayaPerformClipboardRole(_ role: String) -> Bool {
        ...
        case "cut", "copy":
            ...
            //
            // NOT COMPOSED BY KAYA, and that is a real limit rather than
            // a formality: UIKit builds this clip down the responder
            // chain, so it carries no marker, and on this platform the
            // marker is the only evidence there is — the witness stands
            // down until kaya composes the next clip. KAYA WILL NOT
            // STAMP A BOARD IT DID NOT COMPOSE to close that hole: a
            // responder that refused leaves the PREVIOUS clip in place,
            // which may be a stranger's, and stamping it would mark a
            // stranger's board as kaya's own — a false OK, which is the
            // one verdict this guard must never produce. No scene
            // reaches this arm today (no .steps activates Edit>Cut or
            // Edit>Copy), so the alternative would also be an unwatched
            // branch.
            kayaClipStaging()
            kayaSendToFocusedResponder(
                role == "cut"
...
func kayaToolbarChromeFits(_ spelling: String) -> String? {
    ...
    /// hierarchy meets them. SwiftUI's `.toolbar` content on iOS is
    /// hosted by the enclosing NavigationStack's UINavigationBar, so a
    /// promoted action ends up there as a real UIBarButtonItem — the
    /// same question `window.toolbar?.items` answers on macOS, asked of
    /// the object graph this platform actually builds.
    ...
    /// One real bar button, reduced to what the harness asks about —
    /// every field taken off the object UIKit built, none off kaya's
    /// model.
    struct KayaToolbarIOSButton {
        /// The spoken name, i.e. what an assistive client is given.
        let name: String
        /// THE GLYPH THIS BUTTON IS REALLY DRAWING, as UIKit names it:
        /// the SF spelling it publishes on the rendered UIImageView's
        /// accessibility identifier. nil is a REAL answer and the one
        /// that matters — this button renders no symbol image at all.
        let sfDrawn: String?
        /// The `kaya-toolbar-symbol:` / `kaya-toolbar-more` identifier
        /// the rendering arm published. NOT the symbol answer (that is
        /// `sfDrawn`, which is UIKit's own naming rather than kaya's
        /// claim); it names WHICH ARM drew, which is what a failure
        /// sentence needs to tell "no symbol declared" from "declared
        /// and undrawable".
        let ident: String?
        /// Enablement from the `notEnabled` trait on the BUTTON element.
        /// Measured to move — 1 (button) before the scene's disable and
        /// 257 (button|notEnabled) after it — while the nested
        /// `_UIModernBarButton` reads 257 throughout, which is why this
        /// is taken off the outermost button and the walk does not
        /// descend into one.
        let enabled: Bool
    }

    /// EVERY BAR BUTTON THE REAL CHROME IS SHOWING, in the order an
    /// assistive client meets them (measured to be left to right: the
    /// hosted PlatterItemViews sit at x=4, 58, 113 in that traversal
    /// order). Order matters — promotion is catalog PREORDER and a bar
    /// holding the right set in the wrong sequence is not the same
    /// lowering.
    ///
    /// THE ROUTE IS MEASURED, NOT ASSUMED, and it is not the macOS one.
    /// SwiftUI's `UIKitNavigationBar` on this OS publishes NO bar button
    /// items whatsoever — `topItem` is nil and `rightBarButtonItems`
    /// with it (measured 2026-08-17, KAYA_TOOLBAR_TRACE) — because
    /// `.toolbar` content is hosted as views inside the bar rather than
    /// bridged through `UINavigationItem`. So the walk is the
    /// accessibility tree under the bar, which is where those hosted
    /// views publish themselves, and it is the same surface a VoiceOver
    /// user is given.
    ///
    /// OUTERMOST BUTTONS ONLY: `_UIButtonBarButton` wraps a
    /// `_UIModernBarButton` that also carries the button trait, and the
    /// inner one reports `notEnabled` even while the button is live. A
    /// walk that descended would double every item and answer
    /// enablement off an element that never moves.
    ...
    private func kayaToolbarIOSReduce(_ button: NSObject) -> KayaToolbarIOSButton {
        let name =
            ...
            .compactMap { $0 }
            .first { !$0.isEmpty } ?? ""
        // THE RENDERED GLYPH. UIKit publishes the SF symbol name on the
        // image view it built for the glyph (`ident=checkmark` under the
        // Save button, `ident=magnifyingglass` under Find — measured),
        // so this is the platform's own record of what is on screen. A
        // button that fell to its text arm has no image view here at
        // all, which is exactly the difference the stamp could not see.
        var sfDrawn: String?
    ...
    /// The SEMANTIC name for a glyph the chrome is really drawing, by
    /// inverting the one table the lowering drew through. nil when no
    ...
    /// arm perturbed to draw an off-table glyph would also fail to
    /// invert, and would then be answered from kaya's claim — the read
    ...
    /// what was measured instead. Shared by expect_toolbar_item and by
    /// expect_menu_symbol's promoted half — one read, so the two verbs
    /// cannot disagree about the same button.
    func kayaToolbarIOSSymbolOf(_ button: KayaToolbarIOSButton) -> String {
        guard let sf = button.sfDrawn else {
            // WHAT THIS MEASURED: the button is on the real bar and
            // UIKit built no glyph image under it. The arm's own
            // identifier rides along when there is one, because it is
            // what tells "the app declared no symbol" from "declared one
            // this OS would not draw" — and it is a REPORT of a second
    ...
    /// The identifier the More menu's trigger publishes, so the chrome
    /// read can find the remainder's home on the REAL bar without
    /// matching an English word — the label "More" is dress and a
    /// localized build would move it.
    let kayaToolbarMoreIdent = "kaya-toolbar-more"

    /// THE expect_toolbar READ on iOS: `<promoted found>/<promoted in
    /// the catalog>/<items on the bar>/<remainder's home>`, the macOS
    /// arm's spelling exactly, off this platform's own object graph.
    ///
    /// The first number walks the REAL bar buttons IN ORDER against the
    /// promoted list's labels; the second is the promoted list, computed
    /// from the catalog by the same helper the bar itself consumes. They
    /// come from two different sides ON PURPOSE — an answer computed
    /// once and reported twice would agree with itself, and "the
    /// promotion list reached no chrome" is precisely the failure being
    /// gated. The third is what the bar really holds, so the failure can
    /// tell a bar that never materialized from one holding the wrong
    /// buttons.
    ///
    /// The remainder's home on a compact iOS window is the MORE menu the
    /// same lowering builds, and this LOOKS FOR IT on the bar rather
    /// than assuming the arm that should have built it ran.
    ...
    /// THE expect_toolbar_item READ on iOS: one aspect of the real bar
    /// button, addressed by the name UIKit publishes for it.
    ///
    /// TOTAL, like kayaMenuStateRead: every failure is a short
    /// description and a retryable non-match, never a panic.
    func kayaToolbarIOSItemRead(_ windowId: UInt64, _ label: String, _ aspect: String) -> String {
        ...
        if aspect == "enabled" || aspect == "disabled" {
            // THE MEASURED PROPERTY: the `notEnabled` trait on the real
            // button element, which is where SwiftUI's `.disabled`
            // lands and the surface a VoiceOver user is given. UIKit
            // offers no second flag to prefer it over here — this bar
            // has no UIBarButtonItem at all (see kayaToolbarIOSButtons)
            // — so unlike the macOS sibling there is no disagreement to
            // report, only one publication and this is it.
            return hit.enabled ? "enabled" : "disabled"
        }
        return kayaToolbarIOSSymbolOf(hit)
    }

    /// KAYA_TOOLBAR_TRACE=1 dumps every property of the real bar this
    /// read could have consulted, plus the accessibility subtree under
    /// each button. It is how the enablement and symbol questions were
    /// ANSWERED rather than guessed — run the toolbar scene once, which
    /// flips Save's enablement halfway through, and read which property
    /// moved. The macOS sibling of this function is what found that
    ...
    func kayaToolbarIOSTrace(_ windowId: UInt64) {
        ...
        // identifier is what the read inverts; the image's description
        // is what says whether a name that failed to invert was
        // renamed under the table or drawn off it — the two causes the
        // failure sentence names and cannot itself tell apart.
        for bar in kayaToolbarIOSBars() {
        ...
        // THE WHOLE SUBTREE, which is how the route above was CHOSEN:
        // the UIBarButtonItem route this trace was first written for
        // dumped nothing at all (topItem nil), and these lines are where
        // the buttons, their traits and the glyph identifiers were
        // found instead.
        for bar in kayaToolbarIOSBars() {
            var seen = Set<ObjectIdentifier>()
...
///
/// BOTH LOWERINGS PUBLISH IT, which is why it is not inside either
/// platform's block: the mac arm hands it to AppKit, which carries it
/// onto the real AXButton, and the iOS arm hands it to SwiftUI, which
/// carries it onto the hosted `_UIButtonBarButton` element. One prefix,
/// one meaning, two hosts — measured on each.
///
/// WHAT IT IS WORTH DIFFERS BY HOST, and the difference is the point.
/// On macOS it is the symbol ANSWER, because nothing on that lowering
/// names the glyph (see KayaPromotedLabelMac). On iOS it is only a
/// DIAGNOSTIC: UIKit publishes the SF name on the rendered image view
/// itself, so the answer there is read off the glyph and this string
/// merely says which arm drew when there is no glyph to read.
                ...
                content.toolbar {
                    ToolbarItemGroup(placement: .primaryAction) {
                        ...
                        Menu {
                            ...
                            // The trigger glyph is dress; the IDENTIFIER
                            // is not. It is how the chrome read finds
                            // the remainder's home on the real bar
                            // without matching an English word.
