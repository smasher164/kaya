...
/// <summary>SearchItem's generated record surface: the collection
...
static class SearchItemKaya
{
    public static RecordCollection<SearchItem> Collection(Tx tx) =>
        tx.CollectionOf<SearchItem>();

    public static readonly Field<string> Name = KayaRecords.FieldAt<string>(0);

    public static SearchItemKayaPatch Patch(Tx tx, RecordCollection<SearchItem> c, object key) =>
        new SearchItemKayaPatch(c.Patch(tx, key));
    ...
    /// replay.</summary>
    public static Widget Each(Tx tx, RecordCollection<SearchItem> c,
        System.Action<SearchItemRow> body) =>
        tx.Each(c.Collection, t => body(new SearchItemRow(t)));
    ...
    public static Node Each(Tpl t, RecordCollection<SearchItem> c,
        System.Action<SearchItemRow> body) =>
        t.Each(c.Collection, inner => body(new SearchItemRow(inner)));
    ...
    public static SearchItemRowSeq Rows(this RecordCollection<SearchItem> c) =>
        new SearchItemRowSeq(c);
}
...
sealed class SearchItemRow
{
    readonly Tpl t;
    public readonly Field<string> Name = SearchItemKaya.Name;

    internal SearchItemRow(Tpl t) => this.t = t;
...
readonly struct SearchItemRowSeq
{
    readonly RecordCollection<SearchItem> c;

    internal SearchItemRowSeq(RecordCollection<SearchItem> c) => this.c = c;

    public SearchItemRowEnumerator GetEnumerator() => new SearchItemRowEnumerator(c);
}

sealed class SearchItemRowEnumerator : System.IDisposable
{
    int state;
    System.Action? close;
    readonly RecordCollection<SearchItem> c;

    internal SearchItemRowEnumerator(RecordCollection<SearchItem> c) => this.c = c;

    public SearchItemRow Current { get; private set; } = default!;
...
sealed class SearchItemKayaPatch
{
    readonly RecordPatch<SearchItem> p;

    internal SearchItemKayaPatch(RecordPatch<SearchItem> p) => this.p = p;

    public SearchItemKayaPatch Name(string v)
    {
        p.Set(SearchItemKaya.Name, v);
        return this;
    }
}
