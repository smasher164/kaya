...
/// <summary>ChatThread's generated record surface: the collection
...
static class ChatThreadKaya
{
    public static RecordCollection<ChatThread> Collection(Tx tx) =>
        tx.CollectionOf<ChatThread>();

    public static readonly Field<string> Title = KayaRecords.FieldAt<string>(0);

    public static ChatThreadKayaPatch Patch(Tx tx, RecordCollection<ChatThread> c, object key) =>
        new ChatThreadKayaPatch(c.Patch(tx, key));
    ...
    /// replay.</summary>
    public static Widget Each(Tx tx, RecordCollection<ChatThread> c,
        System.Action<ChatThreadRow> body) =>
        tx.Each(c.Collection, t => body(new ChatThreadRow(t)));
    ...
    public static Node Each(Tpl t, RecordCollection<ChatThread> c,
        System.Action<ChatThreadRow> body) =>
        t.Each(c.Collection, inner => body(new ChatThreadRow(inner)));
    ...
    public static ChatThreadRowSeq Rows(this RecordCollection<ChatThread> c) =>
        new ChatThreadRowSeq(c);
}
...
{
    readonly Tpl t;
    public readonly Field<string> Title = ChatThreadKaya.Title;

    internal ChatThreadRow(Tpl t) => this.t = t;
...
readonly struct ChatThreadRowSeq
{
    readonly RecordCollection<ChatThread> c;

    internal ChatThreadRowSeq(RecordCollection<ChatThread> c) => this.c = c;

    public ChatThreadRowEnumerator GetEnumerator() => new ChatThreadRowEnumerator(c);
}

sealed class ChatThreadRowEnumerator : System.IDisposable
{
    int state;
    System.Action? close;
    readonly RecordCollection<ChatThread> c;

    internal ChatThreadRowEnumerator(RecordCollection<ChatThread> c) => this.c = c;

    public ChatThreadRow Current { get; private set; } = default!;
...
sealed class ChatThreadKayaPatch
{
    readonly RecordPatch<ChatThread> p;

    internal ChatThreadKayaPatch(RecordPatch<ChatThread> p) => this.p = p;

    public ChatThreadKayaPatch Title(string v)
    {
        p.Set(ChatThreadKaya.Title, v);
        return this;
    }
}
