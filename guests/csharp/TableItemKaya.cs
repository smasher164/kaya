...
/// <summary>TableItem's generated record surface: the collection
...
static class TableItemKaya
{
    public static RecordCollection<TableItem> Collection(Tx tx) =>
        tx.CollectionOf<TableItem>();

    public static readonly Field<string> Name = KayaRecords.FieldAt<string>(0);
    public static readonly Field<string> Size = KayaRecords.FieldAt<string>(1);

    public static TableItemKayaPatch Patch(Tx tx, RecordCollection<TableItem> c, object key) =>
        new TableItemKayaPatch(c.Patch(tx, key));
    ...
    public static TableItemRowSeq Rows(this RecordCollection<TableItem> c) =>
        new TableItemRowSeq(c);
}
...
sealed class TableItemRow
{
    readonly Tpl t;
    public readonly Field<string> Name = TableItemKaya.Name;
    public readonly Field<string> Size = TableItemKaya.Size;

    internal TableItemRow(Tpl t) => this.t = t;
...
readonly struct TableItemRowSeq
{
    readonly RecordCollection<TableItem> c;

    internal TableItemRowSeq(RecordCollection<TableItem> c) => this.c = c;

    public TableItemRowEnumerator GetEnumerator() => new TableItemRowEnumerator(c);
}

sealed class TableItemRowEnumerator : System.IDisposable
{
    int state;
    System.Action close;
    readonly RecordCollection<TableItem> c;

    internal TableItemRowEnumerator(RecordCollection<TableItem> c) => this.c = c;

    public TableItemRow Current { get; private set; }
...
sealed class TableItemKayaPatch
{
    readonly RecordPatch<TableItem> p;

    internal TableItemKayaPatch(RecordPatch<TableItem> p) => this.p = p;

    public TableItemKayaPatch Name(string v)
    {
        p.Set(TableItemKaya.Name, v);
        return this;
    }

    public TableItemKayaPatch Size(string v)
    {
        p.Set(TableItemKaya.Size, v);
        return this;
    }
}
