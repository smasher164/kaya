...
/// <summary>Track's generated record surface: the collection
...
static class TrackKaya
{
    public static RecordCollection<Track> Collection(Tx tx) =>
        tx.CollectionOf<Track>();

    public static readonly Field<string> Name = KayaRecords.FieldAt<string>(0);
    public static readonly Field<double> Level = KayaRecords.FieldAt<double>(1);

    public static TrackKayaPatch Patch(Tx tx, RecordCollection<Track> c, object key) =>
        new TrackKayaPatch(c.Patch(tx, key));
    ...
    /// replay.</summary>
    public static Widget Each(Tx tx, RecordCollection<Track> c,
        System.Action<TrackRow> body) =>
        tx.Each(c.Collection, t => body(new TrackRow(t)));
    ...
    public static Node Each(Tpl t, RecordCollection<Track> c,
        System.Action<TrackRow> body) =>
        t.Each(c.Collection, inner => body(new TrackRow(inner)));
    ...
    public static TrackRowSeq Rows(this RecordCollection<Track> c) =>
        new TrackRowSeq(c);
}
...
{
    readonly Tpl t;
    public readonly Field<string> Name = TrackKaya.Name;
    public readonly Field<double> Level = TrackKaya.Level;

    internal TrackRow(Tpl t) => this.t = t;
...
readonly struct TrackRowSeq
{
    readonly RecordCollection<Track> c;

    internal TrackRowSeq(RecordCollection<Track> c) => this.c = c;

    public TrackRowEnumerator GetEnumerator() => new TrackRowEnumerator(c);
}

sealed class TrackRowEnumerator : System.IDisposable
{
    int state;
    System.Action? close;
    readonly RecordCollection<Track> c;

    internal TrackRowEnumerator(RecordCollection<Track> c) => this.c = c;

    public TrackRow Current { get; private set; } = default!;
...
sealed class TrackKayaPatch
{
    readonly RecordPatch<Track> p;

    internal TrackKayaPatch(RecordPatch<Track> p) => this.p = p;

    public TrackKayaPatch Name(string v)
    {
        p.Set(TrackKaya.Name, v);
        return this;
    }

    public TrackKayaPatch Level(double v)
    {
        p.Set(TrackKaya.Level, v);
        return this;
    }
}
