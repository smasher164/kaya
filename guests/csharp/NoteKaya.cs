...
/// <summary>Note's generated record surface: the collection
...
static class NoteKaya
{
    public static RecordCollection<Note> Collection(Tx tx) =>
        tx.CollectionOf<Note>();

    public static readonly Field<string> Title = KayaRecords.FieldAt<string>(0);
    public static readonly Field<Document> Body = KayaRecords.FieldAt<Document>(1);

    public static NoteKayaPatch Patch(Tx tx, RecordCollection<Note> c, object key) =>
        new NoteKayaPatch(c.Patch(tx, key));
    ...
    /// replay.</summary>
    public static Widget Each(Tx tx, RecordCollection<Note> c,
        System.Action<NoteRow> body) =>
        tx.Each(c.Collection, t => body(new NoteRow(t)));
    ...
    public static Node Each(Tpl t, RecordCollection<Note> c,
        System.Action<NoteRow> body) =>
        t.Each(c.Collection, inner => body(new NoteRow(inner)));
    ...
    public static NoteRowSeq Rows(this RecordCollection<Note> c) =>
        new NoteRowSeq(c);
}
...
{
    readonly Tpl t;
    public readonly Field<string> Title = NoteKaya.Title;
    public readonly Field<Document> Body = NoteKaya.Body;

    internal NoteRow(Tpl t) => this.t = t;
...
readonly struct NoteRowSeq
{
    readonly RecordCollection<Note> c;

    internal NoteRowSeq(RecordCollection<Note> c) => this.c = c;

    public NoteRowEnumerator GetEnumerator() => new NoteRowEnumerator(c);
}

sealed class NoteRowEnumerator : System.IDisposable
{
    int state;
    System.Action? close;
    readonly RecordCollection<Note> c;

    internal NoteRowEnumerator(RecordCollection<Note> c) => this.c = c;

    public NoteRow Current { get; private set; } = default!;
...
sealed class NoteKayaPatch
{
    readonly RecordPatch<Note> p;

    internal NoteKayaPatch(RecordPatch<Note> p) => this.p = p;

    public NoteKayaPatch Title(string v)
    {
        p.Set(NoteKaya.Title, v);
        return this;
    }

    public NoteKayaPatch Body(Document v)
    {
        p.Set(NoteKaya.Body, v);
        return this;
    }
}
