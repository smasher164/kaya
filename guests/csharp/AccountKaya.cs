...
/// <summary>Account's generated record surface: the collection
...
static class AccountKaya
{
    public static RecordCollection<Account> Collection(Tx tx) =>
        tx.CollectionOf<Account>();

    public static readonly Field<string> Name = KayaRecords.FieldAt<string>(0);
    public static readonly Field<string> Note = KayaRecords.FieldAt<string>(1);

    public static AccountKayaPatch Patch(Tx tx, RecordCollection<Account> c, object key) =>
        new AccountKayaPatch(c.Patch(tx, key));
    ...
    /// replay.</summary>
    public static Widget Each(Tx tx, RecordCollection<Account> c,
        System.Action<AccountRow> body) =>
        tx.Each(c.Collection, t => body(new AccountRow(t)));
    ...
    public static Node Each(Tpl t, RecordCollection<Account> c,
        System.Action<AccountRow> body) =>
        t.Each(c.Collection, inner => body(new AccountRow(inner)));
    ...
    public static AccountRowSeq Rows(this RecordCollection<Account> c) =>
        new AccountRowSeq(c);
}
...
{
    readonly Tpl t;
    public readonly Field<string> Name = AccountKaya.Name;
    public readonly Field<string> Note = AccountKaya.Note;

    internal AccountRow(Tpl t) => this.t = t;
...
readonly struct AccountRowSeq
{
    readonly RecordCollection<Account> c;

    internal AccountRowSeq(RecordCollection<Account> c) => this.c = c;

    public AccountRowEnumerator GetEnumerator() => new AccountRowEnumerator(c);
}
...
sealed class AccountKayaPatch
{
    readonly RecordPatch<Account> p;

    internal AccountKayaPatch(RecordPatch<Account> p) => this.p = p;

    public AccountKayaPatch Name(string v)
    {
        p.Set(AccountKaya.Name, v);
        return this;
    }

    public AccountKayaPatch Note(string v)
    {
        p.Set(AccountKaya.Note, v);
        return this;
    }
}
