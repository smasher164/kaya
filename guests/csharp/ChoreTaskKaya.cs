...
/// <summary>ChoreTask's generated record surface: the collection
...
static class ChoreTaskKaya
{
    public static RecordCollection<ChoreTask> Collection(Tx tx) =>
        tx.CollectionOf<ChoreTask>();
    ...
    public static ChoreTaskKayaPatch Patch(Tx tx, RecordCollection<ChoreTask> c, object key) =>
        new ChoreTaskKayaPatch(c.Patch(tx, key));
    ...
    /// replay.</summary>
    public static Widget Each(Tx tx, RecordCollection<ChoreTask> c,
        System.Action<ChoreTaskRow> body) =>
        tx.Each(c.Collection, t => body(new ChoreTaskRow(t)));
    ...
    public static Node Each(Tpl t, RecordCollection<ChoreTask> c,
        System.Action<ChoreTaskRow> body) =>
        t.Each(c.Collection, inner => body(new ChoreTaskRow(inner)));
    ...
    public static ChoreTaskRowSeq Rows(this RecordCollection<ChoreTask> c) =>
        new ChoreTaskRowSeq(c);
}
...
sealed class ChoreTaskRow
{
    readonly Tpl t;
    public readonly Field<string> Name = ChoreTaskKaya.Name;
    public readonly Field<System.DateOnly> Due = ChoreTaskKaya.Due;

    internal ChoreTaskRow(Tpl t) => this.t = t;
...
readonly struct ChoreTaskRowSeq
{
    readonly RecordCollection<ChoreTask> c;

    internal ChoreTaskRowSeq(RecordCollection<ChoreTask> c) => this.c = c;

    public ChoreTaskRowEnumerator GetEnumerator() => new ChoreTaskRowEnumerator(c);
}

sealed class ChoreTaskRowEnumerator : System.IDisposable
{
    int state;
    System.Action? close;
    readonly RecordCollection<ChoreTask> c;

    internal ChoreTaskRowEnumerator(RecordCollection<ChoreTask> c) => this.c = c;

    public ChoreTaskRow Current { get; private set; } = default!;
...
sealed class ChoreTaskKayaPatch
{
    readonly RecordPatch<ChoreTask> p;

    internal ChoreTaskKayaPatch(RecordPatch<ChoreTask> p) => this.p = p;

    public ChoreTaskKayaPatch Name(string v)
    {
        p.Set(ChoreTaskKaya.Name, v);
        return this;
    }

    public ChoreTaskKayaPatch Due(System.DateOnly v)
    {
        p.Set(ChoreTaskKaya.Due, v);
        return this;
    }
}
