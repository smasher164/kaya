...
/// <summary>ChatMessage's generated record surface: the collection
...
static class ChatMessageKaya
{
    public static RecordCollection<ChatMessage> Collection(Tx tx) =>
        tx.CollectionOf<ChatMessage>();

    public static readonly Field<string> Text = KayaRecords.FieldAt<string>(0);

    public static ChatMessageKayaPatch Patch(Tx tx, RecordCollection<ChatMessage> c, object key) =>
        new ChatMessageKayaPatch(c.Patch(tx, key));
    ...
    /// replay.</summary>
    public static Widget Each(Tx tx, RecordCollection<ChatMessage> c,
        System.Action<ChatMessageRow> body) =>
        tx.Each(c.Collection, t => body(new ChatMessageRow(t)));
    ...
    public static Node Each(Tpl t, RecordCollection<ChatMessage> c,
        System.Action<ChatMessageRow> body) =>
        t.Each(c.Collection, inner => body(new ChatMessageRow(inner)));
    ...
    public static ChatMessageRowSeq Rows(this RecordCollection<ChatMessage> c) =>
        new ChatMessageRowSeq(c);
}
...
sealed class ChatMessageRow
{
    readonly Tpl t;
    public readonly Field<string> Text = ChatMessageKaya.Text;

    internal ChatMessageRow(Tpl t) => this.t = t;
...
readonly struct ChatMessageRowSeq
{
    readonly RecordCollection<ChatMessage> c;

    internal ChatMessageRowSeq(RecordCollection<ChatMessage> c) => this.c = c;

    public ChatMessageRowEnumerator GetEnumerator() => new ChatMessageRowEnumerator(c);
}

sealed class ChatMessageRowEnumerator : System.IDisposable
{
    int state;
    System.Action? close;
    readonly RecordCollection<ChatMessage> c;

    internal ChatMessageRowEnumerator(RecordCollection<ChatMessage> c) => this.c = c;

    public ChatMessageRow Current { get; private set; } = default!;
...
sealed class ChatMessageKayaPatch
{
    readonly RecordPatch<ChatMessage> p;

    internal ChatMessageKayaPatch(RecordPatch<ChatMessage> p) => this.p = p;

    public ChatMessageKayaPatch Text(string v)
    {
        p.Set(ChatMessageKaya.Text, v);
        return this;
    }
}
