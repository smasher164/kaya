...
/** SearchItem's generated record surface: the collection factory,
 * exact-index field tokens, and a named-setter patch (each set
 * records one update_field — a patch is recorded writes, never a
 * diff). */
final class SearchItemKaya {
    static KayaRecords.Collection<String, Search.SearchItem> collection(KayaApp.Tx tx) {
        return KayaRecords.collectionOf(tx, Search.SearchItem.class);
    }

    static final KayaRecords.Field<String> NAME = KayaRecords.fieldAt(0);

    static Patch patch(KayaApp.Tx tx, KayaRecords.Collection<String, Search.SearchItem> c, String key) {
        return new Patch(c.patch(tx, key));
    }

    static final class Patch {
        private final KayaRecords.Patch<String, Search.SearchItem> p;

        Patch(KayaRecords.Patch<String, Search.SearchItem> p) {
            this.p = p;
        }
    ...
    /** The record template: `for (var row : SearchItemKaya.rows(tx, c))`
    ...
    static KayaApp.Rows<KayaApp.Widget, Row> rows(
            KayaApp.Tx tx, KayaRecords.Collection<String, Search.SearchItem> c) {
    ...
    static KayaApp.Rows<KayaApp.Node, Row> rows(
            KayaApp.RowSurface row, KayaRecords.Collection<String, Search.SearchItem> c) {
    ...
    static final class Row extends KayaApp.RowSurface {
        private final KayaRecords.Collection<String, Search.SearchItem> c;
        final KayaRecords.Field<String> name = NAME;

        Row(KayaApp.Tpl t, KayaRecords.Collection<String, Search.SearchItem> c) {
            super(t);
            this.c = c;
        }
