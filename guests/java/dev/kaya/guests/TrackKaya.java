...
/** Track's generated record surface: the collection factory,
 * exact-index field tokens, and a named-setter patch (each set
 * records one update_field — a patch is recorded writes, never a
 * diff). */
final class TrackKaya {
    static KayaRecords.Collection<String, Sliders.Track> collection(KayaApp.Tx tx) {
        return KayaRecords.collectionOf(tx, Sliders.Track.class);
    }

    static final KayaRecords.Field<String> NAME = KayaRecords.fieldAt(0);
    static final KayaRecords.Field<Double> LEVEL = KayaRecords.fieldAt(1);

    static Patch patch(KayaApp.Tx tx, KayaRecords.Collection<String, Sliders.Track> c, String key) {
        return new Patch(c.patch(tx, key));
    }

    static final class Patch {
        private final KayaRecords.Patch<String, Sliders.Track> p;

        Patch(KayaRecords.Patch<String, Sliders.Track> p) {
            this.p = p;
        }
        ...
        Patch level(double v) {
            p.set(LEVEL, v);
            return this;
        }
    }

    /** The record template: `for (var row : TrackKaya.rows(tx, c))`
    ...
    static KayaApp.Rows<KayaApp.Widget, Row> rows(
            KayaApp.Tx tx, KayaRecords.Collection<String, Sliders.Track> c) {
    ...
    static KayaApp.Rows<KayaApp.Node, Row> rows(
            KayaApp.RowSurface row, KayaRecords.Collection<String, Sliders.Track> c) {
    ...
    static final class Row extends KayaApp.RowSurface {
        private final KayaRecords.Collection<String, Sliders.Track> c;
        final KayaRecords.Field<String> name = NAME;
        final KayaRecords.Field<Double> level = LEVEL;

        Row(KayaApp.Tpl t, KayaRecords.Collection<String, Sliders.Track> c) {
            super(t);
            this.c = c;
        }
