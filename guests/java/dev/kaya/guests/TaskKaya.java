...
/** Task's generated record surface: the collection factory,
 * exact-index field tokens, and a named-setter patch (each set
 * records one update_field — a patch is recorded writes, never a
 * diff). */
final class TaskKaya {
    static KayaRecords.Collection<String, Pickers.Task> collection(KayaApp.Tx tx) {
        return KayaRecords.collectionOf(tx, Pickers.Task.class);
    }

    static final KayaRecords.Field<String> NAME = KayaRecords.fieldAt(0);
    static final KayaRecords.Field<java.time.LocalDate> DUE = KayaRecords.fieldAt(1);

    static Patch patch(KayaApp.Tx tx, KayaRecords.Collection<String, Pickers.Task> c, String key) {
        return new Patch(c.patch(tx, key));
    }

    static final class Patch {
        private final KayaRecords.Patch<String, Pickers.Task> p;

        Patch(KayaRecords.Patch<String, Pickers.Task> p) {
            this.p = p;
        }
        ...
        Patch due(java.time.LocalDate v) {
            p.set(DUE, v);
            return this;
        }
    }

    /** The record template: `for (var row : TaskKaya.rows(tx, c))`
    ...
    static KayaApp.Rows<KayaApp.Widget, Row> rows(
            KayaApp.Tx tx, KayaRecords.Collection<String, Pickers.Task> c) {
    ...
    static KayaApp.Rows<KayaApp.Node, Row> rows(
            KayaApp.RowSurface row, KayaRecords.Collection<String, Pickers.Task> c) {
    ...
    static final class Row extends KayaApp.RowSurface {
        private final KayaRecords.Collection<String, Pickers.Task> c;
        final KayaRecords.Field<String> name = NAME;
        final KayaRecords.Field<java.time.LocalDate> due = DUE;

        Row(KayaApp.Tpl t, KayaRecords.Collection<String, Pickers.Task> c) {
            super(t);
            this.c = c;
        }
