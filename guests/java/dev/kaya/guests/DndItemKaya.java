...
/** DndItem's generated record surface: the collection factory,
 * exact-index field tokens, and a named-setter patch (each set
 * records one update_field — a patch is recorded writes, never a
 * diff). */
final class DndItemKaya {
    static KayaRecords.Collection<String, Dnd.DndItem> collection(KayaApp.Tx tx) {
        return KayaRecords.collectionOf(tx, Dnd.DndItem.class);
    }

    static final KayaRecords.Field<String> TITLE = KayaRecords.fieldAt(0);

    static Patch patch(KayaApp.Tx tx, KayaRecords.Collection<String, Dnd.DndItem> c, String key) {
        return new Patch(c.patch(tx, key));
    }

    static final class Patch {
        private final KayaRecords.Patch<String, Dnd.DndItem> p;

        Patch(KayaRecords.Patch<String, Dnd.DndItem> p) {
            this.p = p;
        }
    ...
    /** The record template: `for (var row : DndItemKaya.rows(tx, c))`
    ...
    static KayaApp.Rows<KayaApp.Widget, Row> rows(
            KayaApp.Tx tx, KayaRecords.Collection<String, Dnd.DndItem> c) {
    ...
    static KayaApp.Rows<KayaApp.Node, Row> rows(
            KayaApp.RowSurface row, KayaRecords.Collection<String, Dnd.DndItem> c) {
    ...
    static final class Row extends KayaApp.RowSurface {
        private final KayaRecords.Collection<String, Dnd.DndItem> c;
        final KayaRecords.Field<String> title = TITLE;

        Row(KayaApp.Tpl t, KayaRecords.Collection<String, Dnd.DndItem> c) {
            super(t);
            this.c = c;
        }
