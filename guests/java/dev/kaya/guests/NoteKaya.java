...
/** Note's generated record surface: the collection factory,
 * exact-index field tokens, and a named-setter patch (each set
 * records one update_field — a patch is recorded writes, never a
 * diff). */
final class NoteKaya {
    static KayaRecords.Collection<String, Richrows.Note> collection(KayaApp.Tx tx) {
        return KayaRecords.collectionOf(tx, Richrows.Note.class);
    }

    static final KayaRecords.Field<String> TITLE = KayaRecords.fieldAt(0);
    static final KayaRecords.Field<dev.kaya.KayaApp.Document> BODY = KayaRecords.fieldAt(1);

    static Patch patch(KayaApp.Tx tx, KayaRecords.Collection<String, Richrows.Note> c, String key) {
        return new Patch(c.patch(tx, key));
    }

    static final class Patch {
        private final KayaRecords.Patch<String, Richrows.Note> p;

        Patch(KayaRecords.Patch<String, Richrows.Note> p) {
            this.p = p;
        }
        ...
        Patch body(dev.kaya.KayaApp.Document v) {
            p.set(BODY, v);
            return this;
        }
    }

    /** The record template: `for (var row : NoteKaya.rows(tx, c))`
    ...
    static KayaApp.Rows<KayaApp.Widget, Row> rows(
            KayaApp.Tx tx, KayaRecords.Collection<String, Richrows.Note> c) {
    ...
    static KayaApp.Rows<KayaApp.Node, Row> rows(
            KayaApp.RowSurface row, KayaRecords.Collection<String, Richrows.Note> c) {
    ...
    static final class Row extends KayaApp.RowSurface {
        private final KayaRecords.Collection<String, Richrows.Note> c;
        final KayaRecords.Field<String> title = TITLE;
        final KayaRecords.Field<dev.kaya.KayaApp.Document> body = BODY;

        Row(KayaApp.Tpl t, KayaRecords.Collection<String, Richrows.Note> c) {
            super(t);
            this.c = c;
        }
