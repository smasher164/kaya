...
/** SubmitThread's generated record surface: the collection factory,
 * exact-index field tokens, and a named-setter patch (each set
 * records one update_field — a patch is recorded writes, never a
 * diff). */
final class SubmitThreadKaya {
    static KayaRecords.Collection<String, Submit.SubmitThread> collection(KayaApp.Tx tx) {
        return KayaRecords.collectionOf(tx, Submit.SubmitThread.class);
    }

    static final KayaRecords.Field<String> TITLE = KayaRecords.fieldAt(0);

    static Patch patch(KayaApp.Tx tx, KayaRecords.Collection<String, Submit.SubmitThread> c, String key) {
        return new Patch(c.patch(tx, key));
    }

    static final class Patch {
        private final KayaRecords.Patch<String, Submit.SubmitThread> p;

        Patch(KayaRecords.Patch<String, Submit.SubmitThread> p) {
            this.p = p;
        }
    ...
    /** The record template: `for (var row : SubmitThreadKaya.rows(tx, c))`
    ...
    static KayaApp.Rows<KayaApp.Widget, Row> rows(
            KayaApp.Tx tx, KayaRecords.Collection<String, Submit.SubmitThread> c) {
    ...
    static KayaApp.Rows<KayaApp.Node, Row> rows(
            KayaApp.RowSurface row, KayaRecords.Collection<String, Submit.SubmitThread> c) {
    ...
    static final class Row extends KayaApp.RowSurface {
        private final KayaRecords.Collection<String, Submit.SubmitThread> c;
        final KayaRecords.Field<String> title = TITLE;

        Row(KayaApp.Tpl t, KayaRecords.Collection<String, Submit.SubmitThread> c) {
            super(t);
            this.c = c;
        }
