...
KayaApp.run { app in
    ...
    @KayaAppActor func what(_ label: String) -> String {
        label.isEmpty ? "typing" : label
    }

    /// Every key the collection holds, in order — THE ONLY PART OF AN UNDO A
    /// COUNT CANNOT SEE (D5).
    @KayaAppActor func keyList(_ tx: KayaAppTx, _ todos: KayaRecordCollection<Todo>) -> String {
    ...
    @KayaAppActor func rowKey(_ path: [KayaValue]) -> Int64 {
    ...
    @KayaAppActor func noteList() -> String {
        if rowNotes.isEmpty {
            return "no notes"
        }
    ...
    @KayaAppActor func foldTexts(_ texts: [KayaUndoText]) {
    ...
    app.build { tx in
        ...
        tx.window(
            ...
            onUndone: { tx, label, delta in
                ...
                // label first.
                tx.write(keys, .str(keyList(tx, todos)))
