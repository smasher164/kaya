...
extension Message: KayaRecord {
    /// The prototype Mirror walks for the schema; every field at
    /// its zero value.
    static let prototype = Message(text: "")

    init(values: [KayaValue]) {
        guard case .str(let text) = values[0] else {
            preconditionFailure("kaya: Message fields out of order")
        }
        self.init(text: text)
    }
}

/// Message's typed field tokens.
enum MessageFields {
    static let text = KayaField<String>(index: 0)
}

/// The collection factory; the struct is the schema.
func messageCollection(_ tx: KayaAppTx) -> KayaRecordCollection<Message> {
    tx.collection(of: Message.self)
}
...
extension KayaRecordCollection where T == Message {
    /// The for-statement form: `for row in messages.rows { … }`
    /// traces the record template — the body runs once, and the
    /// tracer plants the For in the enclosing container builder.
    var rows: KayaRowTrace<MessageRow> {
        KayaRowTrace(collection: collection) { MessageRow(t: $0) }
    }
}
...
@discardableResult
func messageEach(
    _ tx: KayaAppTx, _ c: KayaRecordCollection<Message>,
    _ body: @escaping (MessageRow) -> Void
) -> KayaWidget {
